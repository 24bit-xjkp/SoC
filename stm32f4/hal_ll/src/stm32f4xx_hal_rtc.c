...
HAL_StatusTypeDef HAL_RTC_Init(RTC_HandleTypeDef* hrtc)
{
    ...
    if(hrtc->State == HAL_RTC_STATE_RESET)
    {
        ...
        if(hrtc->MspDeInitCallback == NULL) { hrtc->MspDeInitCallback = HAL_RTC_MspDeInit; }
    }
    ...
    if(status == HAL_OK) { hrtc->State = HAL_RTC_STATE_READY; }
    ...
    HAL_RTC_RegisterCallback(RTC_HandleTypeDef* hrtc, HAL_RTC_CallbackIDTypeDef CallbackID, pRTC_CallbackTypeDef pCallback)
{
    ...
    if(HAL_RTC_STATE_READY == hrtc->State)
    {
        switch(CallbackID)
        {
            case HAL_RTC_ALARM_A_EVENT_CB_ID: hrtc->AlarmAEventCallback = pCallback; break;

            case HAL_RTC_ALARM_B_EVENT_CB_ID: hrtc->AlarmBEventCallback = pCallback; break;

            case HAL_RTC_TIMESTAMP_EVENT_CB_ID: hrtc->TimeStampEventCallback = pCallback; break;

            case HAL_RTC_WAKEUPTIMER_EVENT_CB_ID: hrtc->WakeUpTimerEventCallback = pCallback; break;

            case HAL_RTC_TAMPER1_EVENT_CB_ID: hrtc->Tamper1EventCallback = pCallback; break;

        #if defined(RTC_TAMPER2_SUPPORT)
            case HAL_RTC_TAMPER2_EVENT_CB_ID: hrtc->Tamper2EventCallback = pCallback; break;
        #endif /* RTC_TAMPER2_SUPPORT */

            case HAL_RTC_MSPINIT_CB_ID: hrtc->MspInitCallback = pCallback; break;

            case HAL_RTC_MSPDEINIT_CB_ID: hrtc->MspDeInitCallback = pCallback; break;
...
HAL_StatusTypeDef HAL_RTC_UnRegisterCallback(RTC_HandleTypeDef* hrtc, HAL_RTC_CallbackIDTypeDef CallbackID)
{
    ...
    if(HAL_RTC_STATE_READY == hrtc->State)
    {
        switch(CallbackID)
        {
            ...
            case HAL_RTC_MSPINIT_CB_ID: hrtc->MspInitCallback = HAL_RTC_MspInit; break;

            case HAL_RTC_MSPDEINIT_CB_ID: hrtc->MspDeInitCallback = HAL_RTC_MspDeInit; break;
...
HAL_StatusTypeDef HAL_RTC_SetTime(RTC_HandleTypeDef* hrtc, RTC_TimeTypeDef* sTime, uint32_t Format)
{
    ...
    if(Format == RTC_FORMAT_BIN)
    {
        ...
        tmpreg = (uint32_t)(((uint32_t)RTC_ByteToBcd2(sTime->Hours) << RTC_TR_HU_Pos) |
                            ...
                            ((uint32_t)RTC_ByteToBcd2(sTime->Seconds)) | (((uint32_t)sTime->TimeFormat) << RTC_TR_PM_Pos));
    }
    else
    {
        if((hrtc->Instance->CR & RTC_CR_FMT) != 0U)
        ...
        tmpreg = (((uint32_t)(sTime->Hours) << RTC_TR_HU_Pos) | ((uint32_t)(sTime->Minutes) << RTC_TR_MNU_Pos) |
                  ((uint32_t)sTime->Seconds) | ((uint32_t)(sTime->TimeFormat) << RTC_TR_PM_Pos));
    }
...
HAL_StatusTypeDef HAL_RTC_SetDate(RTC_HandleTypeDef* hrtc, RTC_DateTypeDef* sDate, uint32_t Format)
{
    ...
    if(Format == RTC_FORMAT_BIN)
    {
        ...
        datetmpreg = (((uint32_t)RTC_ByteToBcd2(sDate->Year) << RTC_DR_YU_Pos) |
                      ((uint32_t)RTC_ByteToBcd2(sDate->Month) << RTC_DR_MU_Pos) | ((uint32_t)RTC_ByteToBcd2(sDate->Date)) |
    ...
    else
    {
        ...
        datetmpreg = ((((uint32_t)sDate->Year) << RTC_DR_YU_Pos) | (((uint32_t)sDate->Month) << RTC_DR_MU_Pos) |
                      ((uint32_t)sDate->Date) | (((uint32_t)sDate->WeekDay) << RTC_DR_WDU_Pos));
    }
...
HAL_StatusTypeDef HAL_RTC_SetAlarm(RTC_HandleTypeDef* hrtc, RTC_AlarmTypeDef* sAlarm, uint32_t Format)
{
    ...
    if(Format == RTC_FORMAT_BIN)
    {
        ...
        else { assert_param(IS_RTC_ALARM_DATE_WEEKDAY_WEEKDAY(sAlarm->AlarmDateWeekDay)); }

        tmpreg =
            (((uint32_t)RTC_ByteToBcd2(sAlarm->AlarmTime.Hours) << RTC_ALRMAR_HU_Pos) |
             ((uint32_t)RTC_ByteToBcd2(sAlarm->AlarmTime.Minutes) << RTC_ALRMAR_MNU_Pos) |
             ((uint32_t)RTC_ByteToBcd2(sAlarm->AlarmTime.Seconds)) | ((uint32_t)(sAlarm->AlarmTime.TimeFormat) << RTC_TR_PM_Pos) |
             ((uint32_t)RTC_ByteToBcd2(sAlarm->AlarmDateWeekDay) << RTC_ALRMAR_DU_Pos) | ((uint32_t)sAlarm->AlarmDateWeekDaySel) |
    ...
    else
    {
        ...
        else { assert_param(IS_RTC_ALARM_DATE_WEEKDAY_WEEKDAY(RTC_Bcd2ToByte(sAlarm->AlarmDateWeekDay))); }

        tmpreg = (((uint32_t)(sAlarm->AlarmTime.Hours) << RTC_ALRMAR_HU_Pos) |
                  ((uint32_t)(sAlarm->AlarmTime.Minutes) << RTC_ALRMAR_MNU_Pos) | ((uint32_t)sAlarm->AlarmTime.Seconds) |
                  ((uint32_t)(sAlarm->AlarmTime.TimeFormat) << RTC_TR_PM_Pos) |
                  ((uint32_t)(sAlarm->AlarmDateWeekDay) << RTC_ALRMAR_DU_Pos) | ((uint32_t)sAlarm->AlarmDateWeekDaySel) |
                  ((uint32_t)sAlarm->AlarmMask));
    }
