...
HAL_StatusTypeDef HAL_LPTIM_Init(LPTIM_HandleTypeDef* hlptim)
{
    ...
    if(hlptim == NULL) { return HAL_ERROR; }
    ...
    if(hlptim->State == HAL_LPTIM_STATE_RESET)
    {
        ...
        if(hlptim->MspInitCallback == NULL) { hlptim->MspInitCallback = HAL_LPTIM_MspInit; }
    ...
    tmpcfgr &= (uint32_t)(~(LPTIM_CFGR_CKSEL | LPTIM_CFGR_TRIGEN | LPTIM_CFGR_PRELOAD | LPTIM_CFGR_WAVPOL | LPTIM_CFGR_PRESC |
                            LPTIM_CFGR_COUNTMODE));

    /* Set initialization parameters */
    tmpcfgr |= (hlptim->Init.Clock.Source | hlptim->Init.Clock.Prescaler | hlptim->Init.OutputPolarity | hlptim->Init.UpdateMode |
...
HAL_StatusTypeDef HAL_LPTIM_DeInit(LPTIM_HandleTypeDef* hlptim)
{
    ...
    if(HAL_LPTIM_GetState(hlptim) == HAL_LPTIM_STATE_TIMEOUT) { return HAL_TIMEOUT; }

        #if (USE_HAL_LPTIM_REGISTER_CALLBACKS == 1)
    if(hlptim->MspDeInitCallback == NULL) { hlptim->MspDeInitCallback = HAL_LPTIM_MspDeInit; }
...
HAL_StatusTypeDef HAL_LPTIM_PWM_Start(LPTIM_HandleTypeDef* hlptim, uint32_t Period, uint32_t Pulse)
{
    ...
    if(LPTIM_WaitForFlag(hlptim, LPTIM_FLAG_ARROK) == HAL_TIMEOUT) { return HAL_TIMEOUT; }
    ...
    if(LPTIM_WaitForFlag(hlptim, LPTIM_FLAG_CMPOK) == HAL_TIMEOUT) { return HAL_TIMEOUT; }
                                             ...
                                             pLPTIM_CallbackTypeDef pCallback)
{
    ...
    if(hlptim->State == HAL_LPTIM_STATE_READY)
    {
        switch(CallbackID)
        {
            case HAL_LPTIM_MSPINIT_CB_ID: hlptim->MspInitCallback = pCallback; break;

            case HAL_LPTIM_MSPDEINIT_CB_ID: hlptim->MspDeInitCallback = pCallback; break;

            case HAL_LPTIM_COMPARE_MATCH_CB_ID: hlptim->CompareMatchCallback = pCallback; break;

            case HAL_LPTIM_AUTORELOAD_MATCH_CB_ID: hlptim->AutoReloadMatchCallback = pCallback; break;

            case HAL_LPTIM_TRIGGER_CB_ID: hlptim->TriggerCallback = pCallback; break;

            case HAL_LPTIM_COMPARE_WRITE_CB_ID: hlptim->CompareWriteCallback = pCallback; break;

            case HAL_LPTIM_AUTORELOAD_WRITE_CB_ID: hlptim->AutoReloadWriteCallback = pCallback; break;

            case HAL_LPTIM_DIRECTION_UP_CB_ID: hlptim->DirectionUpCallback = pCallback; break;

            case HAL_LPTIM_DIRECTION_DOWN_CB_ID: hlptim->DirectionDownCallback = pCallback; break;
...
static HAL_StatusTypeDef LPTIM_WaitForFlag(const LPTIM_HandleTypeDef* hlptim, uint32_t flag)
{
    ...
    do {
        count--;
        if(count == 0UL) { result = HAL_TIMEOUT; }
    }
...
void LPTIM_Disable(LPTIM_HandleTypeDef* hlptim)
{
    ...
    {
        case LPTIM1_BASE: tmpclksource = __HAL_RCC_GET_LPTIM1_SOURCE(); break;
        default: break;
    }
    ...
    if((tmpCMP != 0UL) || (tmpARR != 0UL))
    {
        ...
        switch((uint32_t)hlptim->Instance)
        {
            case LPTIM1_BASE: __HAL_RCC_LPTIM1_CONFIG(RCC_LPTIM1CLKSOURCE_PCLK1); break;
            default: break;
        }
        ...
        if(tmpCMP != 0UL)
        {
            ...
            /* Wait for the completion of the write operation to the LPTIM_CMP register */
            if(LPTIM_WaitForFlag(hlptim, LPTIM_FLAG_CMPOK) == HAL_TIMEOUT) { hlptim->State = HAL_LPTIM_STATE_TIMEOUT; }
            __HAL_LPTIM_CLEAR_FLAG(hlptim, LPTIM_FLAG_CMPOK);
        }
        ...
        if(tmpARR != 0UL)
        {
            ...
            /* Wait for the completion of the write operation to the LPTIM_ARR register */
            if(LPTIM_WaitForFlag(hlptim, LPTIM_FLAG_ARROK) == HAL_TIMEOUT) { hlptim->State = HAL_LPTIM_STATE_TIMEOUT; }
        ...
        switch((uint32_t)hlptim->Instance)
        {
            case LPTIM1_BASE: __HAL_RCC_LPTIM1_CONFIG(tmpclksource); break;
            default: break;
        }
    }
