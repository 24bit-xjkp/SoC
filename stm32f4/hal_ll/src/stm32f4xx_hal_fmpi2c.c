 ...
 */
        ...
        #define FMPI2C_STATE_MSK                                                                                                 \
            ((uint32_t)((uint32_t)((uint32_t)HAL_FMPI2C_STATE_BUSY_TX | (uint32_t)HAL_FMPI2C_STATE_BUSY_RX) &                    \
        ...
        #define FMPI2C_STATE_MASTER_BUSY_TX                                                                                      \
            ((uint32_t)(((uint32_t)HAL_FMPI2C_STATE_BUSY_TX & FMPI2C_STATE_MSK) | (uint32_t)HAL_FMPI2C_MODE_MASTER))
        /*!< Master Busy TX, combinaison of State LSB and Mode enum */
        #define FMPI2C_STATE_MASTER_BUSY_RX                                                                                      \
            ((uint32_t)(((uint32_t)HAL_FMPI2C_STATE_BUSY_RX & FMPI2C_STATE_MSK) | (uint32_t)HAL_FMPI2C_MODE_MASTER))
        /*!< Master Busy RX, combinaison of State LSB and Mode enum */
        #define FMPI2C_STATE_SLAVE_BUSY_TX                                                                                       \
            ((uint32_t)(((uint32_t)HAL_FMPI2C_STATE_BUSY_TX & FMPI2C_STATE_MSK) | (uint32_t)HAL_FMPI2C_MODE_SLAVE))
        /*!< Slave Busy TX, combinaison of State LSB and Mode enum */
        #define FMPI2C_STATE_SLAVE_BUSY_RX                                                                                       \
            ((uint32_t)(((uint32_t)HAL_FMPI2C_STATE_BUSY_RX & FMPI2C_STATE_MSK) | (uint32_t)HAL_FMPI2C_MODE_SLAVE))
        /*!< Slave Busy RX, combinaison of State LSB and Mode enum  */
        #define FMPI2C_STATE_MEM_BUSY_TX                                                                                         \
            ((uint32_t)(((uint32_t)HAL_FMPI2C_STATE_BUSY_TX & FMPI2C_STATE_MSK) | (uint32_t)HAL_FMPI2C_MODE_MEM))
        /*!< Memory Busy TX, combinaison of State LSB and Mode enum */
        #define FMPI2C_STATE_MEM_BUSY_RX                                                                                         \
            ((uint32_t)(((uint32_t)HAL_FMPI2C_STATE_BUSY_RX & FMPI2C_STATE_MSK) | (uint32_t)HAL_FMPI2C_MODE_MEM))
        ...
        #define FMPI2C_XFER_LISTEN_IT                                                                                            \
            (uint16_t)(0x8000U) /*!< Bit field can be combinated with @ref FMPI2C_XFER_TX_IT                                     \
...
static void
    FMPI2C_TransferConfig(FMPI2C_HandleTypeDef* hfmpi2c, uint16_t DevAddress, uint8_t Size, uint32_t Mode, uint32_t Request);
...
HAL_StatusTypeDef HAL_FMPI2C_Init(FMPI2C_HandleTypeDef* hfmpi2c)
{
    ...
    if(hfmpi2c == NULL) { return HAL_ERROR; }
    ...
    if(hfmpi2c->State == HAL_FMPI2C_STATE_RESET)
    {
        ...
        if(hfmpi2c->MspInitCallback == NULL) { hfmpi2c->MspInitCallback = HAL_FMPI2C_MspInit; /* Legacy weak MspInit  */ }
    ...
    if(hfmpi2c->Init.AddressingMode == FMPI2C_ADDRESSINGMODE_10BIT) { SET_BIT(hfmpi2c->Instance->CR2, FMPI2C_CR2_ADD10); }
    else
    {
...
HAL_StatusTypeDef HAL_FMPI2C_DeInit(FMPI2C_HandleTypeDef* hfmpi2c)
{
    ...
    __HAL_FMPI2C_DISABLE(hfmpi2c);
        ...
        #if (USE_HAL_FMPI2C_REGISTER_CALLBACKS == 1)
    if(hfmpi2c->MspDeInitCallback == NULL) { hfmpi2c->MspDeInitCallback = HAL_FMPI2C_MspDeInit; /* Legacy weak MspDeInit  */ }
                                              ...
                                              pFMPI2C_CallbackTypeDef pCallback)
{
    ...
    if(HAL_FMPI2C_STATE_READY == hfmpi2c->State)
    {
        switch(CallbackID)
        {
            case HAL_FMPI2C_MASTER_TX_COMPLETE_CB_ID: hfmpi2c->MasterTxCpltCallback = pCallback; break;

            case HAL_FMPI2C_MASTER_RX_COMPLETE_CB_ID: hfmpi2c->MasterRxCpltCallback = pCallback; break;

            case HAL_FMPI2C_SLAVE_TX_COMPLETE_CB_ID: hfmpi2c->SlaveTxCpltCallback = pCallback; break;

            case HAL_FMPI2C_SLAVE_RX_COMPLETE_CB_ID: hfmpi2c->SlaveRxCpltCallback = pCallback; break;

            case HAL_FMPI2C_LISTEN_COMPLETE_CB_ID: hfmpi2c->ListenCpltCallback = pCallback; break;

            case HAL_FMPI2C_MEM_TX_COMPLETE_CB_ID: hfmpi2c->MemTxCpltCallback = pCallback; break;

            case HAL_FMPI2C_MEM_RX_COMPLETE_CB_ID: hfmpi2c->MemRxCpltCallback = pCallback; break;

            case HAL_FMPI2C_ERROR_CB_ID: hfmpi2c->ErrorCallback = pCallback; break;

            case HAL_FMPI2C_ABORT_CB_ID: hfmpi2c->AbortCpltCallback = pCallback; break;

            case HAL_FMPI2C_MSPINIT_CB_ID: hfmpi2c->MspInitCallback = pCallback; break;

            case HAL_FMPI2C_MSPDEINIT_CB_ID: hfmpi2c->MspDeInitCallback = pCallback; break;
...
HAL_StatusTypeDef HAL_FMPI2C_RegisterAddrCallback(FMPI2C_HandleTypeDef* hfmpi2c, pFMPI2C_AddrCallbackTypeDef pCallback)
{
    ...
    if(HAL_FMPI2C_STATE_READY == hfmpi2c->State) { hfmpi2c->AddrCallback = pCallback; }
    else
    {
        /* Update the error code */
        hfmpi2c->ErrorCode |= HAL_FMPI2C_ERROR_INVALID_CALLBACK;
                                             ...
                                             uint32_t Timeout)
{
    ...
    if(hfmpi2c->State == HAL_FMPI2C_STATE_READY)
    {
        ...
        if(hfmpi2c->XferSize > 0U)
        {
            ...
            FMPI2C_TransferConfig(hfmpi2c, DevAddress, (uint8_t)(hfmpi2c->XferSize + 1U), xfermode, FMPI2C_GENERATE_START_WRITE);
        }
        else
        {
            /* Send Slave Address */
        ...
        while(hfmpi2c->XferCount > 0U)
        {
            ...
            if(FMPI2C_WaitOnTXISFlagUntilTimeout(hfmpi2c, Timeout, tickstart) != HAL_OK) { return HAL_ERROR; }
            /* Write data to TXDR */
            hfmpi2c->Instance->TXDR = *hfmpi2c->pBuffPtr;
            ...
            if((hfmpi2c->XferCount != 0U) && (hfmpi2c->XferSize == 0U))
            {
                ...
                {
                    hfmpi2c->XferSize = MAX_NBYTE_SIZE;
                    FMPI2C_TransferConfig(hfmpi2c,
                                          DevAddress,
                                          (uint8_t)hfmpi2c->XferSize,
                                          FMPI2C_RELOAD_MODE,
                                          FMPI2C_NO_STARTSTOP);
                }
                else
        ...
        if(FMPI2C_WaitOnSTOPFlagUntilTimeout(hfmpi2c, Timeout, tickstart) != HAL_OK) { return HAL_ERROR; }
 ...
 */
HAL_StatusTypeDef
    HAL_FMPI2C_Master_Receive(FMPI2C_HandleTypeDef* hfmpi2c, uint16_t DevAddress, uint8_t* pData, uint16_t Size, uint32_t Timeout)
{
    uint32_t tickstart;
    ...
    if(hfmpi2c->State == HAL_FMPI2C_STATE_READY)
    {
        ...
        while(hfmpi2c->XferCount > 0U)
        {
            ...
            if(FMPI2C_WaitOnRXNEFlagUntilTimeout(hfmpi2c, Timeout, tickstart) != HAL_OK) { return HAL_ERROR; }
...
HAL_StatusTypeDef HAL_FMPI2C_Master_Transmit_IT(FMPI2C_HandleTypeDef* hfmpi2c, uint16_t DevAddress, uint8_t* pData, uint16_t Size)
{
    ...
    if(hfmpi2c->State == HAL_FMPI2C_STATE_READY)
    {
        if(__HAL_FMPI2C_GET_FLAG(hfmpi2c, FMPI2C_FLAG_BUSY) == SET) { return HAL_BUSY; }
        ...
        else { FMPI2C_TransferConfig(hfmpi2c, DevAddress, (uint8_t)hfmpi2c->XferSize, xfermode, FMPI2C_GENERATE_START_WRITE); }
    ...
    HAL_FMPI2C_Master_Transmit_DMA(FMPI2C_HandleTypeDef* hfmpi2c, uint16_t DevAddress, uint8_t* pData, uint16_t Size)
{
    ...
    if(hfmpi2c->State == HAL_FMPI2C_STATE_READY)
    {
        ...
        if(hfmpi2c->XferSize > 0U)
        {
            if(hfmpi2c->hdmatx != NULL)
            {
                ...
                dmaxferstatus = HAL_DMA_Start_IT(hfmpi2c->hdmatx,
                                                 (uint32_t)hfmpi2c->pBuffPtr,
                                                 (uint32_t)&hfmpi2c->Instance->TXDR,
                                                 hfmpi2c->XferSize);
            }
            else
            {
        ...
        else
        {
            ...
            FMPI2C_TransferConfig(hfmpi2c, DevAddress, (uint8_t)sizetoxfer, FMPI2C_AUTOEND_MODE, FMPI2C_GENERATE_START_WRITE);
                                           ...
                                           uint16_t Size)
{
    ...
    if(hfmpi2c->State == HAL_FMPI2C_STATE_READY)
    {
        ...
        if(hfmpi2c->XferCount > MAX_NBYTE_SIZE) { hfmpi2c->XferSize = MAX_NBYTE_SIZE; }
        else { hfmpi2c->XferSize = hfmpi2c->XferCount; }
                                                    ...
                                                    uint32_t XferOptions)
{
    ...
    if(hfmpi2c->State == HAL_FMPI2C_STATE_READY)
    {
        ...
        if((hfmpi2c->XferSize > 0U) && ((XferOptions == FMPI2C_FIRST_FRAME) || (XferOptions == FMPI2C_FIRST_AND_LAST_FRAME)))
        {
            /* Preload TX register */
            /* Write data to TXDR */
            hfmpi2c->Instance->TXDR = *hfmpi2c->pBuffPtr;
        ...
        else
        {
            ...
            if(hfmpi2c->XferCount <= MAX_NBYTE_SIZE) { xfermode = hfmpi2c->XferOptions; }
        }
        ...
        else { FMPI2C_TransferConfig(hfmpi2c, DevAddress, (uint8_t)hfmpi2c->XferSize, xfermode, xferrequest); }
...
void HAL_FMPI2C_EV_IRQHandler(FMPI2C_HandleTypeDef* hfmpi2c) /* Derogation MISRAC2012-Rule-8.13 */
{
    ...
    if(hfmpi2c->XferISR != NULL) { hfmpi2c->XferISR(hfmpi2c, itflags, itsources); }
}
 ...
 * @retval HAL mode
 */
HAL_FMPI2C_ModeTypeDef HAL_FMPI2C_GetMode(const FMPI2C_HandleTypeDef* hfmpi2c) { return hfmpi2c->Mode; }
 ...
 */
uint32_t HAL_FMPI2C_GetError(const FMPI2C_HandleTypeDef* hfmpi2c) { return hfmpi2c->ErrorCode; }
            ...
            (FMPI2C_CHECK_IT_SOURCE(ITSources, FMPI2C_IT_TCI) != RESET))
    {
        if((hfmpi2c->XferCount != 0U) && (hfmpi2c->XferSize == 0U))
        {
            ...
            if(hfmpi2c->XferCount > MAX_NBYTE_SIZE)
            {
                ...
                if(FMPI2C_GET_DIR(hfmpi2c) == FMPI2C_DIRECTION_RECEIVE) { hfmpi2c->XferSize = 1U; }
                else { hfmpi2c->XferSize = MAX_NBYTE_SIZE; }
                FMPI2C_TransferConfig(hfmpi2c, devaddress, (uint8_t)hfmpi2c->XferSize, FMPI2C_RELOAD_MODE, FMPI2C_NO_STARTSTOP);
            }
            else
            {
...
static HAL_StatusTypeDef FMPI2C_Mem_ISR_IT(struct __FMPI2C_HandleTypeDef* hfmpi2c, uint32_t ITFlags, uint32_t ITSources)
{
            ...
            (FMPI2C_CHECK_IT_SOURCE(ITSources, FMPI2C_IT_TCI) != RESET))
    {
        ...
        if(hfmpi2c->State == HAL_FMPI2C_STATE_BUSY_RX) { direction = FMPI2C_GENERATE_START_READ; }
...
static HAL_StatusTypeDef FMPI2C_Master_ISR_DMA(struct __FMPI2C_HandleTypeDef* hfmpi2c, uint32_t ITFlags, uint32_t ITSources)
{
    ...
    else if((FMPI2C_CHECK_FLAG(ITFlags, FMPI2C_FLAG_TCR) != RESET) && (FMPI2C_CHECK_IT_SOURCE(ITSources, FMPI2C_IT_TCI) != RESET))
    {
        ...
        if(hfmpi2c->XferCount != 0U)
        {
            ...
            {
                hfmpi2c->XferSize = hfmpi2c->XferCount;
                if(hfmpi2c->XferOptions != FMPI2C_NO_OPTION_FRAME) { xfermode = hfmpi2c->XferOptions; }
                else { xfermode = FMPI2C_AUTOEND_MODE; }
            }
            ...
            /* Enable DMA Request */
            if(hfmpi2c->State == HAL_FMPI2C_STATE_BUSY_RX) { hfmpi2c->Instance->CR1 |= FMPI2C_CR1_RXDMAEN; }
            else { hfmpi2c->Instance->CR1 |= FMPI2C_CR1_TXDMAEN; }
        }
        else
        {
            /* Call TxCpltCallback() if no stop mode is set */
            ...
            (FMPI2C_CHECK_IT_SOURCE(ITSources, FMPI2C_IT_NACKI) != RESET))
    {
           ...
           (FMPI2C_CHECK_IT_SOURCE(ITSources, FMPI2C_CR1_RXDMAEN) != RESET))
        {
            ...
            if(hfmpi2c->hdmarx != NULL)
            {
                ...
                {
                    if(FMPI2C_GET_DMA_REMAIN_DATA(hfmpi2c->hdmarx) == 0U) { treatdmanack = 1U; }
                }
            }
            ...
            if(hfmpi2c->hdmatx != NULL)
            {
                if(FMPI2C_CHECK_IT_SOURCE(ITSources, FMPI2C_CR1_TXDMAEN) != RESET)
                {
                    if(FMPI2C_GET_DMA_REMAIN_DATA(hfmpi2c->hdmatx) == 0U) { treatdmanack = 1U; }
                }
            }
                                                   ...
                                                   uint32_t Tickstart)
{
    ...
    /* Wait until TCR flag is set */
    if(FMPI2C_WaitOnFlagUntilTimeout(hfmpi2c, FMPI2C_FLAG_TCR, RESET, Timeout, Tickstart) != HAL_OK) { return HAL_ERROR; }
                                                  ...
                                                  uint32_t Tickstart)
{
    ...
    if(FMPI2C_WaitOnFlagUntilTimeout(hfmpi2c, FMPI2C_FLAG_TC, RESET, Timeout, Tickstart) != HAL_OK) { return HAL_ERROR; }
...
static void FMPI2C_ITSlaveCplt(FMPI2C_HandleTypeDef* hfmpi2c, uint32_t ITFlags)
{
    ...
    if(FMPI2C_CHECK_IT_SOURCE(tmpcr1value, FMPI2C_CR1_TXDMAEN) != RESET)
    {
        ...
        if(hfmpi2c->hdmatx != NULL) { hfmpi2c->XferCount = (uint16_t)FMPI2C_GET_DMA_REMAIN_DATA(hfmpi2c->hdmatx); }
    }
    else if(FMPI2C_CHECK_IT_SOURCE(tmpcr1value, FMPI2C_CR1_RXDMAEN) != RESET)
    {
        /* Disable DMA Request */
        ...
        if(hfmpi2c->hdmarx != NULL) { hfmpi2c->XferCount = (uint16_t)FMPI2C_GET_DMA_REMAIN_DATA(hfmpi2c->hdmarx); }
    }
    else { /* Do nothing */ }
...
static void FMPI2C_ITError(FMPI2C_HandleTypeDef* hfmpi2c, uint32_t ErrorCode)
{
    ...
    if((hfmpi2c->hdmatx != NULL) &&
       ((tmppreviousstate == FMPI2C_STATE_MASTER_BUSY_TX) || (tmppreviousstate == FMPI2C_STATE_SLAVE_BUSY_TX)))
    {
        if((hfmpi2c->Instance->CR1 & FMPI2C_CR1_TXDMAEN) == FMPI2C_CR1_TXDMAEN) { hfmpi2c->Instance->CR1 &= ~FMPI2C_CR1_TXDMAEN; }
        ...
        else { FMPI2C_TreatErrorCallback(hfmpi2c); }
    }
    /* Abort DMA RX transfer if any */
    else if((hfmpi2c->hdmarx != NULL) &&
            ((tmppreviousstate == FMPI2C_STATE_MASTER_BUSY_RX) || (tmppreviousstate == FMPI2C_STATE_SLAVE_BUSY_RX)))
    {
        if((hfmpi2c->Instance->CR1 & FMPI2C_CR1_RXDMAEN) == FMPI2C_CR1_RXDMAEN) { hfmpi2c->Instance->CR1 &= ~FMPI2C_CR1_RXDMAEN; }
...
static void FMPI2C_Flush_TXDR(FMPI2C_HandleTypeDef* hfmpi2c)
{
    ...
    if(__HAL_FMPI2C_GET_FLAG(hfmpi2c, FMPI2C_FLAG_TXIS) != RESET) { hfmpi2c->Instance->TXDR = 0x00U; }

    /* Flush TX register if not empty */
    if(__HAL_FMPI2C_GET_FLAG(hfmpi2c, FMPI2C_FLAG_TXE) == RESET) { __HAL_FMPI2C_CLEAR_FLAG(hfmpi2c, FMPI2C_FLAG_TXE); }
}
...
static void FMPI2C_DMAMasterTransmitCplt(DMA_HandleTypeDef* hdma)
{
    ...
    else
    {
        ...
        /* Enable the DMA stream */
        if(HAL_DMA_Start_IT(hfmpi2c->hdmatx,
                            (uint32_t)hfmpi2c->pBuffPtr,
                            (uint32_t)&hfmpi2c->Instance->TXDR,
...
static void FMPI2C_DMAMasterReceiveCplt(DMA_HandleTypeDef* hdma)
{
    ...
    else
    {
        ...
        /* Enable the DMA stream */
        if(HAL_DMA_Start_IT(hfmpi2c->hdmarx,
                            (uint32_t)&hfmpi2c->Instance->RXDR,
                            (uint32_t)hfmpi2c->pBuffPtr,
                            hfmpi2c->XferSize) != HAL_OK)
        {
            /* Call the corresponding callback to inform upper layer of End of Transfer */
...
static void FMPI2C_DMASlaveReceiveCplt(DMA_HandleTypeDef* hdma)
{
    ...
    if((FMPI2C_GET_DMA_REMAIN_DATA(hfmpi2c->hdmarx) == 0U) && (tmpoptions != FMPI2C_NO_OPTION_FRAME))
    {
        /* Disable DMA Request */
        hfmpi2c->Instance->CR1 &= ~FMPI2C_CR1_RXDMAEN;
...
static void FMPI2C_DMAError(DMA_HandleTypeDef* hdma)
{
    ...
    if(hfmpi2c->hdmatx != NULL)
    {
        if(FMPI2C_GET_DMA_REMAIN_DATA(hfmpi2c->hdmatx) == 0U) { treatdmaerror = 1U; }
    }

    if(hfmpi2c->hdmarx != NULL)
    {
        if(FMPI2C_GET_DMA_REMAIN_DATA(hfmpi2c->hdmarx) == 0U) { treatdmaerror = 1U; }
    }
...
static void FMPI2C_DMAAbort(DMA_HandleTypeDef* hdma)
{
    ...
    if(hfmpi2c->hdmatx != NULL) { hfmpi2c->hdmatx->XferAbortCallback = NULL; }
    if(hfmpi2c->hdmarx != NULL) { hfmpi2c->hdmarx->XferAbortCallback = NULL; }
                                                       ...
                                                       uint32_t Tickstart)
{
    while(__HAL_FMPI2C_GET_FLAG(hfmpi2c, Flag) == Status)
    {
        ...
        if(FMPI2C_IsErrorOccurred(hfmpi2c, Timeout, Tickstart) != HAL_OK) { return HAL_ERROR; }
...
static HAL_StatusTypeDef FMPI2C_WaitOnRXNEFlagUntilTimeout(FMPI2C_HandleTypeDef* hfmpi2c, uint32_t Timeout, uint32_t Tickstart)
{
    ...
    {
        /* Check if an error is detected */
        if(FMPI2C_IsErrorOccurred(hfmpi2c, Timeout, Tickstart) != HAL_OK) { status = HAL_ERROR; }
        ...
        if((__HAL_FMPI2C_GET_FLAG(hfmpi2c, FMPI2C_FLAG_STOPF) == SET) && (status == HAL_OK))
        {
            ...
            else { hfmpi2c->ErrorCode = HAL_FMPI2C_ERROR_NONE; }
        }
        ...
        while((__HAL_FMPI2C_GET_FLAG(hfmpi2c, FMPI2C_FLAG_STOPF) == RESET) && (status == HAL_OK))
        {
            ...
            if(Timeout != HAL_MAX_DELAY)
            {
                if(((HAL_GetTick() - tickstart) > Timeout) || (Timeout == 0U))
                {
                    ...
                    if((__HAL_FMPI2C_GET_FLAG(hfmpi2c, FMPI2C_FLAG_BUSY) != RESET) && (tmp1 != FMPI2C_CR2_STOP) &&
    ...
    FMPI2C_TransferConfig(FMPI2C_HandleTypeDef* hfmpi2c, uint16_t DevAddress, uint8_t Size, uint32_t Mode, uint32_t Request)
{
    ...
    /* Declaration of tmp to prevent undefined behavior of volatile usage */
    uint32_t tmp =
        ((uint32_t)(((uint32_t)DevAddress & FMPI2C_CR2_SADD) | (((uint32_t)Size << FMPI2C_CR2_NBYTES_Pos) & FMPI2C_CR2_NBYTES) |
                    (uint32_t)Mode | (uint32_t)Request) &
         (~0x80000000U));

    /* update CR2 register */
    MODIFY_REG(
        hfmpi2c->Instance->CR2,
...
static void FMPI2C_ConvertOtherXferOptions(FMPI2C_HandleTypeDef* hfmpi2c)
{
    ...
    if(hfmpi2c->XferOptions == FMPI2C_OTHER_FRAME) { hfmpi2c->XferOptions = FMPI2C_FIRST_FRAME; }
    /* else if user set XferOptions to FMPI2C_OTHER_AND_LAST_FRAME */
    /* it request implicitly to generate a restart condition    */
    /* then generate a stop condition at the end of transfer    */
    /* set XferOptions to FMPI2C_FIRST_AND_LAST_FRAME              */
    else if(hfmpi2c->XferOptions == FMPI2C_OTHER_AND_LAST_FRAME) { hfmpi2c->XferOptions = FMPI2C_FIRST_AND_LAST_FRAME; }
    else { /* Nothing to do */ }
}
