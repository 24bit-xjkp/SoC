...
#include "stm32f4xx.h"
    ...
    typedef struct
    {
        ...
        uint32_t NSS; /*!< Specifies whether the NSS signal is managed by hardware (NSS pin) or by software using the
     ...
     *         @arg @ref LL_SPI_PROTOCOL_TI
     */
    __STATIC_INLINE uint32_t LL_SPI_GetStandard(const SPI_TypeDef* SPIx) { return (uint32_t)(READ_BIT(SPIx->CR2, SPI_CR2_FRF)); }
     ...
     *         @arg @ref LL_SPI_DATAWIDTH_16BIT
     */
    __STATIC_INLINE uint32_t LL_SPI_GetDataWidth(const SPI_TypeDef* SPIx) { return (uint32_t)(READ_BIT(SPIx->CR1, SPI_CR1_DFF)); }
