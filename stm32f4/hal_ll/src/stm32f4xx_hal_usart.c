...
HAL_StatusTypeDef HAL_USART_Init(USART_HandleTypeDef* husart)
{
    ...
    if(husart == NULL) { return HAL_ERROR; }
    ...
    if(husart->State == HAL_USART_STATE_RESET)
    {
        ...
        if(husart->MspInitCallback == NULL) { husart->MspInitCallback = HAL_USART_MspInit; }
...
HAL_StatusTypeDef HAL_USART_DeInit(USART_HandleTypeDef* husart)
{
    ...
    #if (USE_HAL_USART_REGISTER_CALLBACKS == 1)
    if(husart->MspDeInitCallback == NULL) { husart->MspDeInitCallback = HAL_USART_MspDeInit; }
...
HAL_StatusTypeDef HAL_USART_Transmit(USART_HandleTypeDef* husart, const uint8_t* pTxData, uint16_t Size, uint32_t Timeout)
{
    ...
    if(husart->State == HAL_USART_STATE_READY)
    {
        if((pTxData == NULL) || (Size == 0)) { return HAL_ERROR; }
        ...
        while(husart->TxXferCount > 0U)
        {
            ...
            if(USART_WaitOnFlagUntilTimeout(husart, USART_FLAG_TXE, RESET, tickstart, Timeout) != HAL_OK) { return HAL_TIMEOUT; }
        ...
        if(USART_WaitOnFlagUntilTimeout(husart, USART_FLAG_TC, RESET, tickstart, Timeout) != HAL_OK) { return HAL_TIMEOUT; }
...
HAL_StatusTypeDef HAL_USART_Receive(USART_HandleTypeDef* husart, uint8_t* pRxData, uint16_t Size, uint32_t Timeout)
{
    ...
    if(husart->State == HAL_USART_STATE_READY)
    {
        if((pRxData == NULL) || (Size == 0)) { return HAL_ERROR; }
        /* Process Locked */
        __HAL_LOCK(husart);
        ...
        while(husart->RxXferCount > 0U)
        {
            ...
            if(USART_WaitOnFlagUntilTimeout(husart, USART_FLAG_RXNE, RESET, tickstart, Timeout) != HAL_OK) { return HAL_TIMEOUT; }
            ...
            else
            {
                ...
                else { *prxdata8bits = (uint8_t)(husart->Instance->DR & (uint8_t)0x07F); }
                prxdata8bits++;
            }
            husart->RxXferCount--;
        }
                                            ...
                                            uint32_t Timeout)
{
    ...
    if(husart->State == HAL_USART_STATE_READY)
    {
        if((pTxData == NULL) || (pRxData == NULL) || (Size == 0)) { return HAL_ERROR; }
        ...
        if((husart->Init.WordLength == USART_WORDLENGTH_9B) && (husart->Init.Parity == USART_PARITY_NONE))
        {
            if(((((uint32_t)pTxData) & 1U) != 0U) || ((((uint32_t)pRxData) & 1U) != 0U)) { return HAL_ERROR; }
        }
        /* Process Locked */
        __HAL_LOCK(husart);
...
HAL_StatusTypeDef HAL_USART_DMAStop(USART_HandleTypeDef* husart)
{
    ...
    if((husart->State == HAL_USART_STATE_BUSY_TX) && dmarequest)
    {
        ...
        if(husart->hdmatx != NULL) { HAL_DMA_Abort(husart->hdmatx); }
    ...
    if((husart->State == HAL_USART_STATE_BUSY_RX) && dmarequest)
    {
        ...
        if(husart->hdmarx != NULL) { HAL_DMA_Abort(husart->hdmarx); }
...
HAL_StatusTypeDef HAL_USART_Abort_IT(USART_HandleTypeDef* husart)
{
    ...
    if(husart->hdmatx != NULL)
    {
        ...
        else { husart->hdmatx->XferAbortCallback = NULL; }
    }
    ...
    if(husart->hdmarx != NULL)
    {
        ...
        else { husart->hdmarx->XferAbortCallback = NULL; }
    }
    ...
    if(HAL_IS_BIT_SET(husart->Instance->CR3, USART_CR3_DMAT))
    {
        ...
        if(husart->hdmatx != NULL)
        {
            ...
            /* Abort DMA TX */
            if(HAL_DMA_Abort_IT(husart->hdmatx) != HAL_OK) { husart->hdmatx->XferAbortCallback = NULL; }
...
void HAL_USART_IRQHandler(USART_HandleTypeDef* husart)
{
    ...
    if(errorflags == RESET)
    {
        ...
        {
            if(husart->State == HAL_USART_STATE_BUSY_RX) { USART_Receive_IT(husart); }
    ...
    {
        if(husart->State == HAL_USART_STATE_BUSY_TX) { USART_Transmit_IT(husart); }
        else { USART_TransmitReceive_IT(husart); }
        return;
    }
...
static void USART_DMAError(DMA_HandleTypeDef* hdma)
{
    ...
    /* Stop USART DMA Tx request if ongoing */
    dmarequest = HAL_IS_BIT_SET(husart->Instance->CR3, USART_CR3_DMAT);
    if((husart->State == HAL_USART_STATE_BUSY_TX) && dmarequest) { USART_EndTxTransfer(husart); }

    /* Stop USART DMA Rx request if ongoing */
    dmarequest = HAL_IS_BIT_SET(husart->Instance->CR3, USART_CR3_DMAR);
    if((husart->State == HAL_USART_STATE_BUSY_RX) && dmarequest) { USART_EndRxTransfer(husart); }
...
static void USART_DMATxAbortCallback(DMA_HandleTypeDef* hdma)
{
    ...
    if(husart->hdmarx != NULL)
    {
        if(husart->hdmarx->XferAbortCallback != NULL) { return; }
    }
...
static void USART_DMARxAbortCallback(DMA_HandleTypeDef* hdma)
{
    ...
    if(husart->hdmatx != NULL)
    {
        if(husart->hdmatx->XferAbortCallback != NULL) { return; }
    }
...
static HAL_StatusTypeDef USART_Transmit_IT(USART_HandleTypeDef* husart)
{
    ...
    if(husart->State == HAL_USART_STATE_BUSY_TX)
    {
        ...
        else { husart->Instance->DR = (uint8_t)(*husart->pTxBuffPtr++ & (uint8_t)0x00FF); }
...
static HAL_StatusTypeDef USART_Receive_IT(USART_HandleTypeDef* husart)
{
    ...
    if(husart->State == HAL_USART_STATE_BUSY_RX)
    {
        ...
        else
        {
            ...
            else { *pdata8bits = (uint8_t)(husart->Instance->DR & (uint8_t)0x007F); }
    ...
    if(husart->State == HAL_USART_STATE_BUSY_TX_RX)
    {
        if(husart->TxXferCount != 0x00U)
        {
            if(__HAL_USART_GET_FLAG(husart, USART_FLAG_TXE) != RESET)
            {
                ...
                if(husart->TxXferCount == 0U) { CLEAR_BIT(husart->Instance->CR1, USART_CR1_TXEIE); }
            }
        }
        ...
        if(husart->RxXferCount != 0x00U)
        {
            if(__HAL_USART_GET_FLAG(husart, USART_FLAG_RXNE) != RESET)
            {
                ...
                else
                {
                    ...
                    else { *husart->pRxBuffPtr = (uint8_t)(husart->Instance->DR & (uint8_t)0x007F); }
                    husart->pRxBuffPtr += 1U;
                }
...
static void USART_SetConfig(USART_HandleTypeDef* husart)
{
    ...
    tmpreg |= (uint32_t)(USART_CLOCK_ENABLE | husart->Init.CLKPolarity | husart->Init.CLKPhase | husart->Init.CLKLastBit |
