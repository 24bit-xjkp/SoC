 ...
 */
        ...
        #define DSI_ERROR_ACK_MASK                                                                                               \
            (DSI_ISR0_AE0 | DSI_ISR0_AE1 | DSI_ISR0_AE2 | DSI_ISR0_AE3 | DSI_ISR0_AE4 | DSI_ISR0_AE5 | DSI_ISR0_AE6 |            \
             DSI_ISR0_AE7 | DSI_ISR0_AE8 | DSI_ISR0_AE9 | DSI_ISR0_AE10 | DSI_ISR0_AE11 | DSI_ISR0_AE12 | DSI_ISR0_AE13 |        \
...
static HAL_StatusTypeDef
    DSI_ShortWrite(DSI_HandleTypeDef* hdsi, uint32_t ChannelID, uint32_t Mode, uint32_t Param1, uint32_t Param2);
    ...
    DSI_ShortWrite(DSI_HandleTypeDef* hdsi, uint32_t ChannelID, uint32_t Mode, uint32_t Param1, uint32_t Param2)
{
    ...
    while((hdsi->Instance->GPSR & DSI_GPSR_CMDFE) == 0U)
    {
        ...
        if((HAL_GetTick() - tickstart) > DSI_TIMEOUT_VALUE) { return HAL_TIMEOUT; }
    }
...
HAL_StatusTypeDef HAL_DSI_Init(DSI_HandleTypeDef* hdsi, DSI_PLLInitTypeDef* PLLInit)
{
    ...
    hdsi->Instance->WRPCR |= (((PLLInit->PLLNDIV) << DSI_WRPCR_PLL_NDIV_Pos) | ((PLLInit->PLLIDF) << DSI_WRPCR_PLL_IDF_Pos) |
...
HAL_StatusTypeDef HAL_DSI_DeInit(DSI_HandleTypeDef* hdsi)
{
    ...
    __HAL_DSI_REG_DISABLE(hdsi);
        ...
        #if (USE_HAL_DSI_REGISTER_CALLBACKS == 1)
    if(hdsi->MspDeInitCallback == NULL) { hdsi->MspDeInitCallback = HAL_DSI_MspDeInit; }
    ...
    HAL_DSI_RegisterCallback(DSI_HandleTypeDef* hdsi, HAL_DSI_CallbackIDTypeDef CallbackID, pDSI_CallbackTypeDef pCallback)
{
    ...
    if(hdsi->State == HAL_DSI_STATE_READY)
    {
        switch(CallbackID)
        {
            case HAL_DSI_TEARING_EFFECT_CB_ID: hdsi->TearingEffectCallback = pCallback; break;

            case HAL_DSI_ENDOF_REFRESH_CB_ID: hdsi->EndOfRefreshCallback = pCallback; break;

            case HAL_DSI_ERROR_CB_ID: hdsi->ErrorCallback = pCallback; break;

            case HAL_DSI_MSPINIT_CB_ID: hdsi->MspInitCallback = pCallback; break;

            case HAL_DSI_MSPDEINIT_CB_ID: hdsi->MspDeInitCallback = pCallback; break;
...
void HAL_DSI_IRQHandler(DSI_HandleTypeDef* hdsi)
{
    ...
    if(hdsi->ErrorMsk != 0U)
    {
        ...
        if((ErrorStatus0 & DSI_ERROR_ACK_MASK) != 0U) { hdsi->ErrorCode |= HAL_DSI_ERROR_ACK; }

        if((ErrorStatus0 & DSI_ERROR_PHY_MASK) != 0U) { hdsi->ErrorCode |= HAL_DSI_ERROR_PHY; }

        if((ErrorStatus1 & DSI_ERROR_TX_MASK) != 0U) { hdsi->ErrorCode |= HAL_DSI_ERROR_TX; }

        if((ErrorStatus1 & DSI_ERROR_RX_MASK) != 0U) { hdsi->ErrorCode |= HAL_DSI_ERROR_RX; }

        if((ErrorStatus1 & DSI_ERROR_ECC_MASK) != 0U) { hdsi->ErrorCode |= HAL_DSI_ERROR_ECC; }

        if((ErrorStatus1 & DSI_ERROR_CRC_MASK) != 0U) { hdsi->ErrorCode |= HAL_DSI_ERROR_CRC; }

        if((ErrorStatus1 & DSI_ERROR_PSE_MASK) != 0U) { hdsi->ErrorCode |= HAL_DSI_ERROR_PSE; }

        if((ErrorStatus1 & DSI_ERROR_EOT_MASK) != 0U) { hdsi->ErrorCode |= HAL_DSI_ERROR_EOT; }

        if((ErrorStatus1 & DSI_ERROR_OVF_MASK) != 0U) { hdsi->ErrorCode |= HAL_DSI_ERROR_OVF; }

        if((ErrorStatus1 & DSI_ERROR_GEN_MASK) != 0U) { hdsi->ErrorCode |= HAL_DSI_ERROR_GEN; }
...
HAL_StatusTypeDef HAL_DSI_ConfigVideoMode(DSI_HandleTypeDef* hdsi, DSI_VidCfgTypeDef* VidCfg)
{
    ...
    if(VidCfg->ColorCoding == DSI_RGB666) { assert_param(IS_DSI_LOOSELY_PACKED(VidCfg->LooselyPacked)); }
...
HAL_StatusTypeDef HAL_DSI_ConfigAdaptedCommandMode(DSI_HandleTypeDef* hdsi, DSI_CmdCfgTypeDef* CmdCfg)
{
    ...
    hdsi->Instance->WCFGR |=
        (CmdCfg->TearingEffectSource | CmdCfg->TearingEffectPolarity | CmdCfg->AutomaticRefresh | CmdCfg->VSyncPol);
...
HAL_StatusTypeDef HAL_DSI_ConfigCommand(DSI_HandleTypeDef* hdsi, DSI_LPCmdTypeDef* LPCmd)
{
    ...
    hdsi->Instance->CMCR |=
        (LPCmd->LPGenShortWriteNoP | LPCmd->LPGenShortWriteOneP | LPCmd->LPGenShortWriteTwoP | LPCmd->LPGenShortReadNoP |
         LPCmd->LPGenShortReadOneP | LPCmd->LPGenShortReadTwoP | LPCmd->LPGenLongWrite | LPCmd->LPDcsShortWriteNoP |
         LPCmd->LPDcsShortWriteOneP | LPCmd->LPDcsShortReadNoP | LPCmd->LPDcsLongWrite | LPCmd->LPMaxReadPacket);
...
HAL_StatusTypeDef HAL_DSI_ConfigPhyTimer(DSI_HandleTypeDef* hdsi, DSI_PHY_TimerTypeDef* PhyTimers)
{
    ...
    hdsi->Instance->DLTCR |=
        (PhyTimers->DataLaneMaxReadTime | ((PhyTimers->DataLaneLP2HSTime) << 16U) | ((PhyTimers->DataLaneHS2LPTime) << 24U));
                                    ...
                                    const uint8_t* ParametersTable)
{
    ...
    for(count = 0U; count < nbBytes; count++) { fifoword |= (((uint32_t)(*(pparams + count))) << (8U + (8U * count))); }
    ...
    while(uicounter != 0U)
    {
        ...
        fifoword = 0U;
        for(count = 0U; count < nbBytes; count++) { fifoword |= (((uint32_t)(*(pparams + count))) << (8U * count)); }
        hdsi->Instance->GPDR = fifoword;
                               ...
                               uint8_t* ParametersTable)
{
    ...
    if(datasize > 2U)
    {
        ...
        if(DSI_ShortWrite(hdsi, ChannelNbr, DSI_MAX_RETURN_PKT_SIZE, ((datasize) & 0xFFU), (((datasize) >> 8U) & 0xFFU)) !=
           HAL_OK)
        {
            /* Process Unlocked */
            __HAL_UNLOCK(hdsi);
    ...
    if(Mode == DSI_DCS_SHORT_PKT_READ) { DSI_ConfigPacketHeader(hdsi->Instance, ChannelNbr, Mode, DCSCmd, 0U); }
    else if(Mode == DSI_GEN_SHORT_PKT_READ_P0) { DSI_ConfigPacketHeader(hdsi->Instance, ChannelNbr, Mode, 0U, 0U); }
...
HAL_StatusTypeDef HAL_DSI_ExitULPM(DSI_HandleTypeDef* hdsi)
{
    ...
    {
        if((hdsi->Instance->PSR & (DSI_PSR_RUE0 | DSI_PSR_UAN0 | DSI_PSR_PSS0 | DSI_PSR_UAN1 | DSI_PSR_PSS1 | DSI_PSR_UANC |
