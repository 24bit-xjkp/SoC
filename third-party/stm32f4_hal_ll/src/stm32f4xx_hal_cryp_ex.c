...
HAL_StatusTypeDef HAL_CRYPEx_AESGCM_GenerateAuthTAG(CRYP_HandleTypeDef* hcryp, uint32_t* AuthTag, uint32_t Timeout)
{
    ...
    if(hcryp->Init.HeaderWidthUnit == CRYP_HEADERWIDTHUNIT_BYTE) { headerlength /= 4U; }
...
{
    if(hcryp->State == HAL_CRYP_STATE_READY) { hcryp->AutoKeyDerivation = ENABLE; }
    else
    {
        /* Busy error code field */
        hcryp->ErrorCode = HAL_CRYP_ERROR_BUSY;
...
{
    if(hcryp->State == HAL_CRYP_STATE_READY) { hcryp->AutoKeyDerivation = DISABLE; }
    else
    {
        /* Busy error code field */
        hcryp->ErrorCode = HAL_CRYP_ERROR_BUSY;
