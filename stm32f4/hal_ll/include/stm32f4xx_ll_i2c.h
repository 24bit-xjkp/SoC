...
#include "stm32f4xx.h"
    ...
    typedef struct
    {
        ...
        uint32_t ClockSpeed; /*!< Specifies the clock frequency.
                                  ...
                                  This feature can be modified afterwards using unitary function @ref
                                LL_I2C_SetClockPeriod() or @ref LL_I2C_SetDutyCycle() or @ref LL_I2C_SetClockSpeedMode()
        ...
        uint32_t TypeAcknowledge; /*!< Specifies the ACKnowledge or Non ACKnowledge condition after the address receive
                                     match code or next received byte. This parameter can be a value of @ref
     ...
     */
    __STATIC_INLINE uint32_t LL_I2C_GetDigitalFilter(I2C_TypeDef* I2Cx) { return (uint32_t)(READ_BIT(I2Cx->FLTR, I2C_FLTR_DNF)); }
    #endif
    #if defined(I2C_FLTR_ANOFF)
     ...
     * @retval None
     */
    __STATIC_INLINE void LL_I2C_EnableAnalogFilter(I2C_TypeDef* I2Cx) { CLEAR_BIT(I2Cx->FLTR, I2C_FLTR_ANOFF); }
     ...
     * @retval None
     */
    __STATIC_INLINE void LL_I2C_DisableAnalogFilter(I2C_TypeDef* I2Cx) { SET_BIT(I2Cx->FLTR, I2C_FLTR_ANOFF); }
     ...
     *
     * @note   Any of these events will generate interrupt if Buffer interrupts are enabled too(using unitary function
     * @ref LL_I2C_EnableIT_BUF()) : Receive buffer not empty (RXNE) Transmit buffer empty (TXE)
     ...
     *         SET: When the address is fully sent (master mode) or when the received slave address matched with one of
     * the enabled slave address (slave mode).
     ...
     * @note   In that case, the ACK bit controls the (N)ACK of the current byte received or the PEC bit indicates that
     * the current byte in shift register is a PEC.
