...
HAL_StatusTypeDef HAL_SAI_InitProtocol(SAI_HandleTypeDef* hsai, uint32_t protocol, uint32_t datasize, uint32_t nbslot)
{
    ...
    switch(protocol)
    {
        ...
        case SAI_I2S_LSBJUSTIFIED: status = SAI_InitI2S(hsai, protocol, datasize, nbslot); break;
        case SAI_PCM_LONG:
        case SAI_PCM_SHORT: status = SAI_InitPCM(hsai, protocol, datasize, nbslot); break;
        default: status = HAL_ERROR; break;
    }

    if(status == HAL_OK) { status = HAL_SAI_Init(hsai); }
...
HAL_StatusTypeDef HAL_SAI_Init(SAI_HandleTypeDef* hsai)
{
    ...
    if(hsai->State == HAL_SAI_STATE_RESET)
    {
        ...
        if(hsai->MspInitCallback == NULL) { hsai->MspInitCallback = HAL_SAI_MspInit; }
    ...
    if(hsai->Init.AudioFrequency != SAI_AUDIO_FREQUENCY_MCKDIV)
    {
        ...
        if((tmpregisterGCR % 10U) > 8U) { hsai->Init.Mckdiv += 1U; }

        /* For SPDIF protocol, SAI shall provide a bit clock twice faster the symbol-rate */
        if(hsai->Init.Protocol == SAI_SPDIF_PROTOCOL) { hsai->Init.Mckdiv = hsai->Init.Mckdiv >> 1; }
    }
    ...
    else { ckstr_bits = (hsai->Init.ClockStrobing == SAI_CLOCKSTROBING_RISINGEDGE) ? SAI_xCR1_CKSTR : 0U; }
    ...
    hsai->Instance->FRCR |= ((hsai->FrameInit.FrameLength - 1U) | hsai->FrameInit.FSOffset | hsai->FrameInit.FSDefinition |
...
HAL_StatusTypeDef HAL_SAI_DeInit(SAI_HandleTypeDef* hsai)
{
    ...
    SET_BIT(hsai->Instance->CR2, SAI_xCR2_FFLUSH);
        ...
        #if (USE_HAL_SAI_REGISTER_CALLBACKS == 1)
    if(hsai->MspDeInitCallback == NULL) { hsai->MspDeInitCallback = HAL_SAI_MspDeInit; }
    ...
    HAL_SAI_RegisterCallback(SAI_HandleTypeDef* hsai, HAL_SAI_CallbackIDTypeDef CallbackID, pSAI_CallbackTypeDef pCallback)
{
    ...
    else
    {
        if(HAL_SAI_STATE_READY == hsai->State)
        {
            switch(CallbackID)
            {
                case HAL_SAI_RX_COMPLETE_CB_ID: hsai->RxCpltCallback = pCallback; break;
                case HAL_SAI_RX_HALFCOMPLETE_CB_ID: hsai->RxHalfCpltCallback = pCallback; break;
                case HAL_SAI_TX_COMPLETE_CB_ID: hsai->TxCpltCallback = pCallback; break;
                case HAL_SAI_TX_HALFCOMPLETE_CB_ID: hsai->TxHalfCpltCallback = pCallback; break;
                case HAL_SAI_ERROR_CB_ID: hsai->ErrorCallback = pCallback; break;
                case HAL_SAI_MSPINIT_CB_ID: hsai->MspInitCallback = pCallback; break;
                case HAL_SAI_MSPDEINIT_CB_ID: hsai->MspDeInitCallback = pCallback; break;
                default:
                    /* update the error code */
                    hsai->ErrorCode |= HAL_SAI_ERROR_INVALID_CALLBACK;
                    /* update return status */
...
HAL_StatusTypeDef HAL_SAI_UnRegisterCallback(SAI_HandleTypeDef* hsai, HAL_SAI_CallbackIDTypeDef CallbackID)
{
    ...
    if(HAL_SAI_STATE_READY == hsai->State)
    {
        switch(CallbackID)
        {
            case HAL_SAI_RX_COMPLETE_CB_ID: hsai->RxCpltCallback = HAL_SAI_RxCpltCallback; break;
            case HAL_SAI_RX_HALFCOMPLETE_CB_ID: hsai->RxHalfCpltCallback = HAL_SAI_RxHalfCpltCallback; break;
            case HAL_SAI_TX_COMPLETE_CB_ID: hsai->TxCpltCallback = HAL_SAI_TxCpltCallback; break;
            case HAL_SAI_TX_HALFCOMPLETE_CB_ID: hsai->TxHalfCpltCallback = HAL_SAI_TxHalfCpltCallback; break;
            case HAL_SAI_ERROR_CB_ID: hsai->ErrorCallback = HAL_SAI_ErrorCallback; break;
            case HAL_SAI_MSPINIT_CB_ID: hsai->MspInitCallback = HAL_SAI_MspInit; break;
            case HAL_SAI_MSPDEINIT_CB_ID: hsai->MspDeInitCallback = HAL_SAI_MspDeInit; break;
            default:
                /* update the error code */
                hsai->ErrorCode |= HAL_SAI_ERROR_INVALID_CALLBACK;
                /* update return status */
...
HAL_StatusTypeDef HAL_SAI_Transmit_IT(SAI_HandleTypeDef* hsai, uint8_t* pData, uint16_t Size)
{
    ...
    if(hsai->State == HAL_SAI_STATE_READY)
    {
        ...
        else if(hsai->Init.DataSize <= SAI_DATASIZE_16) { hsai->InterruptServiceRoutine = SAI_Transmit_IT16Bit; }
        else { hsai->InterruptServiceRoutine = SAI_Transmit_IT32Bit; }
...
HAL_StatusTypeDef HAL_SAI_Receive_IT(SAI_HandleTypeDef* hsai, uint8_t* pData, uint16_t Size)
{
    ...
    if(hsai->State == HAL_SAI_STATE_READY)
    {
        ...
        else if(hsai->Init.DataSize <= SAI_DATASIZE_16) { hsai->InterruptServiceRoutine = SAI_Receive_IT16Bit; }
        else { hsai->InterruptServiceRoutine = SAI_Receive_IT32Bit; }
    ...
    if(hsai->State != HAL_SAI_STATE_RESET)
    {
        ...
        else if(((itflags & SAI_FLAG_OVRUDR) == SAI_FLAG_OVRUDR) && ((itsources & SAI_IT_OVRUDR) == SAI_IT_OVRUDR))
        {
            ...
            hsai->ErrorCode |= tmperror;
                ...
                /* the transfer is not stopped, we will forward the information to the user and we let the user decide what
                 * needs to be done */
        #if (USE_HAL_SAI_REGISTER_CALLBACKS == 1)
...
static HAL_StatusTypeDef SAI_InitI2S(SAI_HandleTypeDef* hsai, uint32_t protocol, uint32_t datasize, uint32_t nbslot)
{
    ...
    if((nbslot & 0x1U) != 0U) { return HAL_ERROR; }
    ...
    {
        if(datasize == SAI_PROTOCOL_DATASIZE_16BITEXTENDED) { hsai->SlotInit.FirstBitOffset = 16U; }
        if(datasize == SAI_PROTOCOL_DATASIZE_24BIT) { hsai->SlotInit.FirstBitOffset = 8U; }
    }
    return HAL_OK;
}
...
static HAL_StatusTypeDef SAI_InitPCM(SAI_HandleTypeDef* hsai, uint32_t protocol, uint32_t datasize, uint32_t nbslot)
{
    ...
    if(protocol == SAI_PCM_SHORT) { hsai->FrameInit.ActiveFrameLength = 1; }
    else
    {
        /* SAI_PCM_LONG */
...
static uint32_t SAI_InterruptFlag(const SAI_HandleTypeDef* hsai, uint32_t mode)
{
    ...
    if(mode == SAI_MODE_IT) { tmpIT |= SAI_IT_FREQ; }
