...
ErrorStatus LL_LPTIM_Init(LPTIM_TypeDef* LPTIMx, const LL_LPTIM_InitTypeDef* LPTIM_InitStruct)
{
    ...
    */
    if(LL_LPTIM_IsEnabled(LPTIMx) == 1UL) { result = ERROR; }
    else
    {
...
void LL_LPTIM_Disable(LPTIM_TypeDef* LPTIMx)
{
    ...
    {
        case LPTIM1_BASE: tmpclksource = LL_RCC_GetLPTIMClockSource(LL_RCC_LPTIM1_CLKSOURCE); break;
        default: break;
    }
    ...
    if((tmpCMP != 0UL) || (tmpARR != 0UL))
    {
        ...
        switch((uint32_t)LPTIMx)
        {
            case LPTIM1_BASE: LL_RCC_SetLPTIMClockSource(LL_RCC_LPTIM1_CLKSOURCE_PCLK1); break;
            default: break;
        }
