...
#include "stm32f4xx_hal_def.h"
    ...
    typedef struct
    {
        ...
        uint32_t NbSectors; /*!< Number of sectors to be erased.
                                 This parameter must be a value between 1 and (max number of sectors - value of Initial
                               sector)*/
...
#define IS_FLASH_ADDRESS(ADDRESS)                                                                                                \
    ((((ADDRESS) >= FLASH_BASE) && ((ADDRESS) <= FLASH_END)) || (((ADDRESS) >= FLASH_OTP_BASE) && ((ADDRESS) <= FLASH_OTP_END)))
