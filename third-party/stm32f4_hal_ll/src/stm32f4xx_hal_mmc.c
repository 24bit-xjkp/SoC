...
HAL_StatusTypeDef HAL_MMC_Init(MMC_HandleTypeDef* hmmc)
{
    ...
    if(hmmc->State == HAL_MMC_STATE_RESET)
    {
        ...
        if(hmmc->MspInitCallback == NULL) { hmmc->MspInitCallback = HAL_MMC_MspInit; }
    ...
    if(HAL_MMC_InitCard(hmmc) == HAL_ERROR) { return HAL_ERROR; }
    ...
    {
        if(HAL_MMC_ConfigWideBusOperation(hmmc, hmmc->Init.BusWide) != HAL_OK) { return HAL_ERROR; }
    }
...
HAL_StatusTypeDef HAL_MMC_DeInit(MMC_HandleTypeDef* hmmc)
{
    ...
    MMC_PowerOFF(hmmc);
        ...
        #if defined(USE_HAL_MMC_REGISTER_CALLBACKS) && (USE_HAL_MMC_REGISTER_CALLBACKS == 1U)
    if(hmmc->MspDeInitCallback == NULL) { hmmc->MspDeInitCallback = HAL_MMC_MspDeInit; }
    ...
    HAL_MMC_ReadBlocks(MMC_HandleTypeDef* hmmc, uint8_t* pData, uint32_t BlockAdd, uint32_t NumberOfBlocks, uint32_t Timeout)
{
    ...
    if(hmmc->State == HAL_MMC_STATE_READY)
    {
        ...
        if((hmmc->MmcCard.CardType) != MMC_HIGH_CAPACITY_CARD) { add *= 512U; }
...
void HAL_MMC_IRQHandler(MMC_HandleTypeDef* hmmc)
{
    ...
    if((__HAL_MMC_GET_FLAG(hmmc, SDIO_FLAG_RXFIFOHF) != RESET) && ((context & MMC_CONTEXT_IT) != 0U)) { MMC_Read_IT(hmmc); }
    ...
    else if((__HAL_MMC_GET_FLAG(hmmc, SDIO_FLAG_TXFIFOHE) != RESET) && ((context & MMC_CONTEXT_IT) != 0U)) { MMC_Write_IT(hmmc); }

        #if defined(SDIO_STA_STBITERR)
    else if(__HAL_MMC_GET_FLAG(hmmc,
                               SDIO_FLAG_DCRCFAIL | SDIO_FLAG_DTIMEOUT | SDIO_FLAG_RXOVERR | SDIO_FLAG_TXUNDERR |
    ...
    {
        /* Set Error code */
        if(__HAL_MMC_GET_FLAG(hmmc, SDIO_FLAG_DCRCFAIL) != RESET) { hmmc->ErrorCode |= HAL_MMC_ERROR_DATA_CRC_FAIL; }
        if(__HAL_MMC_GET_FLAG(hmmc, SDIO_FLAG_DTIMEOUT) != RESET) { hmmc->ErrorCode |= HAL_MMC_ERROR_DATA_TIMEOUT; }
        if(__HAL_MMC_GET_FLAG(hmmc, SDIO_FLAG_RXOVERR) != RESET) { hmmc->ErrorCode |= HAL_MMC_ERROR_RX_OVERRUN; }
        if(__HAL_MMC_GET_FLAG(hmmc, SDIO_FLAG_TXUNDERR) != RESET) { hmmc->ErrorCode |= HAL_MMC_ERROR_TX_UNDERRUN; }
        #if defined(SDIO_STA_STBITERR)
        if(__HAL_MMC_GET_FLAG(hmmc, SDIO_FLAG_STBITERR) != RESET) { hmmc->ErrorCode |= HAL_MMC_ERROR_DATA_TIMEOUT; }
        #endif /* SDIO_STA_STBITERR */
        ...
        else if((context & MMC_CONTEXT_DMA) != 0U)
        {
            ...
            if(((context & MMC_CONTEXT_WRITE_SINGLE_BLOCK) != 0U) || ((context & MMC_CONTEXT_WRITE_MULTIPLE_BLOCK) != 0U))
            {
                if(hmmc->hdmatx != NULL)
                {
                    ...
                    if(HAL_DMA_Abort_IT(hmmc->hdmatx) != HAL_OK) { MMC_DMATxAbort(hmmc->hdmatx); }
                }
            }
            else if(((context & SD_CONTEXT_READ_SINGLE_BLOCK) != 0U) || ((context & SD_CONTEXT_READ_MULTIPLE_BLOCK) != 0U))
            {
                if(hmmc->hdmarx != NULL)
                {
                    ...
                    /* Abort DMA in IT mode */
                    if(HAL_DMA_Abort_IT(hmmc->hdmarx) != HAL_OK) { MMC_DMARxAbort(hmmc->hdmarx); }
                }
            }
            else
            {
    ...
    HAL_MMC_RegisterCallback(MMC_HandleTypeDef* hmmc, HAL_MMC_CallbackIDTypeDef CallbackId, pMMC_CallbackTypeDef pCallback)
{
    ...
    if(hmmc->State == HAL_MMC_STATE_READY)
    {
        switch(CallbackId)
        {
            case HAL_MMC_TX_CPLT_CB_ID: hmmc->TxCpltCallback = pCallback; break;
            case HAL_MMC_RX_CPLT_CB_ID: hmmc->RxCpltCallback = pCallback; break;
            case HAL_MMC_ERROR_CB_ID: hmmc->ErrorCallback = pCallback; break;
            case HAL_MMC_ABORT_CB_ID: hmmc->AbortCpltCallback = pCallback; break;
            case HAL_MMC_MSP_INIT_CB_ID: hmmc->MspInitCallback = pCallback; break;
            case HAL_MMC_MSP_DEINIT_CB_ID: hmmc->MspDeInitCallback = pCallback; break;
            default:
                /* Update the error code */
                hmmc->ErrorCode |= HAL_MMC_ERROR_INVALID_CALLBACK;
...
HAL_StatusTypeDef HAL_MMC_UnRegisterCallback(MMC_HandleTypeDef* hmmc, HAL_MMC_CallbackIDTypeDef CallbackId)
{
    ...
    if(hmmc->State == HAL_MMC_STATE_READY)
    {
        switch(CallbackId)
        {
            case HAL_MMC_TX_CPLT_CB_ID: hmmc->TxCpltCallback = HAL_MMC_TxCpltCallback; break;
            case HAL_MMC_RX_CPLT_CB_ID: hmmc->RxCpltCallback = HAL_MMC_RxCpltCallback; break;
            case HAL_MMC_ERROR_CB_ID: hmmc->ErrorCallback = HAL_MMC_ErrorCallback; break;
            case HAL_MMC_ABORT_CB_ID: hmmc->AbortCpltCallback = HAL_MMC_AbortCallback; break;
            case HAL_MMC_MSP_INIT_CB_ID: hmmc->MspInitCallback = HAL_MMC_MspInit; break;
            case HAL_MMC_MSP_DEINIT_CB_ID: hmmc->MspDeInitCallback = HAL_MMC_MspDeInit; break;
            default:
                /* Update the error code */
                hmmc->ErrorCode |= HAL_MMC_ERROR_INVALID_CALLBACK;
                /* update return status */
...
HAL_StatusTypeDef HAL_MMC_GetCardCSD(MMC_HandleTypeDef* hmmc, HAL_MMC_CardCSDTypeDef* pCSD)
{
    ...
    if(MMC_ReadExtCSD(hmmc, &block_nbr, 212, 0x0FFFFFFFU) != HAL_OK) /* Field SEC_COUNT [215:212] */ { return HAL_ERROR; }
...
HAL_StatusTypeDef HAL_MMC_ConfigWideBusOperation(MMC_HandleTypeDef* hmmc, uint32_t WideMode)
{
    ...
    if(errorstate == HAL_MMC_ERROR_NONE)
    {
        if(WideMode == SDIO_BUS_WIDE_8B) { errorstate = SDMMC_CmdSwitch(hmmc->Instance, 0x03B70200U); }
        else if(WideMode == SDIO_BUS_WIDE_4B) { errorstate = SDMMC_CmdSwitch(hmmc->Instance, 0x03B70100U); }
        else if(WideMode == SDIO_BUS_WIDE_1B) { errorstate = SDMMC_CmdSwitch(hmmc->Instance, 0x03B70000U); }
        else
        {
        ...
        if(errorstate == HAL_MMC_ERROR_NONE)
        {
            ...
            do {
                ...
                if(errorstate != HAL_MMC_ERROR_NONE) { break; }
            ...
            if((count != 0U) && (errorstate == HAL_MMC_ERROR_NONE))
            {
                ...
                if((response & 0x80U) != 0U) { errorstate = SDMMC_ERROR_GENERAL_UNKNOWN_ERR; }
                else
                {
            ...
            else if(count == 0U) { errorstate = SDMMC_ERROR_TIMEOUT; }
            else { /* Nothing to do */ }
        }
    }
...
HAL_MMC_CardStateTypeDef HAL_MMC_GetCardState(MMC_HandleTypeDef* hmmc)
{
    ...
    if(errorstate != HAL_MMC_ERROR_NONE) { hmmc->ErrorCode |= errorstate; }
...
HAL_StatusTypeDef HAL_MMC_Abort(MMC_HandleTypeDef* hmmc)
{
    ...
    if((hmmc->hdmatx != NULL) || (hmmc->hdmarx != NULL))
    {
        ...
        if(hmmc->hdmatx != NULL)
        {
            if(HAL_DMA_Abort(hmmc->hdmatx) != HAL_OK) { hmmc->ErrorCode |= HAL_MMC_ERROR_DMA; }
        }
        /* Abort the MMC DMA Rx Stream */
        if(hmmc->hdmarx != NULL)
        {
            if(HAL_DMA_Abort(hmmc->hdmarx) != HAL_OK) { hmmc->ErrorCode |= HAL_MMC_ERROR_DMA; }
        }
    }
    ...
    if(hmmc->ErrorCode != HAL_MMC_ERROR_NONE) { return HAL_ERROR; }
    return HAL_OK;
}
...
HAL_StatusTypeDef HAL_MMC_Abort_IT(MMC_HandleTypeDef* hmmc)
{
    ...
    if((hmmc->hdmatx != NULL) || (hmmc->hdmarx != NULL))
    {
        ...
        {
            hmmc->hdmatx->XferAbortCallback = MMC_DMATxAbort;
            if(HAL_DMA_Abort_IT(hmmc->hdmatx) != HAL_OK) { hmmc->hdmatx = NULL; }
        }
        /* Abort the MMC DMA Rx Stream */
        if(hmmc->hdmarx != NULL)
        {
            hmmc->hdmarx->XferAbortCallback = MMC_DMARxAbort;
            if(HAL_DMA_Abort_IT(hmmc->hdmarx) != HAL_OK) { hmmc->hdmarx = NULL; }
        }
    }
...
static void MMC_DMATxAbort(DMA_HandleTypeDef* hdma)
{
    ...
    if(hmmc->hdmatx != NULL) { hmmc->hdmatx = NULL; }
...
static void MMC_DMARxAbort(DMA_HandleTypeDef* hdma)
{
    ...
    if(hmmc->hdmarx != NULL) { hmmc->hdmarx = NULL; }
...
static uint32_t MMC_InitCard(MMC_HandleTypeDef* hmmc)
{
    ...
    if(errorstate != HAL_MMC_ERROR_NONE) { return errorstate; }
    else
    {
    ...
    /* Get CSD parameters */
    if(HAL_MMC_GetCardCSD(hmmc, &CSD) != HAL_OK) { return hmmc->ErrorCode; }
    ...
    if(HAL_MMC_GetCardExtCSD(hmmc, hmmc->Ext_CSD, SDMMC_DATATIMEOUT) != HAL_OK) { return hmmc->ErrorCode; }
...
static uint32_t MMC_PowerON(MMC_HandleTypeDef* hmmc)
{
    ...
    {
        if(count++ == SDMMC_MAX_VOLT_TRIAL) { return HAL_MMC_ERROR_INVALID_VOLTRANGE; }

        /* SEND CMD1 APP_CMD with voltage range as argument */
        errorstate = SDMMC_CmdOpCondition(hmmc->Instance, MMC_VOLTAGE_RANGE);
        if(errorstate != HAL_MMC_ERROR_NONE) { return HAL_MMC_ERROR_UNSUPPORTED_FEATURE; }
    ...
    if(((response & (0xFF000000U)) >> 24U) == 0xC0U) { hmmc->MmcCard.CardType = MMC_HIGH_CAPACITY_CARD; }
    else { hmmc->MmcCard.CardType = MMC_LOW_CAPACITY_CARD; }
    ...
    while(!__HAL_MMC_GET_FLAG(hmmc, SDIO_FLAG_RXOVERR | SDIO_FLAG_DCRCFAIL | SDIO_FLAG_DTIMEOUT | SDIO_FLAG_DATAEND))
    {
        if(__HAL_MMC_GET_FLAG(hmmc, SDIO_FLAG_RXFIFOHF))
        {
            ...
            for(count = 0U; count < 8U; count++)
            {
                ...
                if((i + count) == ((uint32_t)FieldIndex / 4U)) { *pFieldData = tmp_data; }
            }
            i += 8U;
        }
...
static uint32_t MMC_PwrClassUpdate(MMC_HandleTypeDef* hmmc, uint32_t Wide)
{
    ...
    if((Wide == SDIO_BUS_WIDE_8B) || (Wide == SDIO_BUS_WIDE_4B))
    {
        ...
        else { power_class = ((power_class >> 24U) & 0x000000FFU); }
        ...
        if(errorstate == HAL_MMC_ERROR_NONE)
        {
            ...
            if((power_class & 0x0FU) != (supported_pwr_class & 0x0FU))
            {
                ...
                if(errorstate == HAL_MMC_ERROR_NONE)
                {
                    ...
                    {
                        /* Check the bit SWITCH_ERROR of the device status */
                        if((response & 0x80U) != 0U) { errorstate = SDMMC_ERROR_UNSUPPORTED_FEATURE; }
                    }
                    else if(count == 0U) { errorstate = SDMMC_ERROR_TIMEOUT; }
                    else { /* Nothing to do */ }
                }
