...
extern "C"
{
    ...
    */
    ...
    __STATIC_INLINE void __NVIC_SetPriority(IRQn_Type IRQn, uint32_t priority)
    {
        ...
        else { SCB->SHP[(((uint32_t)IRQn) & 0xFUL) - 4UL] = (uint8_t)((priority << (8U - __NVIC_PRIO_BITS)) & (uint32_t)0xFFUL); }
    }
    ...
    __STATIC_INLINE uint32_t __NVIC_GetPriority(IRQn_Type IRQn)
    {
        ...
        if((int32_t)(IRQn) >= 0) { return (((uint32_t)NVIC->IP[((uint32_t)IRQn)] >> (8U - __NVIC_PRIO_BITS))); }
        else { return (((uint32_t)SCB->SHP[(((uint32_t)IRQn) & 0xFUL) - 4UL] >> (8U - __NVIC_PRIO_BITS))); }
    }
