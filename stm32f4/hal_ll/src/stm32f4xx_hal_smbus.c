  ...
  ******************************************************************************
  ...
  [..]
      ...
      (++) When address slave/device SMBUS match, HAL_SMBUS_AddrCallback() is executed and user can
           add his own code to check the Address Match Code and the transmission direction request by master/host
  (Write/Read).
 ...
 */
    ...
    #define SMBUS_STATE_MSK                                                                                                      \
        ((uint32_t)((HAL_SMBUS_STATE_BUSY_TX | HAL_SMBUS_STATE_BUSY_RX) &                                                        \
                    (~(uint32_t)HAL_SMBUS_STATE_READY)))       /*!< Mask State define, keep only RX and TX bits            */
...
HAL_StatusTypeDef HAL_SMBUS_Init(SMBUS_HandleTypeDef* hsmbus)
{
    ...
    if(hsmbus == NULL) { return HAL_ERROR; }
    ...
    if(hsmbus->State == HAL_SMBUS_STATE_RESET)
    {
        ...
        if(hsmbus->MspInitCallback == NULL) { hsmbus->MspInitCallback = HAL_SMBUS_MspInit; /* Legacy weak MspInit  */ }
    ...
    MODIFY_REG(hsmbus->Instance->CCR,
    ...
    MODIFY_REG(hsmbus->Instance->CR1,
               (I2C_CR1_NOSTRETCH | I2C_CR1_ENGC | I2C_CR1_ENPEC | I2C_CR1_ENARP | I2C_CR1_SMBTYPE | I2C_CR1_SMBUS),
    ...
    MODIFY_REG(hsmbus->Instance->OAR2,
...
HAL_StatusTypeDef HAL_SMBUS_DeInit(SMBUS_HandleTypeDef* hsmbus)
{
    ...
    #if (USE_HAL_SMBUS_REGISTER_CALLBACKS == 1)
    if(hsmbus->MspDeInitCallback == NULL) { hsmbus->MspDeInitCallback = HAL_SMBUS_MspDeInit; /* Legacy weak MspDeInit  */ }
                                             ...
                                             pSMBUS_CallbackTypeDef pCallback)
{
    ...
    if(HAL_SMBUS_STATE_READY == hsmbus->State)
    {
        switch(CallbackID)
        {
            case HAL_SMBUS_MASTER_TX_COMPLETE_CB_ID: hsmbus->MasterTxCpltCallback = pCallback; break;

            case HAL_SMBUS_MASTER_RX_COMPLETE_CB_ID: hsmbus->MasterRxCpltCallback = pCallback; break;

            case HAL_SMBUS_SLAVE_TX_COMPLETE_CB_ID: hsmbus->SlaveTxCpltCallback = pCallback; break;

            case HAL_SMBUS_SLAVE_RX_COMPLETE_CB_ID: hsmbus->SlaveRxCpltCallback = pCallback; break;

            case HAL_SMBUS_LISTEN_COMPLETE_CB_ID: hsmbus->ListenCpltCallback = pCallback; break;

            case HAL_SMBUS_ERROR_CB_ID: hsmbus->ErrorCallback = pCallback; break;

            case HAL_SMBUS_ABORT_CB_ID: hsmbus->AbortCpltCallback = pCallback; break;

            case HAL_SMBUS_MSPINIT_CB_ID: hsmbus->MspInitCallback = pCallback; break;

            case HAL_SMBUS_MSPDEINIT_CB_ID: hsmbus->MspDeInitCallback = pCallback; break;
...
HAL_StatusTypeDef HAL_SMBUS_RegisterAddrCallback(SMBUS_HandleTypeDef* hsmbus, pSMBUS_AddrCallbackTypeDef pCallback)
{
    ...
    if(HAL_SMBUS_STATE_READY == hsmbus->State) { hsmbus->AddrCallback = pCallback; }
    else
    {
        /* Update the error code */
        hsmbus->ErrorCode |= HAL_SMBUS_ERROR_INVALID_CALLBACK;
...
HAL_StatusTypeDef HAL_SMBUS_UnRegisterAddrCallback(SMBUS_HandleTypeDef* hsmbus)
{
    ...
    if(HAL_SMBUS_STATE_READY == hsmbus->State) { hsmbus->AddrCallback = HAL_SMBUS_AddrCallback; /* Legacy weak AddrCallback  */ }
    else
    {
        /* Update the error code */
        hsmbus->ErrorCode |= HAL_SMBUS_ERROR_INVALID_CALLBACK;
                                               ...
                                               uint32_t XferOptions)
{
    ...
    if(hsmbus->State == HAL_SMBUS_STATE_READY)
    {
        ...
        if((XferOptions == SMBUS_FIRST_AND_LAST_FRAME_NO_PEC) || (XferOptions == SMBUS_FIRST_AND_LAST_FRAME_WITH_PEC) ||
                                              ...
                                              uint32_t XferOptions)
{
    ...
    if(hsmbus->State == HAL_SMBUS_STATE_READY)
    {
        ...
        if((hsmbus->PreviousState == SMBUS_STATE_MASTER_BUSY_TX) || (hsmbus->PreviousState == SMBUS_STATE_NONE))
        {
            ...
            if((XferOptions == SMBUS_NEXT_FRAME) || (XferOptions == SMBUS_FIRST_AND_LAST_FRAME_NO_PEC) ||
               (XferOptions == SMBUS_FIRST_AND_LAST_FRAME_WITH_PEC) || (XferOptions == SMBUS_FIRST_FRAME) ||
               (XferOptions == SMBUS_NO_OPTION_FRAME))
            {
                /* Enable Acknowledge */
...
HAL_StatusTypeDef HAL_SMBUS_IsDeviceReady(SMBUS_HandleTypeDef* hsmbus, uint16_t DevAddress, uint32_t Trials, uint32_t Timeout)
{
    ...
    if(hsmbus->State == HAL_SMBUS_STATE_READY)
    {
        ...
        do {
            ...
            if(SMBUS_WaitOnFlagUntilTimeout(hsmbus, SMBUS_FLAG_SB, RESET, Timeout, tickstart) != HAL_OK) { return HAL_TIMEOUT; }
            ...
            {
                if((Timeout == 0U) || ((HAL_GetTick() - tickstart) > Timeout)) { hsmbus->State = HAL_SMBUS_STATE_TIMEOUT; }
                tmp1 = __HAL_SMBUS_GET_FLAG(hsmbus, SMBUS_FLAG_ADDR);
                tmp2 = __HAL_SMBUS_GET_FLAG(hsmbus, SMBUS_FLAG_AF);
                tmp3 = hsmbus->State;
            }
...
void HAL_SMBUS_EV_IRQHandler(SMBUS_HandleTypeDef* hsmbus)
{
    ...
    {
        /* SB Set ----------------------------------------------------------------*/
        if(((sr1itflags & SMBUS_FLAG_SB) != RESET) && ((itsources & SMBUS_IT_EVT) != RESET)) { SMBUS_Master_SB(hsmbus); }
        /* ADD10 Set -------------------------------------------------------------*/
        ...
        /* ADDR Set --------------------------------------------------------------*/
        else if(((sr1itflags & SMBUS_FLAG_ADDR) != RESET) && ((itsources & SMBUS_IT_EVT) != RESET)) { SMBUS_Master_ADDR(hsmbus); }
    ...
    {
        /* ADDR set --------------------------------------------------------------*/
        if(((sr1itflags & SMBUS_FLAG_ADDR) != RESET) && ((itsources & SMBUS_IT_EVT) != RESET)) { SMBUS_Slave_ADDR(hsmbus); }
        /* STOPF set --------------------------------------------------------------*/
...
void HAL_SMBUS_ER_IRQHandler(SMBUS_HandleTypeDef* hsmbus)
{
    ...
    if(hsmbus->ErrorCode != HAL_SMBUS_ERROR_NONE) { SMBUS_ITError(hsmbus); }
}
...
static HAL_StatusTypeDef SMBUS_MasterTransmit_TXE(SMBUS_HandleTypeDef* hsmbus)
{
    ...
    if((hsmbus->XferSize == 0U) && (CurrentState == HAL_SMBUS_STATE_BUSY_TX))
    {
        ...
        if(((CurrentXferOptions == SMBUS_FIRST_FRAME) || (CurrentXferOptions == SMBUS_NEXT_FRAME)) &&
    ...
    else if(CurrentState == HAL_SMBUS_STATE_BUSY_TX)
    {
        ...
        if((hsmbus->XferCount == 2U) && (SMBUS_GET_PEC_MODE(hsmbus) == SMBUS_PEC_ENABLE) &&
           ((hsmbus->XferOptions == SMBUS_FIRST_AND_LAST_FRAME_WITH_PEC) || (hsmbus->XferOptions == SMBUS_LAST_FRAME_WITH_PEC)))
        {
            hsmbus->XferCount--;
        }
...
static HAL_StatusTypeDef SMBUS_Master_SB(SMBUS_HandleTypeDef* hsmbus)
{
    if(hsmbus->Init.AddressingMode == SMBUS_ADDRESSINGMODE_7BIT)
    {
        ...
        if(hsmbus->State == HAL_SMBUS_STATE_BUSY_TX) { hsmbus->Instance->DR = SMBUS_7BIT_ADD_WRITE(hsmbus->Devaddress); }
        else { hsmbus->Instance->DR = SMBUS_7BIT_ADD_READ(hsmbus->Devaddress); }
    }
    else
    {
...
static HAL_StatusTypeDef SMBUS_Slave_ADDR(SMBUS_HandleTypeDef* hsmbus)
{
    ...
    if(__HAL_SMBUS_GET_FLAG(hsmbus, SMBUS_FLAG_TRA) == RESET) { TransferDirection = SMBUS_DIRECTION_TRANSMIT; }

    if(__HAL_SMBUS_GET_FLAG(hsmbus, SMBUS_FLAG_DUALF) == RESET) { SlaveAddrCode = hsmbus->Init.OwnAddress1; }
    else { SlaveAddrCode = hsmbus->Init.OwnAddress2; }
...
static HAL_StatusTypeDef SMBUS_Slave_AF(SMBUS_HandleTypeDef* hsmbus)
{
    ...
    if(((CurrentXferOptions == SMBUS_FIRST_AND_LAST_FRAME_NO_PEC) ||
        (CurrentXferOptions == SMBUS_FIRST_AND_LAST_FRAME_WITH_PEC) || (CurrentXferOptions == SMBUS_LAST_FRAME_NO_PEC) ||
...
static void SMBUS_ITError(SMBUS_HandleTypeDef* hsmbus)
{
    ...
    else
    {
        ...
        if(hsmbus->State != HAL_SMBUS_STATE_ABORT) { hsmbus->State = HAL_SMBUS_STATE_READY; }
        hsmbus->PreviousState = SMBUS_STATE_NONE;
        hsmbus->Mode = HAL_SMBUS_MODE_NONE;
    }
