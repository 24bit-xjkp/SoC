...
static HAL_StatusTypeDef
    I2S_WaitFlagStateUntilTimeout(I2S_HandleTypeDef* hi2s, uint32_t Flag, FlagStatus State, uint32_t Timeout);
...
HAL_StatusTypeDef HAL_I2S_Init(I2S_HandleTypeDef* hi2s)
{
    ...
    if(hi2s->State == HAL_I2S_STATE_RESET)
    {
        ...
        if(hi2s->MspInitCallback == NULL) { hi2s->MspInitCallback = HAL_I2S_MspInit; /* Legacy weak MspInit  */ }
    ...
    if(hi2s->Init.AudioFreq != I2S_AUDIOFREQ_DEFAULT)
    {
        ...
        if(IS_I2S_APB1_INSTANCE(hi2s->Instance)) { i2sclk = HAL_RCCEx_GetPeriphCLKFreq(RCC_PERIPHCLK_I2S_APB1); }
        else { i2sclk = HAL_RCCEx_GetPeriphCLKFreq(RCC_PERIPHCLK_I2S_APB2); }
    #else
        ...
        if(hi2s->Init.MCLKOutput == I2S_MCLKOUTPUT_ENABLE)
        {
            ...
            else { tmp = (uint32_t)(((((i2sclk / (packetlength * 8U)) * 10U) / hi2s->Init.AudioFreq)) + 5U); }
        }
        else
        {
    ...
    if(hi2s->Init.FullDuplexMode == I2S_FULLDUPLEXMODE_ENABLE)
    {
        ...
        if((hi2s->Init.Mode == I2S_MODE_MASTER_TX) || (hi2s->Init.Mode == I2S_MODE_SLAVE_TX)) { tmp = I2S_MODE_SLAVE_RX; }
        else /* I2S_MODE_MASTER_RX ||  I2S_MODE_SLAVE_RX */ { tmp = I2S_MODE_SLAVE_TX; }

        /* Configure the I2S Slave with the I2S Master parameter values */
        tmpreg |= (uint16_t)((uint16_t)SPI_I2SCFGR_I2SMOD | (uint16_t)tmp | (uint16_t)hi2s->Init.Standard |
                             (uint16_t)hi2s->Init.DataFormat | (uint16_t)hi2s->Init.CPOL);
...
HAL_StatusTypeDef HAL_I2S_DeInit(I2S_HandleTypeDef* hi2s)
{
    ...
    #if (USE_HAL_I2S_REGISTER_CALLBACKS == 1U)
    if(hi2s->MspDeInitCallback == NULL) { hi2s->MspDeInitCallback = HAL_I2S_MspDeInit; /* Legacy weak MspDeInit  */ }
    ...
    HAL_I2S_RegisterCallback(I2S_HandleTypeDef* hi2s, HAL_I2S_CallbackIDTypeDef CallbackID, pI2S_CallbackTypeDef pCallback)
{
    ...
    if(HAL_I2S_STATE_READY == hi2s->State)
    {
        switch(CallbackID)
        {
            case HAL_I2S_TX_COMPLETE_CB_ID: hi2s->TxCpltCallback = pCallback; break;

            case HAL_I2S_RX_COMPLETE_CB_ID: hi2s->RxCpltCallback = pCallback; break;

        #if defined(SPI_I2S_FULLDUPLEX_SUPPORT)
            case HAL_I2S_TX_RX_COMPLETE_CB_ID: hi2s->TxRxCpltCallback = pCallback; break;
        #endif /* SPI_I2S_FULLDUPLEX_SUPPORT */

            case HAL_I2S_TX_HALF_COMPLETE_CB_ID: hi2s->TxHalfCpltCallback = pCallback; break;

            case HAL_I2S_RX_HALF_COMPLETE_CB_ID: hi2s->RxHalfCpltCallback = pCallback; break;

        #if defined(SPI_I2S_FULLDUPLEX_SUPPORT)
            case HAL_I2S_TX_RX_HALF_COMPLETE_CB_ID: hi2s->TxRxHalfCpltCallback = pCallback; break;
        #endif /* SPI_I2S_FULLDUPLEX_SUPPORT */

            case HAL_I2S_ERROR_CB_ID: hi2s->ErrorCallback = pCallback; break;

            case HAL_I2S_MSPINIT_CB_ID: hi2s->MspInitCallback = pCallback; break;

            case HAL_I2S_MSPDEINIT_CB_ID: hi2s->MspDeInitCallback = pCallback; break;
