...
HAL_StatusTypeDef HAL_RTCEx_SetTamper(RTC_HandleTypeDef* hrtc, RTC_TamperTypeDef* sTamper)
{
    ...
    tmpreg &= ~(RTC_TAMPERFILTER_MASK | RTC_TAMPERSAMPLINGFREQ_RTCCLK_MASK | RTC_TAMPERPRECHARGEDURATION_MASK |
                RTC_TAMPER_PULLUP_MASK | RTC_TAFCR_TAMP1INSEL | RTC_TIMESTAMPONTAMPERDETECTION_MASK);

    /* Set remaining parameters of desired configuration into temporary variable */
    tmpreg |= ((uint32_t)sTamper->Filter | (uint32_t)sTamper->SamplingFrequency | (uint32_t)sTamper->PrechargeDuration |
               (uint32_t)sTamper->TamperPullUp | (uint32_t)sTamper->PinSelection | (uint32_t)sTamper->TimeStampOnTamperDetection);
                                           ...
                                           uint32_t SmoothCalibMinusPulsesValue)
{
    ...
    hrtc->Instance->CALR =
        (uint32_t)((uint32_t)SmoothCalibPeriod | (uint32_t)SmoothCalibPlusPulses | (uint32_t)SmoothCalibMinusPulsesValue);
