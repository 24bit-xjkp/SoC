...
HAL_StatusTypeDef HAL_SDRAM_Init(SDRAM_HandleTypeDef* hsdram, FMC_SDRAM_TimingTypeDef* Timing)
{
    ...
    if(hsdram == NULL) { return HAL_ERROR; }
    ...
    if(hsdram->State == HAL_SDRAM_STATE_RESET)
    {
        ...
        if(hsdram->MspInitCallback == NULL) { hsdram->MspInitCallback = HAL_SDRAM_MspInit; }
...
{
        #if (USE_HAL_SDRAM_REGISTER_CALLBACKS == 1)
    if(hsdram->MspDeInitCallback == NULL) { hsdram->MspDeInitCallback = HAL_SDRAM_MspDeInit; }
...
HAL_StatusTypeDef HAL_SDRAM_Read_8b(SDRAM_HandleTypeDef* hsdram, uint32_t* pAddress, uint8_t* pDstBuffer, uint32_t BufferSize)
{
    ...
    if(state == HAL_SDRAM_STATE_BUSY) { return HAL_BUSY; }
...
HAL_StatusTypeDef HAL_SDRAM_Write_8b(SDRAM_HandleTypeDef* hsdram, uint32_t* pAddress, uint8_t* pSrcBuffer, uint32_t BufferSize)
{
    ...
    /* Check the SDRAM controller state */
    if(hsdram->State == HAL_SDRAM_STATE_BUSY) { return HAL_BUSY; }
...
HAL_StatusTypeDef HAL_SDRAM_Read_16b(SDRAM_HandleTypeDef* hsdram, uint32_t* pAddress, uint16_t* pDstBuffer, uint32_t BufferSize)
{
    ...
    else if((state == HAL_SDRAM_STATE_READY) || (state == HAL_SDRAM_STATE_WRITE_PROTECTED))
    {
        ...
        if((BufferSize % 2U) != 0U) { *pdestbuff = (uint16_t)((*pSdramAddress) & 0x0000FFFFU); }
...
HAL_StatusTypeDef HAL_SDRAM_Write_16b(SDRAM_HandleTypeDef* hsdram, uint32_t* pAddress, uint16_t* pSrcBuffer, uint32_t BufferSize)
{
    ...
    else if(hsdram->State == HAL_SDRAM_STATE_READY)
    {
        ...
        if((BufferSize % 2U) != 0U) { *psdramaddress = ((uint32_t)(*psrcbuff) & 0x0000FFFFU) | ((*psdramaddress) & 0xFFFF0000U); }
...
HAL_StatusTypeDef HAL_SDRAM_Read_DMA(SDRAM_HandleTypeDef* hsdram, uint32_t* pAddress, uint32_t* pDstBuffer, uint32_t BufferSize)
{
    ...
    /* Check the SDRAM controller state */
    if(state == HAL_SDRAM_STATE_BUSY) { status = HAL_BUSY; }
    else if((state == HAL_SDRAM_STATE_READY) || (state == HAL_SDRAM_STATE_WRITE_PROTECTED))
    {
        /* Process Locked */
        __HAL_LOCK(hsdram);
        ...
        if(state == HAL_SDRAM_STATE_READY) { hsdram->hdma->XferCpltCallback = SDRAM_DMACplt; }
        else { hsdram->hdma->XferCpltCallback = SDRAM_DMACpltProt; }
...
HAL_StatusTypeDef HAL_SDRAM_Write_DMA(SDRAM_HandleTypeDef* hsdram, uint32_t* pAddress, uint32_t* pSrcBuffer, uint32_t BufferSize)
{
    ...
    /* Check the SDRAM controller state */
    if(hsdram->State == HAL_SDRAM_STATE_BUSY) { status = HAL_BUSY; }
    else if(hsdram->State == HAL_SDRAM_STATE_READY)
    {
        /* Process Locked */
        __HAL_LOCK(hsdram);
...
HAL_StatusTypeDef HAL_SDRAM_SendCommand(SDRAM_HandleTypeDef* hsdram, FMC_SDRAM_CommandTypeDef* Command, uint32_t Timeout)
{
    ...
    else if((state == HAL_SDRAM_STATE_READY) || (state == HAL_SDRAM_STATE_PRECHARGED))
    {
        ...
        if(Command->CommandMode == FMC_SDRAM_CMD_PALL) { hsdram->State = HAL_SDRAM_STATE_PRECHARGED; }
        else { hsdram->State = HAL_SDRAM_STATE_READY; }
    }
    else { return HAL_ERROR; }
