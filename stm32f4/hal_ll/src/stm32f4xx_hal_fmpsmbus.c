...
static HAL_StatusTypeDef
    FMPSMBUS_WaitOnFlagUntilTimeout(FMPSMBUS_HandleTypeDef* hfmpsmbus, uint32_t Flag, FlagStatus Status, uint32_t Timeout);
...
HAL_StatusTypeDef HAL_FMPSMBUS_Init(FMPSMBUS_HandleTypeDef* hfmpsmbus)
{
    ...
    if(hfmpsmbus == NULL) { return HAL_ERROR; }
    ...
    if(hfmpsmbus->State == HAL_FMPSMBUS_STATE_RESET)
    {
        ...
        if(hfmpsmbus->MspInitCallback == NULL) { hfmpsmbus->MspInitCallback = HAL_FMPSMBUS_MspInit; /* Legacy weak MspInit  */ }
    ...
    if(hfmpsmbus->Init.AddressingMode == FMPSMBUS_ADDRESSINGMODE_10BIT) { hfmpsmbus->Instance->CR2 = (FMPI2C_CR2_ADD10); }
    ...
    hfmpsmbus->Instance->OAR2 =
        (hfmpsmbus->Init.DualAddressMode | hfmpsmbus->Init.OwnAddress2 | (hfmpsmbus->Init.OwnAddress2Masks << 8U));

    /*---------------------------- FMPSMBUSx CR1 Configuration ------------------------*/
    /* Configure FMPSMBUSx: Generalcall and NoStretch mode */
    hfmpsmbus->Instance->CR1 =
        (hfmpsmbus->Init.GeneralCallMode | hfmpsmbus->Init.NoStretchMode | hfmpsmbus->Init.PacketErrorCheckMode |
         hfmpsmbus->Init.PeripheralMode | hfmpsmbus->Init.AnalogFilter);
                                                ...
                                                pFMPSMBUS_CallbackTypeDef pCallback)
{
    ...
    if(HAL_FMPSMBUS_STATE_READY == hfmpsmbus->State)
    {
        switch(CallbackID)
        {
            case HAL_FMPSMBUS_MASTER_TX_COMPLETE_CB_ID: hfmpsmbus->MasterTxCpltCallback = pCallback; break;

            case HAL_FMPSMBUS_MASTER_RX_COMPLETE_CB_ID: hfmpsmbus->MasterRxCpltCallback = pCallback; break;

            case HAL_FMPSMBUS_SLAVE_TX_COMPLETE_CB_ID: hfmpsmbus->SlaveTxCpltCallback = pCallback; break;

            case HAL_FMPSMBUS_SLAVE_RX_COMPLETE_CB_ID: hfmpsmbus->SlaveRxCpltCallback = pCallback; break;

            case HAL_FMPSMBUS_LISTEN_COMPLETE_CB_ID: hfmpsmbus->ListenCpltCallback = pCallback; break;

            case HAL_FMPSMBUS_ERROR_CB_ID: hfmpsmbus->ErrorCallback = pCallback; break;

            case HAL_FMPSMBUS_MSPINIT_CB_ID: hfmpsmbus->MspInitCallback = pCallback; break;

            case HAL_FMPSMBUS_MSPDEINIT_CB_ID: hfmpsmbus->MspDeInitCallback = pCallback; break;
...
HAL_StatusTypeDef HAL_FMPSMBUS_RegisterAddrCallback(FMPSMBUS_HandleTypeDef* hfmpsmbus, pFMPSMBUS_AddrCallbackTypeDef pCallback)
{
    ...
    if(HAL_FMPSMBUS_STATE_READY == hfmpsmbus->State) { hfmpsmbus->AddrCallback = pCallback; }
    else
    {
        /* Update the error code */
        hfmpsmbus->ErrorCode |= HAL_FMPSMBUS_ERROR_INVALID_CALLBACK;
                                                  ...
                                                  uint32_t XferOptions)
{
    ...
    if(hfmpsmbus->State == HAL_FMPSMBUS_STATE_READY)
    {
        ...
        if(hfmpsmbus->pBuffPtr == NULL) { hfmpsmbus->XferOptions &= ~FMPSMBUS_AUTOEND_MODE; }

        if(Size > MAX_NBYTE_SIZE) { hfmpsmbus->XferSize = MAX_NBYTE_SIZE; }
        else { hfmpsmbus->XferSize = Size; }

        sizetoxfer = hfmpsmbus->XferSize;
        if((sizetoxfer > 0U) &&
           ((XferOptions == FMPSMBUS_FIRST_FRAME) || (XferOptions == FMPSMBUS_FIRST_AND_LAST_FRAME_NO_PEC) ||
            (XferOptions == FMPSMBUS_FIRST_FRAME_WITH_PEC) || (XferOptions == FMPSMBUS_FIRST_AND_LAST_FRAME_WITH_PEC)))
        {
        ...
        {
            FMPSMBUS_TransferConfig(hfmpsmbus,
                                    DevAddress,
                                    (uint8_t)sizetoxfer,
        ...
        else
        {
            ...
            /* If PEC mode is enable, size to transmit manage by SW part should be Size-1 byte, corresponding to PEC
             * byte */
...
HAL_StatusTypeDef HAL_FMPSMBUS_Master_Abort_IT(FMPSMBUS_HandleTypeDef* hfmpsmbus, uint16_t DevAddress)
{
    if(hfmpsmbus->State == HAL_FMPSMBUS_STATE_READY)
    {
        ...
        /* Note : The FMPSMBUS interrupts must be enabled after unlocking current process
                  to avoid the risk of FMPSMBUS interrupt handle execution before current
                  process unlock */
        if(hfmpsmbus->State == HAL_FMPSMBUS_STATE_MASTER_BUSY_TX) { FMPSMBUS_Enable_IRQ(hfmpsmbus, FMPSMBUS_IT_TX); }
        else if(hfmpsmbus->State == HAL_FMPSMBUS_STATE_MASTER_BUSY_RX) { FMPSMBUS_Enable_IRQ(hfmpsmbus, FMPSMBUS_IT_RX); }
        else { /* Nothing to do */ }
    ...
    HAL_FMPSMBUS_Slave_Receive_IT(FMPSMBUS_HandleTypeDef* hfmpsmbus, uint8_t* pData, uint16_t Size, uint32_t XferOptions)
{
    ...
    if((hfmpsmbus->State & HAL_FMPSMBUS_STATE_LISTEN) == HAL_FMPSMBUS_STATE_LISTEN)
    {
        ...
        else { FMPSMBUS_TransferConfig(hfmpsmbus, 0, 1, hfmpsmbus->XferOptions | FMPSMBUS_RELOAD_MODE, FMPSMBUS_NO_STARTSTOP); }
    ...
    HAL_FMPSMBUS_IsDeviceReady(FMPSMBUS_HandleTypeDef* hfmpsmbus, uint16_t DevAddress, uint32_t Trials, uint32_t Timeout)
{
    ...
    if(hfmpsmbus->State == HAL_FMPSMBUS_STATE_READY)
    {
        if(__HAL_FMPSMBUS_GET_FLAG(hfmpsmbus, FMPSMBUS_FLAG_BUSY) != RESET) { return HAL_BUSY; }
...
void HAL_FMPSMBUS_EV_IRQHandler(FMPSMBUS_HandleTypeDef* hfmpsmbus)
{
    ...
    if((FMPSMBUS_CHECK_IT_SOURCE(tmpcr1value, (FMPSMBUS_IT_TCI | FMPSMBUS_IT_STOPI | FMPSMBUS_IT_NACKI | FMPSMBUS_IT_TXI)) !=
        RESET) &&
    ...
    if((FMPSMBUS_CHECK_IT_SOURCE(tmpcr1value, (FMPSMBUS_IT_TCI | FMPSMBUS_IT_STOPI | FMPSMBUS_IT_NACKI | FMPSMBUS_IT_RXI)) !=
        RESET) &&
 ...
 */
uint32_t HAL_FMPSMBUS_GetError(const FMPSMBUS_HandleTypeDef* hfmpsmbus) { return hfmpsmbus->ErrorCode; }
...
static void FMPSMBUS_Flush_TXDR(FMPSMBUS_HandleTypeDef* hfmpsmbus)
{
    ...
    if(__HAL_FMPSMBUS_GET_FLAG(hfmpsmbus, FMPSMBUS_FLAG_TXIS) != RESET) { hfmpsmbus->Instance->TXDR = 0x00U; }
 ...
 */
static void
    FMPSMBUS_TransferConfig(FMPSMBUS_HandleTypeDef* hfmpsmbus, uint16_t DevAddress, uint8_t Size, uint32_t Mode, uint32_t Request)
{
    /* Check the parameters */
    assert_param(IS_FMPSMBUS_ALL_INSTANCE(hfmpsmbus->Instance));
    ...
    MODIFY_REG(hfmpsmbus->Instance->CR2,
               ((FMPI2C_CR2_SADD | FMPI2C_CR2_NBYTES | FMPI2C_CR2_RELOAD | FMPI2C_CR2_AUTOEND |
                 (FMPI2C_CR2_RD_WRN & (uint32_t)(Request >> (31UL - FMPI2C_CR2_RD_WRN_Pos))) | FMPI2C_CR2_START |
                 FMPI2C_CR2_STOP | FMPI2C_CR2_PECBYTE)),
...
static void FMPSMBUS_ConvertOtherXferOptions(FMPSMBUS_HandleTypeDef* hfmpsmbus)
{
    ...
    if(hfmpsmbus->XferOptions == FMPSMBUS_OTHER_FRAME_NO_PEC) { hfmpsmbus->XferOptions = FMPSMBUS_FIRST_FRAME; }
