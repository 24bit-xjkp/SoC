...
HAL_StatusTypeDef HAL_SMARTCARD_Init(SMARTCARD_HandleTypeDef* hsc)
{
    ...
    if(hsc->gState == HAL_SMARTCARD_STATE_RESET)
    {
        ...
        if(hsc->MspInitCallback == NULL) { hsc->MspInitCallback = HAL_SMARTCARD_MspInit; }
...
HAL_StatusTypeDef HAL_SMARTCARD_DeInit(SMARTCARD_HandleTypeDef* hsc)
{
    ...
    #if USE_HAL_SMARTCARD_REGISTER_CALLBACKS == 1
    if(hsc->MspDeInitCallback == NULL) { hsc->MspDeInitCallback = HAL_SMARTCARD_MspDeInit; }
    /* DeInit the low level hardware */
                                                 ...
                                                 pSMARTCARD_CallbackTypeDef pCallback)
{
    ...
    if(hsc->gState == HAL_SMARTCARD_STATE_READY)
    {
        switch(CallbackID)
        {
            ...
            case HAL_SMARTCARD_TX_COMPLETE_CB_ID: hsc->TxCpltCallback = pCallback; break;

            case HAL_SMARTCARD_RX_COMPLETE_CB_ID: hsc->RxCpltCallback = pCallback; break;

            case HAL_SMARTCARD_ERROR_CB_ID: hsc->ErrorCallback = pCallback; break;

            case HAL_SMARTCARD_ABORT_COMPLETE_CB_ID: hsc->AbortCpltCallback = pCallback; break;

            case HAL_SMARTCARD_ABORT_TRANSMIT_COMPLETE_CB_ID: hsc->AbortTransmitCpltCallback = pCallback; break;

            case HAL_SMARTCARD_ABORT_RECEIVE_COMPLETE_CB_ID: hsc->AbortReceiveCpltCallback = pCallback; break;

            case HAL_SMARTCARD_MSPINIT_CB_ID: hsc->MspInitCallback = pCallback; break;

            case HAL_SMARTCARD_MSPDEINIT_CB_ID: hsc->MspDeInitCallback = pCallback; break;
...
HAL_StatusTypeDef HAL_SMARTCARD_UnRegisterCallback(SMARTCARD_HandleTypeDef* hsc, HAL_SMARTCARD_CallbackIDTypeDef CallbackID)
{
    ...
    else if(HAL_SMARTCARD_STATE_RESET == hsc->gState)
    {
        switch(CallbackID)
        {
            case HAL_SMARTCARD_MSPINIT_CB_ID: hsc->MspInitCallback = HAL_SMARTCARD_MspInit; break;

            case HAL_SMARTCARD_MSPDEINIT_CB_ID: hsc->MspDeInitCallback = HAL_SMARTCARD_MspDeInit; break;
...
HAL_StatusTypeDef HAL_SMARTCARD_Transmit(SMARTCARD_HandleTypeDef* hsc, const uint8_t* pData, uint16_t Size, uint32_t Timeout)
{
    ...
    if(hsc->gState == HAL_SMARTCARD_STATE_READY)
    {
        ...
        if(SMARTCARD_WaitOnFlagUntilTimeout(hsc, SMARTCARD_FLAG_TC, RESET, tickstart, Timeout) != HAL_OK) { return HAL_TIMEOUT; }
...
HAL_StatusTypeDef HAL_SMARTCARD_Abort_IT(SMARTCARD_HandleTypeDef* hsc)
{
    ...
    if(hsc->hdmatx != NULL)
    {
        ...
        if(HAL_IS_BIT_SET(hsc->Instance->CR3, USART_CR3_DMAT)) { hsc->hdmatx->XferAbortCallback = SMARTCARD_DMATxAbortCallback; }
        else { hsc->hdmatx->XferAbortCallback = NULL; }
    }
    ...
    if(hsc->hdmarx != NULL)
    {
        /* Set DMA Abort Complete callback if SMARTCARD DMA Rx request if enabled.
           Otherwise, set it to NULL */
        if(HAL_IS_BIT_SET(hsc->Instance->CR3, USART_CR3_DMAR)) { hsc->hdmarx->XferAbortCallback = SMARTCARD_DMARxAbortCallback; }
        else { hsc->hdmarx->XferAbortCallback = NULL; }
    }
    ...
    if(HAL_IS_BIT_SET(hsc->Instance->CR3, USART_CR3_DMAT))
    {
        ...
        if(hsc->hdmatx != NULL)
        {
            ...
            /* Abort DMA TX */
            if(HAL_DMA_Abort_IT(hsc->hdmatx) != HAL_OK) { hsc->hdmatx->XferAbortCallback = NULL; }
...
void HAL_SMARTCARD_IRQHandler(SMARTCARD_HandleTypeDef* hsc)
{
       ...
       (((cr3its & USART_CR3_EIE) != RESET) || ((cr1its & (USART_CR1_RXNEIE | USART_CR1_PEIE)) != RESET)))
    {
        ...
        {
            /* SMARTCARD in mode Receiver ----------------------------------------*/
            if(((isrflags & USART_SR_RXNE) != RESET) && ((cr1its & USART_CR1_RXNEIE) != RESET)) { SMARTCARD_Receive_IT(hsc); }
...
static void SMARTCARD_DMAError(DMA_HandleTypeDef* hdma)
{
    ...
    if((hsc->gState == HAL_SMARTCARD_STATE_BUSY_TX) && dmarequest) { SMARTCARD_EndTxTransfer(hsc); }

    /* Stop SMARTCARD DMA Rx request if ongoing */
    dmarequest = HAL_IS_BIT_SET(hsc->Instance->CR3, USART_CR3_DMAR);
    if((hsc->RxState == HAL_SMARTCARD_STATE_BUSY_RX) && dmarequest) { SMARTCARD_EndRxTransfer(hsc); }
...
static void SMARTCARD_DMATxAbortCallback(DMA_HandleTypeDef* hdma)
{
    ...
    if(hsc->hdmarx != NULL)
    {
        if(hsc->hdmarx->XferAbortCallback != NULL) { return; }
    }
...
static void SMARTCARD_DMARxAbortCallback(DMA_HandleTypeDef* hdma)
{
    ...
    if(hsc->hdmatx != NULL)
    {
        if(hsc->hdmatx->XferAbortCallback != NULL) { return; }
    }
...
static void SMARTCARD_SetConfig(SMARTCARD_HandleTypeDef* hsc)
{
    ...
    tmpreg |=
        (uint32_t)(USART_CR2_CLKEN | hsc->Init.CLKPolarity | hsc->Init.CLKPhase | hsc->Init.CLKLastBit | hsc->Init.StopBits);
    /* Write to USART CR2 */
