...
HAL_StatusTypeDef HAL_SPDIFRX_Init(SPDIFRX_HandleTypeDef* hspdif)
{
    ...
    if(hspdif == NULL) { return HAL_ERROR; }
    ...
    if(hspdif->State == HAL_SPDIFRX_STATE_RESET)
    {
        ...
        if(hspdif->MspInitCallback == NULL) { hspdif->MspInitCallback = HAL_SPDIFRX_MspInit; /* Legacy weak MspInit  */ }
    ...
    tmpreg &= ~(SPDIFRX_CR_RXSTEO | SPDIFRX_CR_DRFMT | SPDIFRX_CR_PMSK | SPDIFRX_CR_VMSK | SPDIFRX_CR_CUMSK | SPDIFRX_CR_PTMSK |
                SPDIFRX_CR_CHSEL | SPDIFRX_CR_NBTR | SPDIFRX_CR_WFA | SPDIFRX_CR_INSEL);

    /* Sets the new configuration of the SPDIFRX peripheral */
    tmpreg |= (hspdif->Init.StereoMode | hspdif->Init.InputSelection | hspdif->Init.Retries | hspdif->Init.WaitForActivity |
               hspdif->Init.ChannelSelection | hspdif->Init.DataFormat | hspdif->Init.PreambleTypeMask |
               hspdif->Init.ChannelStatusMask | hspdif->Init.ValidityBitMask | hspdif->Init.ParityErrorMask);
...
HAL_StatusTypeDef HAL_SPDIFRX_DeInit(SPDIFRX_HandleTypeDef* hspdif)
{
    ...
    __HAL_SPDIFRX_IDLE(hspdif);
            ...
            #if (USE_HAL_SPDIFRX_REGISTER_CALLBACKS == 1)
    if(hspdif->MspDeInitCallback == NULL) { hspdif->MspDeInitCallback = HAL_SPDIFRX_MspDeInit; /* Legacy weak MspDeInit  */ }
                                               ...
                                               pSPDIFRX_CallbackTypeDef pCallback)
{
    ...
    if(HAL_SPDIFRX_STATE_READY == hspdif->State)
    {
        switch(CallbackID)
        {
            case HAL_SPDIFRX_RX_HALF_CB_ID: hspdif->RxHalfCpltCallback = pCallback; break;

            case HAL_SPDIFRX_RX_CPLT_CB_ID: hspdif->RxCpltCallback = pCallback; break;

            case HAL_SPDIFRX_CX_HALF_CB_ID: hspdif->CxHalfCpltCallback = pCallback; break;

            case HAL_SPDIFRX_CX_CPLT_CB_ID: hspdif->CxCpltCallback = pCallback; break;

            case HAL_SPDIFRX_ERROR_CB_ID: hspdif->ErrorCallback = pCallback; break;

            case HAL_SPDIFRX_MSPINIT_CB_ID: hspdif->MspInitCallback = pCallback; break;

            case HAL_SPDIFRX_MSPDEINIT_CB_ID: hspdif->MspDeInitCallback = pCallback; break;
...
HAL_StatusTypeDef HAL_SPDIFRX_UnRegisterCallback(SPDIFRX_HandleTypeDef* hspdif, HAL_SPDIFRX_CallbackIDTypeDef CallbackID)
{
    ...
    if(HAL_SPDIFRX_STATE_READY == hspdif->State)
    {
        switch(CallbackID)
        {
            case HAL_SPDIFRX_RX_HALF_CB_ID: hspdif->RxHalfCpltCallback = HAL_SPDIFRX_RxHalfCpltCallback; break;

            case HAL_SPDIFRX_RX_CPLT_CB_ID: hspdif->RxCpltCallback = HAL_SPDIFRX_RxCpltCallback; break;

            case HAL_SPDIFRX_CX_HALF_CB_ID: hspdif->CxHalfCpltCallback = HAL_SPDIFRX_CxHalfCpltCallback; break;

            case HAL_SPDIFRX_CX_CPLT_CB_ID: hspdif->CxCpltCallback = HAL_SPDIFRX_CxCpltCallback; break;

            case HAL_SPDIFRX_ERROR_CB_ID: hspdif->ErrorCallback = HAL_SPDIFRX_ErrorCallback; break;
...
HAL_StatusTypeDef HAL_SPDIFRX_SetDataFormat(SPDIFRX_HandleTypeDef* hspdif, SPDIFRX_SetDataFormatTypeDef sDataFormat)
{
    ...
    if(((tmpreg & SPDIFRX_STATE_RCV) == SPDIFRX_STATE_RCV) &&
       (((tmpreg & SPDIFRX_CR_DRFMT) != sDataFormat.DataFormat) || ((tmpreg & SPDIFRX_CR_RXSTEO) != sDataFormat.StereoMode)))
    {
        return HAL_ERROR;
    }
    ...
    tmpreg |= (sDataFormat.StereoMode | sDataFormat.DataFormat | sDataFormat.PreambleTypeMask | sDataFormat.ChannelStatusMask |
...
HAL_StatusTypeDef HAL_SPDIFRX_DMAStop(SPDIFRX_HandleTypeDef* hspdif)
{
    ...
    if(hspdif->hdmaDrRx != NULL) { __HAL_DMA_DISABLE(hspdif->hdmaDrRx); }
    if(hspdif->hdmaCsRx != NULL) { __HAL_DMA_DISABLE(hspdif->hdmaCsRx); }
 ...
 * @retval HAL state
 */
HAL_SPDIFRX_StateTypeDef HAL_SPDIFRX_GetState(const SPDIFRX_HandleTypeDef* const hspdif) { return hspdif->State; }
 ...
 */
uint32_t HAL_SPDIFRX_GetError(const SPDIFRX_HandleTypeDef* const hspdif) { return hspdif->ErrorCode; }
