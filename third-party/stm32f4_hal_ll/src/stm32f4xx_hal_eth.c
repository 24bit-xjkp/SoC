 ...
 */
        ...
        #define ETH_DMARXDESC_ERRORS_MASK                                                                                        \
            ((uint32_t)(ETH_DMARXDESC_DBE | ETH_DMARXDESC_RE | ETH_DMARXDESC_OE | ETH_DMARXDESC_RWT | ETH_DMARXDESC_LC |         \
                        ETH_DMARXDESC_CE | ETH_DMARXDESC_DE | ETH_DMARXDESC_IPV4HCE))
        ...
        #define INCR_TX_DESC_INDEX(inx, offset)                                                                                  \
            do {                                                                                                                 \
                (inx) += (offset);                                                                                               \
                if((inx) >= (uint32_t)ETH_TX_DESC_CNT) { (inx) = ((inx) - (uint32_t)ETH_TX_DESC_CNT); }                          \
            }                                                                                                                    \
            while(0)
        ...
        #define INCR_RX_DESC_INDEX(inx, offset)                                                                                  \
            do {                                                                                                                 \
                (inx) += (offset);                                                                                               \
                if((inx) >= (uint32_t)ETH_RX_DESC_CNT) { (inx) = ((inx) - (uint32_t)ETH_RX_DESC_CNT); }                          \
            }                                                                                                                    \
            while(0)
/**
 * @}
...
HAL_StatusTypeDef HAL_ETH_Init(ETH_HandleTypeDef* heth)
{
    ...
    if(heth->gState == HAL_ETH_STATE_RESET)
    {
        ...
        if(heth->MspInitCallback == NULL) { heth->MspInitCallback = HAL_ETH_MspInit; }
...
HAL_StatusTypeDef HAL_ETH_DeInit(ETH_HandleTypeDef* heth)
{
    ...
    if(heth->MspDeInitCallback == NULL) { heth->MspDeInitCallback = HAL_ETH_MspDeInit; }
    ...
    HAL_ETH_RegisterCallback(ETH_HandleTypeDef* heth, HAL_ETH_CallbackIDTypeDef CallbackID, pETH_CallbackTypeDef pCallback)
{
    ...
    if(heth->gState == HAL_ETH_STATE_READY)
    {
        switch(CallbackID)
        {
            case HAL_ETH_TX_COMPLETE_CB_ID: heth->TxCpltCallback = pCallback; break;

            case HAL_ETH_RX_COMPLETE_CB_ID: heth->RxCpltCallback = pCallback; break;
            ...
            case HAL_ETH_MSPDEINIT_CB_ID: heth->MspDeInitCallback = pCallback; break;
...
HAL_StatusTypeDef HAL_ETH_UnRegisterCallback(ETH_HandleTypeDef* heth, HAL_ETH_CallbackIDTypeDef CallbackID)
{
    ...
    if(heth->gState == HAL_ETH_STATE_READY)
    {
        switch(CallbackID)
        {
            case HAL_ETH_TX_COMPLETE_CB_ID: heth->TxCpltCallback = HAL_ETH_TxCpltCallback; break;

            case HAL_ETH_RX_COMPLETE_CB_ID: heth->RxCpltCallback = HAL_ETH_RxCpltCallback; break;

            case HAL_ETH_ERROR_CB_ID: heth->ErrorCallback = HAL_ETH_ErrorCallback; break;

            case HAL_ETH_PMT_CB_ID: heth->PMTCallback = HAL_ETH_PMTCallback; break;

            case HAL_ETH_WAKEUP_CB_ID: heth->WakeUpCallback = HAL_ETH_WakeUpCallback; break;

            case HAL_ETH_MSPINIT_CB_ID: heth->MspInitCallback = HAL_ETH_MspInit; break;

            case HAL_ETH_MSPDEINIT_CB_ID: heth->MspDeInitCallback = HAL_ETH_MspDeInit; break;
...
HAL_StatusTypeDef HAL_ETH_ReadData(ETH_HandleTypeDef* heth, void** pAppBuff)
{
    ...
    if(heth->gState != HAL_ETH_STATE_STARTED) { return HAL_ERROR; }
    ...
    while((READ_BIT(dmarxdesc->DESC0, ETH_DMARXDESC_OWN) == (uint32_t)RESET) && (desccnt < desccntmax) && (rxdataready == 0U))
    {
        ...
        if((READ_BIT(dmarxdesc->DESC0, ETH_DMARXDESC_FS) != (uint32_t)RESET) || (heth->RxDescList.pRxStart != NULL))
        {
            ...
            heth->rxLinkCallback(&heth->RxDescList.pRxStart,
...
static void ETH_UpdateDescriptor(ETH_HandleTypeDef* heth)
{
    ...
    while((desccount > 0U) && (allocStatus != 0U))
    {
        ...
        if(allocStatus != 0U)
        {
            ...
            else { WRITE_REG(dmarxdesc->DESC1, heth->Init.RxBuffLen | ETH_DMARXDESC_RCH); }
    ...
    while((numOfBuf != 0U) && (pktTxStatus != 0U))
    {
        ...
        if(pktInUse != 0U)
        {
            ...
            if((heth->Init.TxDesc[idx].DESC0 & ETH_DMATXDESC_OWN) == 0U)
            {
                ...
                else { timestamp->TimeStampHigh = timestamp->TimeStampLow = UINT32_MAX; }
        #endif /* HAL_ETH_USE_PTP */
...
HAL_StatusTypeDef HAL_ETH_PTP_SetConfig(ETH_HandleTypeDef* heth, ETH_PTP_ConfigTypeDef* ptpconfig)
{
    ...
    if(ptpconfig == NULL) { return HAL_ERROR; }

    tmpTSCR = ptpconfig->Timestamp | ((uint32_t)ptpconfig->TimestampUpdate << ETH_PTPTSCR_TSFCU_Pos) |
...
HAL_StatusTypeDef HAL_ETH_PTP_GetConfig(ETH_HandleTypeDef* heth, ETH_PTP_ConfigTypeDef* ptpconfig)
{
    ...
    ptpconfig->TimestampUpdate =
        ((READ_BIT(heth->Instance->PTPTSCR, ETH_PTPTSCR_TSFCU) >> ETH_PTPTSCR_TSFCU_Pos) > 0U) ? ENABLE : DISABLE;
    ptpconfig->TimestampAll =
        ((READ_BIT(heth->Instance->PTPTSCR, ETH_PTPTSCR_TSSARFE) >> ETH_PTPTSCR_TSSARFE_Pos) > 0U) ? ENABLE : DISABLE;
    ptpconfig->TimestampRolloverMode =
        ((READ_BIT(heth->Instance->PTPTSCR, ETH_PTPTSCR_TSSSR) >> ETH_PTPTSCR_TSSSR_Pos) > 0U) ? ENABLE : DISABLE;
    ptpconfig->TimestampV2 =
        ((READ_BIT(heth->Instance->PTPTSCR, ETH_PTPTSCR_TSPTPPSV2E) >> ETH_PTPTSCR_TSPTPPSV2E_Pos) > 0U) ? ENABLE : DISABLE;
    ptpconfig->TimestampEthernet =
        ((READ_BIT(heth->Instance->PTPTSCR, ETH_PTPTSCR_TSSPTPOEFE) >> ETH_PTPTSCR_TSSPTPOEFE_Pos) > 0U) ? ENABLE : DISABLE;
    ptpconfig->TimestampIPv6 =
        ((READ_BIT(heth->Instance->PTPTSCR, ETH_PTPTSCR_TSSIPV6FE) >> ETH_PTPTSCR_TSSIPV6FE_Pos) > 0U) ? ENABLE : DISABLE;
    ptpconfig->TimestampIPv4 =
        ((READ_BIT(heth->Instance->PTPTSCR, ETH_PTPTSCR_TSSIPV4FE) >> ETH_PTPTSCR_TSSIPV4FE_Pos) > 0U) ? ENABLE : DISABLE;
    ptpconfig->TimestampEvent =
        ((READ_BIT(heth->Instance->PTPTSCR, ETH_PTPTSCR_TSSEME) >> ETH_PTPTSCR_TSSEME_Pos) > 0U) ? ENABLE : DISABLE;
    ptpconfig->TimestampMaster =
        ((READ_BIT(heth->Instance->PTPTSCR, ETH_PTPTSCR_TSSMRME) >> ETH_PTPTSCR_TSSMRME_Pos) > 0U) ? ENABLE : DISABLE;
    ptpconfig->TimestampFilter =
        ((READ_BIT(heth->Instance->PTPTSCR, ETH_PTPTSCR_TSPFFMAE) >> ETH_PTPTSCR_TSPFFMAE_Pos) > 0U) ? ENABLE : DISABLE;
    ptpconfig->TimestampClockType =
        ((READ_BIT(heth->Instance->PTPTSCR, ETH_PTPTSCR_TSCNT) >> ETH_PTPTSCR_TSCNT_Pos) > 0U) ? ENABLE : DISABLE;
    ...
    HAL_ETH_PTP_AddTimeOffset(ETH_HandleTypeDef* heth, ETH_PtpUpdateTypeDef ptpoffsettype, ETH_TimeTypeDef* timeoffset)
{
    if(heth->IsPtpConfigured == HAL_ETH_PTP_CONFIGURED)
    {
        if(ptpoffsettype == HAL_ETH_PTP_NEGATIVE_UPDATE)
        {
            ...
            else { heth->Instance->PTPTSLUR = ETH_PTPTSHR_VALUE - timeoffset->NanoSeconds + 1U; }
        }
        else
        {
            /* Set Seconds update */
...
void HAL_ETH_IRQHandler(ETH_HandleTypeDef* heth)
{
    ...
    if(((dma_flag & ETH_DMASR_AIS) != 0U) && ((dma_itsource & ETH_DMAIER_AISE) != 0U))
    {
        ...
        else
        {
            ...
            heth->DMAErrorCode =
                READ_BIT(heth->Instance->DMASR, (ETH_DMASR_ETS | ETH_DMASR_RWTS | ETH_DMASR_RBUS | ETH_DMASR_AIS));
...
HAL_StatusTypeDef HAL_ETH_ReadPHYRegister(ETH_HandleTypeDef* heth, uint32_t PHYAddr, uint32_t PHYReg, uint32_t* pRegValue)
{
    ...
    {
        /* Check for the Timeout */
        if((HAL_GetTick() - tickstart) > PHY_READ_TO) { return HAL_ERROR; }
...
HAL_StatusTypeDef HAL_ETH_WritePHYRegister(const ETH_HandleTypeDef* heth, uint32_t PHYAddr, uint32_t PHYReg, uint32_t RegValue)
{
    ...
    {
        /* Check for the Timeout */
        if((HAL_GetTick() - tickstart) > PHY_WRITE_TO) { return HAL_ERROR; }
...
{
    if(macconf == NULL) { return HAL_ERROR; }
...
{
    if(dmaconf == NULL) { return HAL_ERROR; }

    dmaconf->DMAArbitration =
        READ_BIT(heth->Instance->DMABMR, (ETH_DMAARBITRATION_RXPRIORTX | ETH_DMAARBITRATION_ROUNDROBIN_RXTX_4_1));
...
HAL_StatusTypeDef HAL_ETH_SetMACFilterConfig(ETH_HandleTypeDef* heth, const ETH_MACFilterConfigTypeDef* pFilterConfig)
{
    ...
    if(pFilterConfig == NULL) { return HAL_ERROR; }

    filterconfig = ((uint32_t)pFilterConfig->PromiscuousMode | ((uint32_t)pFilterConfig->HashUnicast << 1) |
...
HAL_StatusTypeDef HAL_ETH_SetSourceMACAddrMatch(const ETH_HandleTypeDef* heth, uint32_t AddrNbr, const uint8_t* pMACAddr)
{
    ...
    if(pMACAddr == NULL) { return HAL_ERROR; }
...
{
    uint32_t tmpreg1;
    if(pHashTable == NULL) { return HAL_ERROR; }
...
void HAL_ETH_SetRxVLANIdentifier(ETH_HandleTypeDef* heth, uint32_t ComparisonBits, uint32_t VLANIdentifier)
{
    ...
    if(ComparisonBits == ETH_VLANTAGCOMPARISON_16BIT) { CLEAR_BIT(heth->Instance->MACVLANTR, ETH_MACVLANTR_VLANTC); }
    else { SET_BIT(heth->Instance->MACVLANTR, ETH_MACVLANTR_VLANTC); }
...
HAL_StatusTypeDef HAL_ETH_SetWakeUpFilter(ETH_HandleTypeDef* heth, uint32_t* pFilter, uint32_t Count)
{
    ...
    if(pFilter == NULL) { return HAL_ERROR; }
...
static void ETH_SetMACConfig(ETH_HandleTypeDef* heth, const ETH_MACConfigTypeDef* macconf)
{
    ...
    tmpreg1 |=
        (uint32_t)(((uint32_t)macconf->CRCStripTypePacket << 25U) |
                   ((uint32_t)((macconf->Watchdog == DISABLE) ? 1U : 0U) << 23U) |
                   ((uint32_t)((macconf->Jabber == DISABLE) ? 1U : 0U) << 22U) | (uint32_t)macconf->InterPacketGapVal |
                   ((uint32_t)macconf->CarrierSenseDuringTransmit << 16U) | macconf->Speed |
                   ((uint32_t)((macconf->ReceiveOwn == DISABLE) ? 1U : 0U) << 13U) | ((uint32_t)macconf->LoopbackMode << 12U) |
                   macconf->DuplexMode | ((uint32_t)macconf->ChecksumOffload << 10U) |
    ...
    tmpreg1 |=
        (uint32_t)((macconf->PauseTime << 16U) | ((uint32_t)((macconf->ZeroQuantaPause == DISABLE) ? 1U : 0U) << 7U) |
                   macconf->PauseLowThreshold | ((uint32_t)((macconf->UnicastPausePacketDetect == ENABLE) ? 1U : 0U) << 3U) |
...
static void ETH_SetDMAConfig(ETH_HandleTypeDef* heth, const ETH_DMAConfigTypeDef* dmaconf)
{
    ...
    tmpreg1 |= (uint32_t)(((uint32_t)((dmaconf->DropTCPIPChecksumErrorFrame == DISABLE) ? 1U : 0U) << 26U) |
                          ...
                          ((uint32_t)dmaconf->TransmitStoreForward << 21U) | dmaconf->TransmitThresholdControl |
                          ((uint32_t)dmaconf->ForwardErrorFrames << 7U) | ((uint32_t)dmaconf->ForwardUndersizedGoodFrames << 6U) |
    ...
    (heth->Instance)->DMABMR = (uint32_t)(((uint32_t)dmaconf->AddressAlignedBeats << 25U) | dmaconf->BurstMode |
...
static void ETH_DMATxDescListInit(ETH_HandleTypeDef* heth)
{
    ...
    for(i = 0; i < (uint32_t)ETH_TX_DESC_CNT; i++)
    {
        ...
        if(i < ((uint32_t)ETH_TX_DESC_CNT - 1U)) { WRITE_REG(dmatxdesc->DESC3, (uint32_t)(heth->Init.TxDesc + i + 1U)); }
        else { WRITE_REG(dmatxdesc->DESC3, (uint32_t)(heth->Init.TxDesc)); }
...
static void ETH_DMARxDescListInit(ETH_HandleTypeDef* heth)
{
    ...
    for(i = 0; i < (uint32_t)ETH_RX_DESC_CNT; i++)
    {
        ...
        if(i < ((uint32_t)ETH_RX_DESC_CNT - 1U)) { WRITE_REG(dmarxdesc->DESC3, (uint32_t)(heth->Init.RxDesc + i + 1U)); }
        else { WRITE_REG(dmarxdesc->DESC3, (uint32_t)(heth->Init.RxDesc)); }
    }
