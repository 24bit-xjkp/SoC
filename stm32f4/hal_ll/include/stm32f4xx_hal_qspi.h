...
#include "stm32f4xx_hal_def.h"
    ...
    typedef struct
    {
        ...
        uint32_t ChipSelectHighTime; /* Specifies the Chip Select High Time. ChipSelectHighTime+1 defines the minimum
    ...
    typedef struct
    {
        ...
        uint32_t AlternateBytes;     /* Specifies the Alternate Bytes to be sent (Size from 1 to 4 bytes according
                                        AlternateBytesSize)          This parameter can be a value (32-bits) between 0x0 and
                                        0xFFFFFFFF */
    ...
    typedef struct
    {
        uint32_t TimeOutPeriod;     /* Specifies the number of clock to wait when the FIFO is full before to release the
                                       chip select.     This parameter can be any value between 0 and 0xFFFF */
    ...
    #define IS_QSPI_DDR_HHC(DDR_HHC) (((DDR_HHC) == QSPI_DDR_HHC_ANALOG_DELAY) || ((DDR_HHC) == QSPI_DDR_HHC_HALF_CLK_DELAY))
    ...
    #define IS_QSPI_TIMEOUT_ACTIVATION(TCEN) (((TCEN) == QSPI_TIMEOUT_COUNTER_DISABLE) || ((TCEN) == QSPI_TIMEOUT_COUNTER_ENABLE))
