                                  ...
                                  FMC_NAND_PCC_TimingTypeDef* IOSpaceTiming)
{
    ...
    if(hpccard == NULL) { return HAL_ERROR; }
    ...
    if(hpccard->State == HAL_PCCARD_STATE_RESET)
    {
        ...
        if(hpccard->MspInitCallback == NULL) { hpccard->MspInitCallback = HAL_PCCARD_MspInit; }
...
{
        #if (USE_HAL_PCCARD_REGISTER_CALLBACKS == 1)
    if(hpccard->MspDeInitCallback == NULL) { hpccard->MspDeInitCallback = HAL_PCCARD_MspDeInit; }
...
HAL_StatusTypeDef HAL_PCCARD_Read_ID(PCCARD_HandleTypeDef* hpccard, uint8_t CompactFlash_ID[], uint8_t* pStatus)
{
    ...
    /* Check the PCCARD controller state */
    if(hpccard->State == HAL_PCCARD_STATE_BUSY) { return HAL_BUSY; }
    ...
    *(__IO uint16_t*)(PCCARD_IO_SPACE_PRIMARY_ADDR | ATA_STATUS_CMD) = (uint16_t)0xECEC;
    ...
    if(timeout == 0U) { *pStatus = PCCARD_TIMEOUT_ERROR; }
    else
    {
    ...
    HAL_PCCARD_Read_Sector(PCCARD_HandleTypeDef* hpccard, uint16_t* pBuffer, uint16_t SectorAddress, uint8_t* pStatus)
{
    ...
    *(__IO uint16_t*)(PCCARD_IO_SPACE_PRIMARY_ADDR | ATA_STATUS_CMD) = (uint16_t)0xE4A0;
    ...
    /* Read bytes */
    for(; index < PCCARD_SECTOR_SIZE; index++) { *(uint16_t*)pBuffer++ = *(uint16_t*)(PCCARD_IO_SPACE_PRIMARY_ADDR); }
    ...
    HAL_PCCARD_Write_Sector(PCCARD_HandleTypeDef* hpccard, uint16_t* pBuffer, uint16_t SectorAddress, uint8_t* pStatus)
{
    ...
    *(__IO uint16_t*)(PCCARD_IO_SPACE_PRIMARY_ADDR | ATA_STATUS_CMD) = (uint16_t)0x30A0;
    ...
    /* Write bytes */
    for(; index < PCCARD_SECTOR_SIZE; index++) { *(uint16_t*)(PCCARD_IO_SPACE_PRIMARY_ADDR) = *(uint16_t*)pBuffer++; }
                                              ...
                                              pPCCARD_CallbackTypeDef pCallback)
{
    ...
    if(hpccard->State == HAL_PCCARD_STATE_READY)
    {
        switch(CallbackId)
        {
            case HAL_PCCARD_MSP_INIT_CB_ID: hpccard->MspInitCallback = pCallback; break;
            case HAL_PCCARD_MSP_DEINIT_CB_ID: hpccard->MspDeInitCallback = pCallback; break;
            case HAL_PCCARD_IT_CB_ID: hpccard->ItCallback = pCallback; break;
            default:
...
HAL_StatusTypeDef HAL_PCCARD_UnRegisterCallback(PCCARD_HandleTypeDef* hpccard, HAL_PCCARD_CallbackIDTypeDef CallbackId)
{
    ...
    if(hpccard->State == HAL_PCCARD_STATE_READY)
    {
        switch(CallbackId)
        {
            case HAL_PCCARD_MSP_INIT_CB_ID: hpccard->MspInitCallback = HAL_PCCARD_MspInit; break;
            case HAL_PCCARD_MSP_DEINIT_CB_ID: hpccard->MspDeInitCallback = HAL_PCCARD_MspDeInit; break;
            case HAL_PCCARD_IT_CB_ID: hpccard->ItCallback = HAL_PCCARD_ITCallback; break;
            default:
                /* update return status */
                status = HAL_ERROR;
                break;
...
HAL_PCCARD_StatusTypeDef HAL_PCCARD_GetStatus(PCCARD_HandleTypeDef* hpccard)
{
    ...
    /* Check the PCCARD controller state */
    if(hpccard->State == HAL_PCCARD_STATE_BUSY) { return HAL_PCCARD_STATUS_ONGOING; }
    ...
    if(timeout == 0U) { status_pccard = PCCARD_TIMEOUT_ERROR; }
...
HAL_PCCARD_StatusTypeDef HAL_PCCARD_ReadStatus(PCCARD_HandleTypeDef* hpccard)
{
    ...
    if((data & PCCARD_TIMEOUT_ERROR) == PCCARD_TIMEOUT_ERROR) { status_pccard = PCCARD_TIMEOUT_ERROR; }
    else if((data & PCCARD_READY) == PCCARD_READY) { status_pccard = PCCARD_READY; }
