...
#include "stm32f4xx.h"
    ...
    {
        if(OverSampling == LL_USART_OVERSAMPLING_8) { USARTx->BRR = (uint16_t)(__LL_USART_DIV_SAMPLING8(PeriphClk, BaudRate)); }
        else { USARTx->BRR = (uint16_t)(__LL_USART_DIV_SAMPLING16(PeriphClk, BaudRate)); }
    }
    ...
    __STATIC_INLINE uint32_t LL_USART_GetBaudRate(const USART_TypeDef* USARTx, uint32_t PeriphClk, uint32_t OverSampling)
    {
        ...
        {
            if((usartdiv & 0xFFFFU) != 0U) { brrresult = PeriphClk / usartdiv; }
        }
        return (brrresult);
    }
