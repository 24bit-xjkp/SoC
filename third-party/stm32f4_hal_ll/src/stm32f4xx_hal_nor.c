...
HAL_StatusTypeDef HAL_NOR_Init(NOR_HandleTypeDef* hnor, FMC_NORSRAM_TimingTypeDef* Timing, FMC_NORSRAM_TimingTypeDef* ExtTiming)
{
    ...
    if(hnor->State == HAL_NOR_STATE_RESET)
    {
        ...
        if(hnor->MspInitCallback == NULL) { hnor->MspInitCallback = HAL_NOR_MspInit; }
    ...
    if(hnor->Init.MemoryDataWidth == FMC_NORSRAM_MEM_BUS_WIDTH_8) { uwNORMemoryDataWidth = NOR_MEMORY_8B; }
    else { uwNORMemoryDataWidth = NOR_MEMORY_16B; }

    /* Initialize the NOR controller state */
    hnor->State = HAL_NOR_STATE_READY;

    /* Select the NOR device address */
    if(hnor->Init.NSBank == FMC_NORSRAM_BANK1) { deviceaddress = NOR_MEMORY_ADRESS1; }
    else if(hnor->Init.NSBank == FMC_NORSRAM_BANK2) { deviceaddress = NOR_MEMORY_ADRESS2; }
    else if(hnor->Init.NSBank == FMC_NORSRAM_BANK3) { deviceaddress = NOR_MEMORY_ADRESS3; }
    else /* FMC_NORSRAM_BANK4 */ { deviceaddress = NOR_MEMORY_ADRESS4; }
    ...
    else
    {
        ...
        else { NOR_WRITE(NOR_ADDR_SHIFT(deviceaddress, uwNORMemoryDataWidth, NOR_CMD_ADDRESS_FIRST_CFI), NOR_CMD_DATA_CFI); }
...
{
        #if (USE_HAL_NOR_REGISTER_CALLBACKS == 1)
    if(hnor->MspDeInitCallback == NULL) { hnor->MspDeInitCallback = HAL_NOR_MspDeInit; }
...
HAL_StatusTypeDef HAL_NOR_Read_ID(NOR_HandleTypeDef* hnor, NOR_IDTypeDef* pNOR_ID)
{
    ...
    state = hnor->State;
    if(state == HAL_NOR_STATE_BUSY) { return HAL_BUSY; }
    else if(state == HAL_NOR_STATE_PROTECTED) { return HAL_ERROR; }
    else if(state == HAL_NOR_STATE_READY)
    {
        ...
        else if(hnor->CommandSet == NOR_INTEL_SHARP_EXT_COMMAND_SET) { NOR_WRITE(deviceaddress, NOR_CMD_DATA_AUTO_SELECT); }
        else
        {
...
HAL_StatusTypeDef HAL_NOR_ReturnToReadMode(NOR_HandleTypeDef* hnor)
{
    ...
    else if(state == HAL_NOR_STATE_READY)
    {
        ...
        if(hnor->CommandSet == NOR_AMD_FUJITSU_COMMAND_SET) { NOR_WRITE(deviceaddress, NOR_CMD_DATA_READ_RESET); }
        else if(hnor->CommandSet == NOR_INTEL_SHARP_EXT_COMMAND_SET) { NOR_WRITE(deviceaddress, NOR_CMD_READ_ARRAY); }
        else
        {
            /* Primary command set not supported by the driver */
            status = HAL_ERROR;
...
HAL_StatusTypeDef HAL_NOR_Read(NOR_HandleTypeDef* hnor, uint32_t* pAddress, uint16_t* pData)
{
    ...
    else if(state == HAL_NOR_STATE_READY)
    {
        ...
        else if(hnor->CommandSet == NOR_INTEL_SHARP_EXT_COMMAND_SET) { NOR_WRITE(pAddress, NOR_CMD_READ_ARRAY); }
        else
        {
            /* Primary command set not supported by the driver */
            status = HAL_ERROR;
...
HAL_StatusTypeDef HAL_NOR_Program(NOR_HandleTypeDef* hnor, uint32_t* pAddress, uint16_t* pData)
{
    ...
    /* Check the NOR controller state */
    if(hnor->State == HAL_NOR_STATE_BUSY) { return HAL_BUSY; }
    else if(hnor->State == HAL_NOR_STATE_READY)
    {
        ...
        else if(hnor->CommandSet == NOR_INTEL_SHARP_EXT_COMMAND_SET) { NOR_WRITE(pAddress, NOR_CMD_WORD_PROGRAM); }
        else
        {
            /* Primary command set not supported by the driver */
            status = HAL_ERROR;
    ...
    HAL_NOR_RegisterCallback(NOR_HandleTypeDef* hnor, HAL_NOR_CallbackIDTypeDef CallbackId, pNOR_CallbackTypeDef pCallback)
{
    ...
    if((state == HAL_NOR_STATE_READY) || (state == HAL_NOR_STATE_RESET) || (state == HAL_NOR_STATE_PROTECTED))
    {
        switch(CallbackId)
        {
            case HAL_NOR_MSP_INIT_CB_ID: hnor->MspInitCallback = pCallback; break;
            case HAL_NOR_MSP_DEINIT_CB_ID: hnor->MspDeInitCallback = pCallback; break;
            default:
...
HAL_StatusTypeDef HAL_NOR_UnRegisterCallback(NOR_HandleTypeDef* hnor, HAL_NOR_CallbackIDTypeDef CallbackId)
{
    ...
    if((state == HAL_NOR_STATE_READY) || (state == HAL_NOR_STATE_RESET) || (state == HAL_NOR_STATE_PROTECTED))
    {
        switch(CallbackId)
        {
            case HAL_NOR_MSP_INIT_CB_ID: hnor->MspInitCallback = HAL_NOR_MspInit; break;
            case HAL_NOR_MSP_DEINIT_CB_ID: hnor->MspDeInitCallback = HAL_NOR_MspDeInit; break;
            default:
                /* update return status */
                status = HAL_ERROR;
                break;
...
HAL_NOR_StatusTypeDef HAL_NOR_GetStatus(NOR_HandleTypeDef* hnor, uint32_t Address, uint32_t Timeout)
{
    ...
    if(hnor->CommandSet == NOR_AMD_FUJITSU_COMMAND_SET)
    {
        while((status != HAL_NOR_STATUS_SUCCESS) && (status != HAL_NOR_STATUS_TIMEOUT))
        {
            ...
            {
                if(((HAL_GetTick() - tickstart) > Timeout) || (Timeout == 0U)) { status = HAL_NOR_STATUS_TIMEOUT; }
            }
            ...
            if((tmpsr1 & NOR_MASK_STATUS_DQ6) == (tmpsr2 & NOR_MASK_STATUS_DQ6)) { return HAL_NOR_STATUS_SUCCESS; }
            ...
            /* If DQ6 did not toggle between the two reads then return HAL_NOR_STATUS_SUCCESS  */
            if((tmpsr1 & NOR_MASK_STATUS_DQ6) == (tmpsr2 & NOR_MASK_STATUS_DQ6)) { return HAL_NOR_STATUS_SUCCESS; }
            if((tmpsr1 & NOR_MASK_STATUS_DQ5) == NOR_MASK_STATUS_DQ5) { return HAL_NOR_STATUS_ERROR; }
        }
    }
    else if(hnor->CommandSet == NOR_INTEL_SHARP_EXT_COMMAND_SET)
    {
        do {
            ...
            if(Timeout != HAL_MAX_DELAY)
            {
                if(((HAL_GetTick() - tickstart) > Timeout) || (Timeout == 0U)) { return HAL_NOR_STATUS_TIMEOUT; }
            }
        }
        ...
        else { status = HAL_NOR_STATUS_SUCCESS; }
    }
    else
    {
        /* Primary command set not supported by the driver */
