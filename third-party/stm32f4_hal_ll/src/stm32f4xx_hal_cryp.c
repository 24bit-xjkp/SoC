  ...
  ==============================================================================
    [..]
      ...
      (#)Initialize the CRYP low level resources by implementing the HAL_CRYP_MspInit():
         (##) Enable the CRYP interface clock using __HAL_RCC_CRYP_CLK_ENABLE()or __HAL_RCC_AES_CLK_ENABLE for TinyAES
 IP
 ...
 */
        ...
        #define CRYP_PHASE_PROCESS 0x00000002U /*!< CRYP peripheral is in processing phase */
            ...
            #define CRYP_OPERATINGMODE_KEYDERIVATION_DECRYPT                                                                     \
                AES_CR_MODE /*!< Key derivation and decryption only used when performing ECB and CBC decryptions (Mode 4) */
...
HAL_StatusTypeDef HAL_CRYP_Init(CRYP_HandleTypeDef* hcryp)
{
    ...
    if(hcryp == NULL) { return HAL_ERROR; }
    ...
    if(hcryp->State == HAL_CRYP_STATE_RESET)
    {
        ...
        if(hcryp->MspInitCallback == NULL) { hcryp->MspInitCallback = HAL_CRYP_MspInit; /* Legacy weak MspInit  */ }
...
HAL_StatusTypeDef HAL_CRYP_DeInit(CRYP_HandleTypeDef* hcryp)
{
    ...
    if(hcryp->MspDeInitCallback == NULL) { hcryp->MspDeInitCallback = HAL_CRYP_MspDeInit; /* Legacy weak MspDeInit  */ }
...
{
    /* Check the CRYP handle allocation */
    if((hcryp == NULL) || (pConf == NULL)) { return HAL_ERROR; }
    ...
    if(hcryp->State == HAL_CRYP_STATE_READY)
    {
        ...
        hcryp->Init.HeaderWidthUnit = pConf->HeaderWidthUnit;
            ...
            /* Set the key size(This bit field is don't care in the DES or TDES modes) data type, AlgoMode and operating
             * mode*/
        #if defined(CRYP)
    ...
    HAL_CRYP_RegisterCallback(CRYP_HandleTypeDef* hcryp, HAL_CRYP_CallbackIDTypeDef CallbackID, pCRYP_CallbackTypeDef pCallback)
{
    ...
    if(hcryp->State == HAL_CRYP_STATE_READY)
    {
        switch(CallbackID)
        {
            case HAL_CRYP_INPUT_COMPLETE_CB_ID: hcryp->InCpltCallback = pCallback; break;

            case HAL_CRYP_OUTPUT_COMPLETE_CB_ID: hcryp->OutCpltCallback = pCallback; break;

            case HAL_CRYP_ERROR_CB_ID: hcryp->ErrorCallback = pCallback; break;

            case HAL_CRYP_MSPINIT_CB_ID: hcryp->MspInitCallback = pCallback; break;

            case HAL_CRYP_MSPDEINIT_CB_ID: hcryp->MspDeInitCallback = pCallback; break;
...
HAL_StatusTypeDef HAL_CRYP_UnRegisterCallback(CRYP_HandleTypeDef* hcryp, HAL_CRYP_CallbackIDTypeDef CallbackID)
{
    ...
    if(hcryp->State == HAL_CRYP_STATE_READY)
    {
        switch(CallbackID)
        {
            ...
            case HAL_CRYP_MSPINIT_CB_ID: hcryp->MspInitCallback = HAL_CRYP_MspInit; break;

            case HAL_CRYP_MSPDEINIT_CB_ID: hcryp->MspDeInitCallback = HAL_CRYP_MspDeInit; break;
...
HAL_StatusTypeDef HAL_CRYP_Encrypt(CRYP_HandleTypeDef* hcryp, uint32_t* Input, uint16_t Size, uint32_t* Output, uint32_t Timeout)
{
    ...
    if(hcryp->State == HAL_CRYP_STATE_READY)
    {
        ...
        if(hcryp->Init.DataWidthUnit == CRYP_DATAWIDTHUNIT_WORD) { hcryp->Size = Size * 4U; }
        else { hcryp->Size = Size; }
...
HAL_StatusTypeDef HAL_CRYP_Encrypt_IT(CRYP_HandleTypeDef* hcryp, uint32_t* Input, uint16_t Size, uint32_t* Output)
{
    ...
    if(hcryp->State == HAL_CRYP_STATE_READY)
    {
        ...
        switch(algo)
        {
            ...
            case CRYP_AES_ECB:
            case CRYP_AES_CBC:
            case CRYP_AES_CTR: status = CRYP_AES_Encrypt_IT(hcryp); break;
            #if defined(CRYP_CR_ALGOMODE_AES_GCM)
            case CRYP_AES_GCM: status = CRYP_AESGCM_Process_IT(hcryp); break;

            case CRYP_AES_CCM: status = CRYP_AESCCM_Process_IT(hcryp); break;
            #endif /* GCM CCM defined*/
            default:
                hcryp->ErrorCode |= HAL_CRYP_ERROR_NOT_SUPPORTED;
                /* Change the CRYP peripheral state */
...
void HAL_CRYP_IRQHandler(CRYP_HandleTypeDef* hcryp)
{
    ...
    if((itstatus & (CRYP_IT_INI | CRYP_IT_OUTI)) != 0U)
    {
        ...
        {
            /* if header phase */
            if((hcryp->Instance->CR & CRYP_PHASE_HEADER) == CRYP_PHASE_HEADER) { CRYP_GCMCCM_SetHeaderPhase_IT(hcryp); }
            else /* if payload phase */ { CRYP_GCMCCM_SetPayloadPhase_IT(hcryp); }
        }
            #endif /* GCM CCM defined*/
        else { /* Nothing to do */ }
    }
    ...
    if(__HAL_CRYP_GET_FLAG(hcryp, CRYP_IT_CCF) != RESET)
    {
        if(__HAL_CRYP_GET_IT_SOURCE(hcryp, CRYP_IT_CCFIE) != RESET)
        {
            ...
            {
                /* if header phase */
                if(hcryp->Init.HeaderSize >= hcryp->CrypHeaderCount) { CRYP_GCMCCM_SetHeaderPhase_IT(hcryp); }
                else /* if payload phase */ { CRYP_GCMCCM_SetPayloadPhase_IT(hcryp); }
            }
            else /* AES Algorithm ECB,CBC or CTR*/ { CRYP_AES_IT(hcryp); }
        }
    }
    ...
    if(__HAL_CRYP_GET_IT_SOURCE(hcryp, CRYP_IT_ERRIE) != RESET)
    {
        ...
        if(__HAL_CRYP_GET_FLAG(hcryp, CRYP_IT_WRERR) != RESET) { hcryp->ErrorCode |= HAL_CRYP_ERROR_WRITE; }
        /* If read Error occurred */
        if(__HAL_CRYP_GET_FLAG(hcryp, CRYP_IT_RDERR) != RESET) { hcryp->ErrorCode |= HAL_CRYP_ERROR_READ; }
    }
        #endif /* End AES or CRYP */
}
...
static HAL_StatusTypeDef CRYP_TDES_Process(CRYP_HandleTypeDef* hcryp, uint32_t Timeout)
{
    ...
    while((hcryp->CrypInCount < (hcryp->Size / 4U)) && (outcount < (hcryp->Size / 4U)))
    {
        ...
        if(((hcryp->Instance->SR & CRYP_FLAG_OFNE) != 0x0U) && (outcount < (hcryp->Size / 4U)))
        {
             ...
             * temporary buffer  */
            for(i = 0U; i < 2U; i++) { temp[i] = hcryp->Instance->DOUT; }
            i = 0U;
...
static void CRYP_AES_ProcessData(CRYP_HandleTypeDef* hcryp, uint32_t Timeout)
{
    ...
    if(((hcryp->Instance->SR & CRYP_FLAG_OFNE) != 0x0U) && (outcount < (hcryp->Size / 4U)))
    {
        /* Read the output block from the Output FIFO and put them in temporary buffer then get CrypOutBuff from
         * temporary buffer  */
        for(i = 0U; i < 4U; i++) { temp[i] = hcryp->Instance->DOUT; }
        i = 0U;
    ...
    /* Read the output block from the output FIFO and put them in temporary buffer then get CrypOutBuff from temporary
     * buffer*/
    for(i = 0U; i < 4U; i++) { temp[i] = hcryp->Instance->DOUTR; }
    i = 0U;
...
static HAL_StatusTypeDef CRYP_AESGCM_Process(CRYP_HandleTypeDef* hcryp, uint32_t Timeout)
{
    ...
    else { hcryp->SizesSum = hcryp->Size; }
    ...
    if(DoKeyIVConfig == 1U)
    {
        ...
        if(CRYP_GCMCCM_SetHeaderPhase(hcryp, Timeout) != HAL_OK) { return HAL_ERROR; }
...
static HAL_StatusTypeDef CRYP_AESGCM_Process_DMA(CRYP_HandleTypeDef* hcryp)
{
    ...
    if(DoKeyIVConfig == 1U)
    {
        ...
        if(CRYP_GCMCCM_SetHeaderPhase_DMA(hcryp) != HAL_OK) { return HAL_ERROR; }
...
static HAL_StatusTypeDef CRYP_AESCCM_Process(CRYP_HandleTypeDef* hcryp, uint32_t Timeout)
{
    ...
    if((hcryp->Size % 16U) != 0U)
    {
        /* CRYP Workaround :  CRYP1 generates correct TAG  during CCM decryption only when ciphertext blocks size is
        multiple of 128 bits. If lthe size of the last block of payload is inferior to 128 bits, when CCM decryption is
        selected, then the TAG message will be wrong.*/
        CRYP_Workaround(hcryp, Timeout);
    }
            #endif /* CRYP */
...
static void CRYP_GCMCCM_SetPayloadPhase_IT(CRYP_HandleTypeDef* hcryp)
{
    ...
    {
        /* Size should be %4 in word and %16 in byte  otherwise TAG will  be incorrectly generated for GCM Encryption &
        CCM Decryption Workaround is implemented in polling mode, so if last block of payload <128bit don't use
        CRYP_AESGCM_Encrypt_IT otherwise TAG is incorrectly generated. */
    ...
    else /* Last block of payload < 128bit*/
    {
        ...
        payload <128bit don't use CRYP_Encrypt_IT otherwise TAG is incorrectly generated for GCM Encryption & CCM
        Decryption. */
...
static HAL_StatusTypeDef CRYP_GCMCCM_SetHeaderPhase(CRYP_HandleTypeDef* hcryp, uint32_t Timeout)
{
    ...
    if(hcryp->Init.HeaderWidthUnit == CRYP_HEADERWIDTHUNIT_WORD) { size_in_bytes = hcryp->Init.HeaderSize * 4U; }
    else { size_in_bytes = hcryp->Init.HeaderSize; }
...
static HAL_StatusTypeDef CRYP_GCMCCM_SetHeaderPhase_DMA(CRYP_HandleTypeDef* hcryp)
{
    ...
    /***************************** Header phase for GCM/GMAC or CCM *********************************/
    if(hcryp->Init.HeaderWidthUnit == CRYP_HEADERWIDTHUNIT_WORD) { headersize_in_bytes = hcryp->Init.HeaderSize * 4U; }
    else { headersize_in_bytes = hcryp->Init.HeaderSize; }
...
static void CRYP_Workaround(CRYP_HandleTypeDef* hcryp, uint32_t Timeout)
{
    ...
    if(hcryp->Init.Algorithm == CRYP_AES_GCM)
    {
        ...
        if((hcryp->Instance->CR & CRYP_CR_ALGODIR) == CRYP_OPERATINGMODE_ENCRYPT)
        {
            ...
            {
                if((npblb % 4U) == 1U) { intermediate_data[lastwordsize - 1U] &= 0xFFFFFF00U; }
                if((npblb % 4U) == 2U) { intermediate_data[lastwordsize - 1U] &= 0xFFFF0000U; }
                if((npblb % 4U) == 3U) { intermediate_data[lastwordsize - 1U] &= 0xFF000000U; }
            }
            else if((hcryp->Instance->CR & CRYP_CR_DATATYPE) == CRYP_DATATYPE_8B)
            {
                if((npblb % 4U) == 1U) { intermediate_data[lastwordsize - 1U] &= __REV(0xFFFFFF00U); }
                if((npblb % 4U) == 2U) { intermediate_data[lastwordsize - 1U] &= __REV(0xFFFF0000U); }
                if((npblb % 4U) == 3U) { intermediate_data[lastwordsize - 1U] &= __REV(0xFF000000U); }
            }
            else if((hcryp->Instance->CR & CRYP_CR_DATATYPE) == CRYP_DATATYPE_16B)
            {
                if((npblb % 4U) == 1U) { intermediate_data[lastwordsize - 1U] &= __ROR((0xFFFFFF00U), 16); }
                if((npblb % 4U) == 2U) { intermediate_data[lastwordsize - 1U] &= __ROR((0xFFFF0000U), 16); }
                if((npblb % 4U) == 3U) { intermediate_data[lastwordsize - 1U] &= __ROR((0xFF000000U), 16); }
            }
            else /*CRYP_DATATYPE_1B*/
            {
                if((npblb % 4U) == 1U) { intermediate_data[lastwordsize - 1U] &= __RBIT(0xFFFFFF00U); }
                if((npblb % 4U) == 2U) { intermediate_data[lastwordsize - 1U] &= __RBIT(0xFFFF0000U); }
                if((npblb % 4U) == 3U) { intermediate_data[lastwordsize - 1U] &= __RBIT(0xFF000000U); }
            }
            ...
            if((hcryp->Instance->SR & CRYP_FLAG_OFNE) != 0x0U)
            {
                for(index = 0U; index < 4U; index++) { intermediate_data[index] = hcryp->Instance->DOUT; }
            }
        }
    ...
    else
    {
        ...
        if((hcryp->Instance->CR & CRYP_CR_ALGODIR) == CRYP_OPERATINGMODE_DECRYPT)
        {
            ...
            for(index = lastwordsize; index < 4U; index++) { intermediate_data[index] = 0U; }
            if((npblb % 4U) == 1U) { intermediate_data[lastwordsize - 1U] &= 0xFFFFFF00U; }
            if((npblb % 4U) == 2U) { intermediate_data[lastwordsize - 1U] &= 0xFFFF0000U; }
            if((npblb % 4U) == 3U) { intermediate_data[lastwordsize - 1U] &= 0xFF000000U; }
            for(index = 0U; index < 4U; index++)
    ...
    if((hcryp->Instance->CR & AES_CR_MODE) == CRYP_OPERATINGMODE_ENCRYPT)
    {
        ...
        /*Write the intermediate_data in the IN FIFO */
        for(index = 0U; index < lastwordsize; index++) { hcryp->Instance->DINR = intermediate_data[index]; }
        while(index < 4U)
        {
            /* pad the data with zeros to have a complete block */
            hcryp->Instance->DINR = 0U;
        ...
        for(index = 0U; index < 4U; index++) { intermediate_data[index] = hcryp->Instance->DOUTR; }
    } /*End of Workaround 2*/
            #endif     /* End AES or CRYP */
}
        #endif /* AES or GCM CCM defined*/
...
static HAL_StatusTypeDef CRYP_WaitOnIFEMFlag(CRYP_HandleTypeDef* hcryp, uint32_t Timeout)
{
    ...
    while(HAL_IS_BIT_CLR(hcryp->Instance->SR, CRYP_FLAG_IFEM))
    {
        ...
        if(Timeout != HAL_MAX_DELAY)
        {
            if(((HAL_GetTick() - tickstart) > Timeout) || (Timeout == 0U)) { return HAL_ERROR; }
        }
    }
    return HAL_OK;
}
