                                ...
                                FMC_NAND_PCC_TimingTypeDef* AttSpace_Timing)
{
    ...
    if(hnand == NULL) { return HAL_ERROR; }
    ...
    if(hnand->State == HAL_NAND_STATE_RESET)
    {
        ...
        if(hnand->MspInitCallback == NULL) { hnand->MspInitCallback = HAL_NAND_MspInit; }
...
{
        #if (USE_HAL_NAND_REGISTER_CALLBACKS == 1)
    if(hnand->MspDeInitCallback == NULL) { hnand->MspDeInitCallback = HAL_NAND_MspDeInit; }
...
HAL_StatusTypeDef HAL_NAND_Read_ID(NAND_HandleTypeDef* hnand, NAND_IDTypeDef* pNAND_ID)
{
    ...
    else if(hnand->State == HAL_NAND_STATE_READY)
    {
        ...
        if(hnand->Init.NandBank == FMC_NAND_BANK2) { deviceaddress = NAND_DEVICE1; }
        else { deviceaddress = NAND_DEVICE2; }
        #else
...
uint32_t HAL_NAND_Address_Inc(const NAND_HandleTypeDef* hnand, NAND_AddressTypeDef* pAddress)
{
    ...
    if(pAddress->Page == hnand->Config.BlockSize)
    {
        ...
        if(pAddress->Block == hnand->Config.PlaneSize)
        {
            ...
            if(pAddress->Plane == (hnand->Config.PlaneNbr)) { status = NAND_INVALID_ADDRESS; }
        }
    }
    ...
    HAL_NAND_RegisterCallback(NAND_HandleTypeDef* hnand, HAL_NAND_CallbackIDTypeDef CallbackId, pNAND_CallbackTypeDef pCallback)
{
    ...
    if(hnand->State == HAL_NAND_STATE_READY)
    {
        switch(CallbackId)
        {
            case HAL_NAND_MSP_INIT_CB_ID: hnand->MspInitCallback = pCallback; break;
            case HAL_NAND_MSP_DEINIT_CB_ID: hnand->MspDeInitCallback = pCallback; break;
...
HAL_StatusTypeDef HAL_NAND_UnRegisterCallback(NAND_HandleTypeDef* hnand, HAL_NAND_CallbackIDTypeDef CallbackId)
{
    ...
    if(hnand->State == HAL_NAND_STATE_READY)
    {
        switch(CallbackId)
        {
            case HAL_NAND_MSP_INIT_CB_ID: hnand->MspInitCallback = HAL_NAND_MspInit; break;
            case HAL_NAND_MSP_DEINIT_CB_ID: hnand->MspDeInitCallback = HAL_NAND_MspDeInit; break;
...
uint32_t HAL_NAND_Read_Status(const NAND_HandleTypeDef* hnand)
{
    ...
    *(__IO uint8_t*)((uint32_t)(deviceaddress | CMD_AREA)) = NAND_CMD_STATUS;
    ...
    /* Return the status */
    if((data & NAND_ERROR) == NAND_ERROR) { return NAND_ERROR; }
    else if((data & NAND_READY) == NAND_READY) { return NAND_READY; }
    else { return NAND_BUSY; }
}
