...
HAL_StatusTypeDef HAL_DFSDM_ChannelInit(DFSDM_Channel_HandleTypeDef* hdfsdm_channel)
{
    ...
    if(hdfsdm_channel == NULL) { return HAL_ERROR; }
    ...
    if(channelHandleTable[DFSDM_GetChannelFromInstance(hdfsdm_channel->Instance)] != NULL) { return HAL_ERROR; }
    ...
    if(hdfsdm_channel->MspInitCallback == NULL) { hdfsdm_channel->MspInitCallback = HAL_DFSDM_ChannelMspInit; }
    ...
    hdfsdm_channel->Instance->CHCFGR1 |=
        (hdfsdm_channel->Init.Input.Multiplexer | hdfsdm_channel->Init.Input.DataPacking | hdfsdm_channel->Init.Input.Pins);
    ...
    hdfsdm_channel->Instance->CHAWSCDR |=
        (hdfsdm_channel->Init.Awd.FilterOrder | ((hdfsdm_channel->Init.Awd.Oversampling - 1U) << DFSDM_CHAWSCDR_AWFOSR_Pos));
    ...
    channelHandleTable[DFSDM_GetChannelFromInstance(hdfsdm_channel->Instance)] = hdfsdm_channel;
        ...
        #else
    /* Check that channel has not been already initialized */
    if(a_dfsdm1ChannelHandle[DFSDM_GetChannelFromInstance(hdfsdm_channel->Instance)] != NULL) { return HAL_ERROR; }
...
HAL_StatusTypeDef HAL_DFSDM_ChannelDeInit(DFSDM_Channel_HandleTypeDef* hdfsdm_channel)
{
    ...
    if(*channelCounterPtr == 0U) { channel0Instance->CHCFGR1 &= ~(DFSDM_CHCFGR1_DFSDMEN); }

            /* Call MSP deinit function */
            #if (USE_HAL_DFSDM_REGISTER_CALLBACKS == 1)
    if(hdfsdm_channel->MspDeInitCallback == NULL) { hdfsdm_channel->MspDeInitCallback = HAL_DFSDM_ChannelMspDeInit; }
    ...
    /* Disable global DFSDM at deinit of last channel */
    if(v_dfsdm1ChannelCounter == 0U) { DFSDM1_Channel0->CHCFGR1 &= ~(DFSDM_CHCFGR1_DFSDMEN); }
                                                     ...
                                                     pDFSDM_Channel_CallbackTypeDef pCallback)
{
    ...
    else
    {
        if(HAL_DFSDM_CHANNEL_STATE_READY == hdfsdm_channel->State)
        {
            switch(CallbackID)
            {
                case HAL_DFSDM_CHANNEL_CKAB_CB_ID: hdfsdm_channel->CkabCallback = pCallback; break;
                case HAL_DFSDM_CHANNEL_SCD_CB_ID: hdfsdm_channel->ScdCallback = pCallback; break;
                case HAL_DFSDM_CHANNEL_MSPINIT_CB_ID: hdfsdm_channel->MspInitCallback = pCallback; break;
                case HAL_DFSDM_CHANNEL_MSPDEINIT_CB_ID: hdfsdm_channel->MspDeInitCallback = pCallback; break;
                default:
                    /* update return status */
                    status = HAL_ERROR;
                    break;
                                                       ...
                                                       HAL_DFSDM_Channel_CallbackIDTypeDef CallbackID)
{
    ...
    if(HAL_DFSDM_CHANNEL_STATE_READY == hdfsdm_channel->State)
    {
        switch(CallbackID)
        {
            case HAL_DFSDM_CHANNEL_CKAB_CB_ID: hdfsdm_channel->CkabCallback = HAL_DFSDM_ChannelCkabCallback; break;
            case HAL_DFSDM_CHANNEL_SCD_CB_ID: hdfsdm_channel->ScdCallback = HAL_DFSDM_ChannelScdCallback; break;
            case HAL_DFSDM_CHANNEL_MSPINIT_CB_ID: hdfsdm_channel->MspInitCallback = HAL_DFSDM_ChannelMspInit; break;
            case HAL_DFSDM_CHANNEL_MSPDEINIT_CB_ID: hdfsdm_channel->MspDeInitCallback = HAL_DFSDM_ChannelMspDeInit; break;
            default:
                /* update return status */
                status = HAL_ERROR;
                break;
...
HAL_StatusTypeDef HAL_DFSDM_ChannelCkabStart(DFSDM_Channel_HandleTypeDef* hdfsdm_channel)
{
    ...
    else
    {
        #if defined(DFSDM2_Channel0)
        /* Get channel counter, channel handle table and channel 0 instance */
        if(IS_DFSDM1_CHANNEL_INSTANCE(hdfsdm_channel->Instance)) { filter0Instance = DFSDM1_Filter0; }
        else { filter0Instance = DFSDM2_Filter0; }
...
{
    /* Check DFSDM Channel handle */
    if(hdfsdm_filter == NULL) { return HAL_ERROR; }
    ...
    if((hdfsdm_filter->Instance == DFSDM1_Filter0) && ((hdfsdm_filter->Init.RegularParam.Trigger == DFSDM_FILTER_SYNC_TRIGGER) ||
                                                       (hdfsdm_filter->Init.InjectedParam.Trigger == DFSDM_FILTER_SYNC_TRIGGER)))
    {
        return HAL_ERROR;
    }
        #if defined(DFSDM2_Channel0)
    if((hdfsdm_filter->Instance == DFSDM2_Filter0) && ((hdfsdm_filter->Init.RegularParam.Trigger == DFSDM_FILTER_SYNC_TRIGGER) ||
                                                       (hdfsdm_filter->Init.InjectedParam.Trigger == DFSDM_FILTER_SYNC_TRIGGER)))
    {
        return HAL_ERROR;
    }
    ...
    /* Call MSP init function */
    if(hdfsdm_filter->MspInitCallback == NULL) { hdfsdm_filter->MspInitCallback = HAL_DFSDM_FilterMspInit; }
    ...
    if(hdfsdm_filter->Init.RegularParam.FastMode == ENABLE) { hdfsdm_filter->Instance->FLTCR1 |= DFSDM_FLTCR1_FAST; }
    else { hdfsdm_filter->Instance->FLTCR1 &= ~(DFSDM_FLTCR1_FAST); }

    if(hdfsdm_filter->Init.RegularParam.DmaMode == ENABLE) { hdfsdm_filter->Instance->FLTCR1 |= DFSDM_FLTCR1_RDMAEN; }
    else { hdfsdm_filter->Instance->FLTCR1 &= ~(DFSDM_FLTCR1_RDMAEN); }
    ...
    if(hdfsdm_filter->Init.InjectedParam.ScanMode == ENABLE) { hdfsdm_filter->Instance->FLTCR1 |= DFSDM_FLTCR1_JSCAN; }
    else { hdfsdm_filter->Instance->FLTCR1 &= ~(DFSDM_FLTCR1_JSCAN); }

    if(hdfsdm_filter->Init.InjectedParam.DmaMode == ENABLE) { hdfsdm_filter->Instance->FLTCR1 |= DFSDM_FLTCR1_JDMAEN; }
    else { hdfsdm_filter->Instance->FLTCR1 &= ~(DFSDM_FLTCR1_JDMAEN); }
...
HAL_StatusTypeDef HAL_DFSDM_FilterDeInit(DFSDM_Filter_HandleTypeDef* hdfsdm_filter)
{
    ...
    hdfsdm_filter->Instance->FLTCR1 &= ~(DFSDM_FLTCR1_DFEN);
        ...
        /* Call MSP deinit function */
        #if (USE_HAL_DFSDM_REGISTER_CALLBACKS == 1)
    if(hdfsdm_filter->MspDeInitCallback == NULL) { hdfsdm_filter->MspDeInitCallback = HAL_DFSDM_FilterMspDeInit; }
                                                    ...
                                                    pDFSDM_Filter_CallbackTypeDef pCallback)
{
    ...
    else
    {
        if(HAL_DFSDM_FILTER_STATE_READY == hdfsdm_filter->State)
        {
            switch(CallbackID)
            {
                case HAL_DFSDM_FILTER_REGCONV_COMPLETE_CB_ID: hdfsdm_filter->RegConvCpltCallback = pCallback; break;
                case HAL_DFSDM_FILTER_REGCONV_HALFCOMPLETE_CB_ID: hdfsdm_filter->RegConvHalfCpltCallback = pCallback; break;
                case HAL_DFSDM_FILTER_INJCONV_COMPLETE_CB_ID: hdfsdm_filter->InjConvCpltCallback = pCallback; break;
                case HAL_DFSDM_FILTER_INJCONV_HALFCOMPLETE_CB_ID: hdfsdm_filter->InjConvHalfCpltCallback = pCallback; break;
                case HAL_DFSDM_FILTER_ERROR_CB_ID: hdfsdm_filter->ErrorCallback = pCallback; break;
                case HAL_DFSDM_FILTER_MSPINIT_CB_ID: hdfsdm_filter->MspInitCallback = pCallback; break;
                case HAL_DFSDM_FILTER_MSPDEINIT_CB_ID: hdfsdm_filter->MspDeInitCallback = pCallback; break;
                default:
                    /* update the error code */
                    hdfsdm_filter->ErrorCode = DFSDM_FILTER_ERROR_INVALID_CALLBACK;
                    /* update return status */
    ...
    if(HAL_DFSDM_FILTER_STATE_READY == hdfsdm_filter->State)
    {
        switch(CallbackID)
        {
            ...
            case HAL_DFSDM_FILTER_INJCONV_HALFCOMPLETE_CB_ID:
                hdfsdm_filter->InjConvHalfCpltCallback = HAL_DFSDM_FilterInjConvHalfCpltCallback;
                break;
            case HAL_DFSDM_FILTER_ERROR_CB_ID: hdfsdm_filter->ErrorCallback = HAL_DFSDM_FilterErrorCallback; break;
            case HAL_DFSDM_FILTER_MSPINIT_CB_ID: hdfsdm_filter->MspInitCallback = HAL_DFSDM_FilterMspInit; break;
            case HAL_DFSDM_FILTER_MSPDEINIT_CB_ID: hdfsdm_filter->MspDeInitCallback = HAL_DFSDM_FilterMspDeInit; break;
            default:
                /* update the error code */
                hdfsdm_filter->ErrorCode = DFSDM_FILTER_ERROR_INVALID_CALLBACK;
                /* update return status */
                                                       ...
                                                       pDFSDM_Filter_AwdCallbackTypeDef pCallback)
{
    ...
    {
        if(HAL_DFSDM_FILTER_STATE_READY == hdfsdm_filter->State) { hdfsdm_filter->AwdCallback = pCallback; }
        else
        {
            /* update the error code */
            hdfsdm_filter->ErrorCode = DFSDM_FILTER_ERROR_INVALID_CALLBACK;
...
HAL_StatusTypeDef HAL_DFSDM_Filter_UnRegisterAwdCallback(DFSDM_Filter_HandleTypeDef* hdfsdm_filter)
{
    ...
    if(HAL_DFSDM_FILTER_STATE_READY == hdfsdm_filter->State) { hdfsdm_filter->AwdCallback = HAL_DFSDM_FilterAwdCallback; }
    else
    {
        /* update the error code */
        hdfsdm_filter->ErrorCode = DFSDM_FILTER_ERROR_INVALID_CALLBACK;
    ...
    HAL_DFSDM_FilterConfigRegChannel(DFSDM_Filter_HandleTypeDef* hdfsdm_filter, uint32_t Channel, uint32_t ContinuousMode)
{
    ...
    if((hdfsdm_filter->State != HAL_DFSDM_FILTER_STATE_RESET) && (hdfsdm_filter->State != HAL_DFSDM_FILTER_STATE_ERROR))
    {
        ...
        else { hdfsdm_filter->Instance->FLTCR1 |= (uint32_t)((Channel & DFSDM_MSB_MASK) << DFSDM_FLTCR1_MSB_RCH_OFFSET); }
...
HAL_StatusTypeDef HAL_DFSDM_FilterRegularStart_DMA(DFSDM_Filter_HandleTypeDef* hdfsdm_filter, int32_t* pData, uint32_t Length)
{
    ...
    if((pData == NULL) || (Length == 0U)) { status = HAL_ERROR; }
    /* Check that DMA is enabled for regular conversion */
    else if((hdfsdm_filter->Instance->FLTCR1 & DFSDM_FLTCR1_RDMAEN) != DFSDM_FLTCR1_RDMAEN) { status = HAL_ERROR; }
    /* Check parameters compatibility */
    else if((hdfsdm_filter->RegularTrigger == DFSDM_FILTER_SW_TRIGGER) &&
            (hdfsdm_filter->RegularContMode == DFSDM_CONTINUOUS_CONV_OFF) && (hdfsdm_filter->hdmaReg->Init.Mode == DMA_NORMAL) &&
            (Length != 1U))
    {
        status = HAL_ERROR;
    }
    else if((hdfsdm_filter->RegularTrigger == DFSDM_FILTER_SW_TRIGGER) &&
            (hdfsdm_filter->RegularContMode == DFSDM_CONTINUOUS_CONV_OFF) && (hdfsdm_filter->hdmaReg->Init.Mode == DMA_CIRCULAR))
    {
        status = HAL_ERROR;
    }
    /* Check DFSDM filter state */
    else if((hdfsdm_filter->State == HAL_DFSDM_FILTER_STATE_READY) || (hdfsdm_filter->State == HAL_DFSDM_FILTER_STATE_INJ))
    {
        ...
        if(HAL_DMA_Start_IT(hdfsdm_filter->hdmaReg, (uint32_t)&hdfsdm_filter->Instance->FLTRDATAR, (uint32_t)pData, Length) !=
           HAL_OK)
        {
...
HAL_StatusTypeDef HAL_DFSDM_FilterInjectedStart_DMA(DFSDM_Filter_HandleTypeDef* hdfsdm_filter, int32_t* pData, uint32_t Length)
{
    ...
    else if((hdfsdm_filter->Instance->FLTCR1 & DFSDM_FLTCR1_JDMAEN) != DFSDM_FLTCR1_JDMAEN) { status = HAL_ERROR; }
    /* Check parameters compatibility */
    else if((hdfsdm_filter->InjectedTrigger == DFSDM_FILTER_SW_TRIGGER) && (hdfsdm_filter->hdmaInj->Init.Mode == DMA_NORMAL) &&
    ...
    else if((hdfsdm_filter->State == HAL_DFSDM_FILTER_STATE_READY) || (hdfsdm_filter->State == HAL_DFSDM_FILTER_STATE_REG))
    {
        ...
        /* Start DMA in interrupt mode */
        if(HAL_DMA_Start_IT(hdfsdm_filter->hdmaInj, (uint32_t)&hdfsdm_filter->Instance->FLTJDATAR, (uint32_t)pData, Length) !=
           HAL_OK)
        {
            /* Set DFSDM filter in error state */
            hdfsdm_filter->State = HAL_DFSDM_FILTER_STATE_ERROR;
...
void HAL_DFSDM_IRQHandler(DFSDM_Filter_HandleTypeDef* hdfsdm_filter)
{
            ...
            ((hdfsdm_filter->Instance->FLTCR2 & DFSDM_FLTCR2_AWDIE) != 0U))
    {
        ...
        if(threshold == DFSDM_AWD_HIGH_THRESHOLD) { reg = reg >> DFSDM_FLTAWSR_AWHTF_Pos; }
        while((reg & 1U) == 0U)
        {
            channel++;
            reg = reg >> 1U;
        ...
        hdfsdm_filter->Instance->FLTAWCFR =
            (threshold == DFSDM_AWD_HIGH_THRESHOLD) ? (1U << (DFSDM_FLTAWSR_AWHTF_Pos + channel)) : (1U << channel);
    ...
    else if((hdfsdm_filter->Instance == DFSDM1_Filter0) && ((hdfsdm_filter->Instance->FLTISR & DFSDM_FLTISR_CKABF) != 0U) &&
            ((hdfsdm_filter->Instance->FLTCR2 & DFSDM_FLTCR2_CKABIE) != 0U))
    {
        ...
        #if defined(DFSDM2_Channel0)
    /* Check if clock absence occurs */
    else if((hdfsdm_filter->Instance == DFSDM2_Filter0) && ((hdfsdm_filter->Instance->FLTISR & DFSDM_FLTISR_CKABF) != 0U) &&
            ((hdfsdm_filter->Instance->FLTCR2 & DFSDM_FLTCR2_CKABIE) != 0U))
    {
        uint32_t reg = 0U;
        uint32_t channel = 0U;
    ...
    else if((hdfsdm_filter->Instance == DFSDM1_Filter0) && ((hdfsdm_filter->Instance->FLTISR & DFSDM_FLTISR_SCDF) != 0U) &&
            ((hdfsdm_filter->Instance->FLTCR2 & DFSDM_FLTCR2_SCDIE) != 0U))
    {
        ...
        #if defined(DFSDM2_Channel0)
    /* Check if short circuit detection occurs */
    else if((hdfsdm_filter->Instance == DFSDM2_Filter0) && ((hdfsdm_filter->Instance->FLTISR & DFSDM_FLTISR_SCDF) != 0U) &&
            ((hdfsdm_filter->Instance->FLTCR2 & DFSDM_FLTCR2_SCDIE) != 0U))
    {
        uint32_t reg = 0U;
        uint32_t channel = 0U;
...
void HAL_DFSDM_DisableDelayClock(uint32_t MCHDLY)
{
    ...
    tmp = SYSCFG->MCHDLYCR;
    if(MCHDLY == HAL_MCHDLY_CLOCK_DFSDM2) { tmp = tmp & (~SYSCFG_MCHDLYCR_MCHDLY2EN); }
    else { tmp = tmp & (~SYSCFG_MCHDLYCR_MCHDLY1EN); }
...
void HAL_DFSDM_ClockIn_SourceSelection(uint32_t source)
{
    ...
    if((source == HAL_DFSDM2_CKIN_PAD) || (source == HAL_DFSDM2_CKIN_DM))
    {
        ...
        if(source == HAL_DFSDM2_CKIN_PAD) { source = 0x000000U; }
    }
    else { tmp = (tmp & ~SYSCFG_MCHDLYCR_DFSDM1CFG); }
...
void HAL_DFSDM_ClockOut_SourceSelection(uint32_t source)
{
    ...
    if((source == HAL_DFSDM2_CKOUT_DFSDM2) || (source == HAL_DFSDM2_CKOUT_M27))
    {
        ...
        if(source == HAL_DFSDM2_CKOUT_DFSDM2) { source = 0x000U; }
    }
    else { tmp = (tmp & ~SYSCFG_MCHDLYCR_DFSDM1CKOSEL); }
...
void HAL_DFSDM_DataIn0_SourceSelection(uint32_t source)
{
    ...
    if((source == HAL_DATAIN0_DFSDM2_PAD) || (source == HAL_DATAIN0_DFSDM2_DATAIN1))
    {
        ...
        if(source == HAL_DATAIN0_DFSDM2_PAD) { source = 0x00000U; }
    }
    else { tmp = (tmp & ~SYSCFG_MCHDLYCR_DFSDM1D0SEL); }
    SYSCFG->MCHDLYCR = (source | tmp);
}
...
void HAL_DFSDM_DataIn2_SourceSelection(uint32_t source)
{
    ...
    if((source == HAL_DATAIN2_DFSDM2_PAD) || (source == HAL_DATAIN2_DFSDM2_DATAIN3))
    {
        ...
        if(source == HAL_DATAIN2_DFSDM2_PAD) { source = 0x0000U; }
    }
    else { tmp = (tmp & ~SYSCFG_MCHDLYCR_DFSDM1D2SEL); }
    SYSCFG->MCHDLYCR = (source | tmp);
}
...
void HAL_DFSDM_BitStreamClkDistribution_Config(uint32_t source)
{
    ...
    else { tmp = (tmp & ~SYSCFG_MCHDLYCR_DFSDM2CK37SEL); }

    if((source == HAL_DFSDM1_CLKIN0_TIM4OC2) || (source == HAL_DFSDM1_CLKIN1_TIM4OC1) || (source == HAL_DFSDM2_CLKIN0_TIM3OC4) ||
       (source == HAL_DFSDM2_CLKIN1_TIM3OC3) || (source == HAL_DFSDM2_CLKIN2_TIM3OC2) || (source == HAL_DFSDM2_CLKIN3_TIM3OC1))
    {
        source = 0x0000U;
    }
...
void HAL_DFSDM_ConfigMultiChannelDelay(DFSDM_MultiChannelConfigTypeDef* mchdlystruct)
{
    ...
    SYSCFG->MCHDLYCR =
        (mchdlyreg | (mchdlystruct->DFSDM1ClockOut) | (mchdlystruct->DFSDM2ClockOut) | (mchdlystruct->DFSDM1ClockIn) |
         (mchdlystruct->DFSDM2ClockIn) | (mchdlystruct->DFSDM1BitClkDistribution) | (mchdlystruct->DFSDM2BitClkDistribution) |
...
static uint32_t DFSDM_GetChannelFromInstance(const DFSDM_Channel_TypeDef* Instance)
{
    uint32_t channel;
        ...
        #if defined(DFSDM2_Channel0)
    if((Instance == DFSDM1_Channel0) || (Instance == DFSDM2_Channel0)) { channel = 0U; }
    else if((Instance == DFSDM1_Channel1) || (Instance == DFSDM2_Channel1)) { channel = 1U; }
    else if((Instance == DFSDM1_Channel2) || (Instance == DFSDM2_Channel2)) { channel = 2U; }
    else if((Instance == DFSDM1_Channel3) || (Instance == DFSDM2_Channel3)) { channel = 3U; }
    else if(Instance == DFSDM2_Channel4) { channel = 4U; }
    else if(Instance == DFSDM2_Channel5) { channel = 5U; }
    else if(Instance == DFSDM2_Channel6) { channel = 6U; }
    else /* DFSDM2_Channel7 */ { channel = 7U; }

        #else
    if(Instance == DFSDM1_Channel0) { channel = 0U; }
    else if(Instance == DFSDM1_Channel1) { channel = 1U; }
    else if(Instance == DFSDM1_Channel2) { channel = 2U; }
    else /* DFSDM1_Channel3 */ { channel = 3U; }
        #endif /* defined(DFSDM2_Channel0) */
...
static void DFSDM_RegConvStart(DFSDM_Filter_HandleTypeDef* hdfsdm_filter)
{
    ...
    /* Update DFSDM filter state */
    hdfsdm_filter->State =
        (hdfsdm_filter->State == HAL_DFSDM_FILTER_STATE_READY) ? HAL_DFSDM_FILTER_STATE_REG : HAL_DFSDM_FILTER_STATE_REG_INJ;
}
...
static void DFSDM_RegConvStop(DFSDM_Filter_HandleTypeDef* hdfsdm_filter)
{
    ...
    if(hdfsdm_filter->RegularTrigger == DFSDM_FILTER_SYNC_TRIGGER) { hdfsdm_filter->Instance->FLTCR1 &= ~(DFSDM_FLTCR1_RSYNC); }
...
static void DFSDM_InjConvStart(DFSDM_Filter_HandleTypeDef* hdfsdm_filter)
{
    ...
    /* Update DFSDM filter state */
    hdfsdm_filter->State =
        (hdfsdm_filter->State == HAL_DFSDM_FILTER_STATE_READY) ? HAL_DFSDM_FILTER_STATE_INJ : HAL_DFSDM_FILTER_STATE_REG_INJ;
}
...
static void DFSDM_InjConvStop(DFSDM_Filter_HandleTypeDef* hdfsdm_filter)
{
    ...
    if(hdfsdm_filter->InjectedTrigger == DFSDM_FILTER_SYNC_TRIGGER) { hdfsdm_filter->Instance->FLTCR1 &= ~(DFSDM_FLTCR1_JSYNC); }
