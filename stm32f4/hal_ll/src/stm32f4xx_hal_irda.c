...
HAL_StatusTypeDef HAL_IRDA_Init(IRDA_HandleTypeDef* hirda)
{
    ...
    if(hirda == NULL) { return HAL_ERROR; }
    ...
    if(hirda->gState == HAL_IRDA_STATE_RESET)
    {
        ...
        if(hirda->MspInitCallback == NULL) { hirda->MspInitCallback = HAL_IRDA_MspInit; }
...
HAL_StatusTypeDef HAL_IRDA_DeInit(IRDA_HandleTypeDef* hirda)
{
    ...
    #if USE_HAL_IRDA_REGISTER_CALLBACKS == 1
    if(hirda->MspDeInitCallback == NULL) { hirda->MspDeInitCallback = HAL_IRDA_MspDeInit; }
    /* DeInit the low level hardware */
    ...
    HAL_IRDA_RegisterCallback(IRDA_HandleTypeDef* hirda, HAL_IRDA_CallbackIDTypeDef CallbackID, pIRDA_CallbackTypeDef pCallback)
{
    ...
    if(hirda->gState == HAL_IRDA_STATE_READY)
    {
        switch(CallbackID)
        {
            case HAL_IRDA_TX_HALFCOMPLETE_CB_ID: hirda->TxHalfCpltCallback = pCallback; break;

            case HAL_IRDA_TX_COMPLETE_CB_ID: hirda->TxCpltCallback = pCallback; break;

            case HAL_IRDA_RX_HALFCOMPLETE_CB_ID: hirda->RxHalfCpltCallback = pCallback; break;

            case HAL_IRDA_RX_COMPLETE_CB_ID: hirda->RxCpltCallback = pCallback; break;

            case HAL_IRDA_ERROR_CB_ID: hirda->ErrorCallback = pCallback; break;

            case HAL_IRDA_ABORT_COMPLETE_CB_ID: hirda->AbortCpltCallback = pCallback; break;

            case HAL_IRDA_ABORT_TRANSMIT_COMPLETE_CB_ID: hirda->AbortTransmitCpltCallback = pCallback; break;

            case HAL_IRDA_ABORT_RECEIVE_COMPLETE_CB_ID: hirda->AbortReceiveCpltCallback = pCallback; break;

            case HAL_IRDA_MSPINIT_CB_ID: hirda->MspInitCallback = pCallback; break;

            case HAL_IRDA_MSPDEINIT_CB_ID: hirda->MspDeInitCallback = pCallback; break;
...
HAL_StatusTypeDef HAL_IRDA_UnRegisterCallback(IRDA_HandleTypeDef* hirda, HAL_IRDA_CallbackIDTypeDef CallbackID)
{
    ...
    else if(HAL_IRDA_STATE_RESET == hirda->gState)
    {
        switch(CallbackID)
        {
            case HAL_IRDA_MSPINIT_CB_ID: hirda->MspInitCallback = HAL_IRDA_MspInit; break;

            case HAL_IRDA_MSPDEINIT_CB_ID: hirda->MspDeInitCallback = HAL_IRDA_MspDeInit; break;
...
HAL_StatusTypeDef HAL_IRDA_Transmit(IRDA_HandleTypeDef* hirda, const uint8_t* pData, uint16_t Size, uint32_t Timeout)
{
    ...
    if(hirda->gState == HAL_IRDA_STATE_READY)
    {
        ...
        while(hirda->TxXferCount > 0U)
        {
            ...
            {
                if(IRDA_WaitOnFlagUntilTimeout(hirda, IRDA_FLAG_TXE, RESET, tickstart, Timeout) != HAL_OK) { return HAL_TIMEOUT; }
                tmp = (const uint16_t*)pData;
                hirda->Instance->DR = (*tmp & (uint16_t)0x01FF);
                if(hirda->Init.Parity == IRDA_PARITY_NONE) { pData += 2U; }
                else { pData += 1U; }
            }
            else
            {
        ...
        if(IRDA_WaitOnFlagUntilTimeout(hirda, IRDA_FLAG_TC, RESET, tickstart, Timeout) != HAL_OK) { return HAL_TIMEOUT; }
    ...
    if(hirda->RxState == HAL_IRDA_STATE_READY)
    {
        ...
        while(hirda->RxXferCount > 0U)
        {
            ...
            else
            {
                ...
                if(hirda->Init.Parity == IRDA_PARITY_NONE) { *pData++ = (uint8_t)(hirda->Instance->DR & (uint8_t)0x00FF); }
                else { *pData++ = (uint8_t)(hirda->Instance->DR & (uint8_t)0x007F); }
            }
        }
...
HAL_StatusTypeDef HAL_IRDA_DMAResume(IRDA_HandleTypeDef* hirda)
{
    ...
    if(hirda->RxState == HAL_IRDA_STATE_BUSY_RX)
    {
        ...
        if(hirda->Init.Parity != IRDA_PARITY_NONE) { SET_BIT(hirda->Instance->CR1, USART_CR1_PEIE); }
        SET_BIT(hirda->Instance->CR3, USART_CR3_EIE);
...
HAL_StatusTypeDef HAL_IRDA_DMAStop(IRDA_HandleTypeDef* hirda)
{
    ...
    */
    ...
    if((hirda->gState == HAL_IRDA_STATE_BUSY_TX) && dmarequest)
    {
        ...
        if(hirda->hdmatx != NULL) { HAL_DMA_Abort(hirda->hdmatx); }
    ...
    if((hirda->RxState == HAL_IRDA_STATE_BUSY_RX) && dmarequest)
    {
        ...
        if(hirda->hdmarx != NULL) { HAL_DMA_Abort(hirda->hdmarx); }
...
HAL_StatusTypeDef HAL_IRDA_Abort_IT(IRDA_HandleTypeDef* hirda)
{
    ...
    if(hirda->hdmatx != NULL)
    {
        ...
        if(HAL_IS_BIT_SET(hirda->Instance->CR3, USART_CR3_DMAT)) { hirda->hdmatx->XferAbortCallback = IRDA_DMATxAbortCallback; }
        else { hirda->hdmatx->XferAbortCallback = NULL; }
    }
    ...
    if(hirda->hdmarx != NULL)
    {
        /* Set DMA Abort Complete callback if IRDA DMA Rx request if enabled.
           Otherwise, set it to NULL */
        if(HAL_IS_BIT_SET(hirda->Instance->CR3, USART_CR3_DMAR)) { hirda->hdmarx->XferAbortCallback = IRDA_DMARxAbortCallback; }
        else { hirda->hdmarx->XferAbortCallback = NULL; }
    }
    ...
    if(HAL_IS_BIT_SET(hirda->Instance->CR3, USART_CR3_DMAT))
    {
        ...
        if(hirda->hdmatx != NULL)
        {
            ...
            /* Abort DMA TX */
            if(HAL_DMA_Abort_IT(hirda->hdmatx) != HAL_OK) { hirda->hdmatx->XferAbortCallback = NULL; }
            else { AbortCplt = 0x00U; }
        }
    }
...
void HAL_IRDA_IRQHandler(IRDA_HandleTypeDef* hirda)
{
       ...
       (((cr3its & USART_CR3_EIE) != RESET) || ((cr1its & (USART_CR1_RXNEIE | USART_CR1_PEIE)) != RESET)))
    {
        ...
        if(((isrflags & USART_SR_PE) != RESET) && ((cr1its & USART_CR1_PEIE) != RESET)) { hirda->ErrorCode |= HAL_IRDA_ERROR_PE; }

        /* IRDA noise error interrupt occurred --------------------------------*/
        if(((isrflags & USART_SR_NE) != RESET) && ((cr3its & USART_CR3_EIE) != RESET)) { hirda->ErrorCode |= HAL_IRDA_ERROR_NE; }

        /* IRDA frame error interrupt occurred --------------------------------*/
        if(((isrflags & USART_SR_FE) != RESET) && ((cr3its & USART_CR3_EIE) != RESET)) { hirda->ErrorCode |= HAL_IRDA_ERROR_FE; }
        ...
        {
            /* IRDA in mode Receiver ---------------------------------------------*/
            if(((isrflags & USART_SR_RXNE) != RESET) && ((cr1its & USART_CR1_RXNEIE) != RESET)) { IRDA_Receive_IT(hirda); }
...
static void IRDA_DMATxAbortCallback(DMA_HandleTypeDef* hdma)
{
    ...
    if(hirda->hdmarx != NULL)
    {
        if(hirda->hdmarx->XferAbortCallback != NULL) { return; }
    }
...
static void IRDA_DMARxAbortCallback(DMA_HandleTypeDef* hdma)
{
    ...
    if(hirda->hdmatx != NULL)
    {
        if(hirda->hdmatx->XferAbortCallback != NULL) { return; }
    }
...
static HAL_StatusTypeDef IRDA_Transmit_IT(IRDA_HandleTypeDef* hirda)
{
    ...
    if(hirda->gState == HAL_IRDA_STATE_BUSY_TX)
    {
        if(hirda->Init.WordLength == IRDA_WORDLENGTH_9B)
        {
            ...
            if(hirda->Init.Parity == IRDA_PARITY_NONE) { hirda->pTxBuffPtr += 2U; }
            else { hirda->pTxBuffPtr += 1U; }
        }
        else { hirda->Instance->DR = (uint8_t)(*hirda->pTxBuffPtr++ & (uint8_t)0x00FF); }
...
static HAL_StatusTypeDef IRDA_Receive_IT(IRDA_HandleTypeDef* hirda)
{
    ...
    if(hirda->RxState == HAL_IRDA_STATE_BUSY_RX)
    {
        ...
        {
            if(hirda->Init.Parity == IRDA_PARITY_NONE) { *hirda->pRxBuffPtr++ = (uint8_t)(uhdata & (uint8_t)0x00FF); }
            else { *hirda->pRxBuffPtr++ = (uint8_t)(uhdata & (uint8_t)0x007F); }
        }
