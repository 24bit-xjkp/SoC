...
HAL_StatusTypeDef HAL_CEC_Init(CEC_HandleTypeDef* hcec)
{
    ...
    if(hcec->gState == HAL_CEC_STATE_RESET)
    {
        ...
        if(hcec->MspInitCallback == NULL) { hcec->MspInitCallback = HAL_CEC_MspInit; /* Legacy weak MspInit  */ }
    ...
    hcec->Instance->CFGR = hcec->Init.SignalFreeTime | hcec->Init.Tolerance | hcec->Init.BRERxStop | hcec->Init.BREErrorBitGen |
                           hcec->Init.LBPEErrorBitGen | hcec->Init.BroadcastMsgNoErrorBitGen | hcec->Init.SignalFreeTimeOption |
...
HAL_StatusTypeDef HAL_CEC_DeInit(CEC_HandleTypeDef* hcec)
{
    ...
    hcec->gState = HAL_CEC_STATE_BUSY;
        ...
        #if (USE_HAL_CEC_REGISTER_CALLBACKS == 1)
    if(hcec->MspDeInitCallback == NULL) { hcec->MspDeInitCallback = HAL_CEC_MspDeInit; /* Legacy weak MspDeInit  */ }
...
HAL_StatusTypeDef HAL_CEC_SetDeviceAddress(CEC_HandleTypeDef* hcec, uint16_t CEC_OwnAddress)
{
    ...
    if((hcec->gState == HAL_CEC_STATE_READY) && (hcec->RxState == HAL_CEC_STATE_READY))
    {
        ...
        if(CEC_OwnAddress != CEC_OWN_ADDRESS_NONE) { hcec->Instance->CFGR |= ((uint32_t)CEC_OwnAddress << 16); }
        else { hcec->Instance->CFGR &= ~(CEC_CFGR_OAR); }
    ...
    HAL_CEC_RegisterCallback(CEC_HandleTypeDef* hcec, HAL_CEC_CallbackIDTypeDef CallbackID, pCEC_CallbackTypeDef pCallback)
{
    ...
    if(hcec->gState == HAL_CEC_STATE_READY)
    {
        switch(CallbackID)
        {
            case HAL_CEC_TX_CPLT_CB_ID: hcec->TxCpltCallback = pCallback; break;

            case HAL_CEC_ERROR_CB_ID: hcec->ErrorCallback = pCallback; break;

            case HAL_CEC_MSPINIT_CB_ID: hcec->MspInitCallback = pCallback; break;

            case HAL_CEC_MSPDEINIT_CB_ID: hcec->MspDeInitCallback = pCallback; break;
...
HAL_StatusTypeDef HAL_CEC_UnRegisterCallback(CEC_HandleTypeDef* hcec, HAL_CEC_CallbackIDTypeDef CallbackID)
{
    ...
    if(hcec->gState == HAL_CEC_STATE_READY)
    {
        switch(CallbackID)
        {
            ...
            case HAL_CEC_MSPDEINIT_CB_ID: hcec->MspDeInitCallback = HAL_CEC_MspDeInit; break;
...
HAL_StatusTypeDef HAL_CEC_RegisterRxCpltCallback(CEC_HandleTypeDef* hcec, pCEC_RxCallbackTypeDef pCallback)
{
    ...
    if(HAL_CEC_STATE_READY == hcec->RxState) { hcec->RxCpltCallback = pCallback; }
    else
    {
        /* Update the error code */
        hcec->ErrorCode |= HAL_CEC_ERROR_INVALID_CALLBACK;
                                      ...
                                      uint32_t Size)
{
    ...
    if(hcec->gState == HAL_CEC_STATE_READY)
    {
        ...
        if(Size == 0U) { __HAL_CEC_LAST_BYTE_TX_SET(hcec); }
...
void HAL_CEC_IRQHandler(CEC_HandleTypeDef* hcec)
{
    ...
    if((itflag & (CEC_ISR_RXOVR | CEC_ISR_BRE | CEC_ISR_SBPE | CEC_ISR_LBPE | CEC_ISR_RXACKE | CEC_ISR_TXUDR | CEC_ISR_TXERR |
                  CEC_ISR_TXACKE)) != 0U)
    {
        hcec->ErrorCode = itflag;
        __HAL_CEC_CLEAR_FLAG(hcec,
                             HAL_CEC_ERROR_RXOVR | HAL_CEC_ERROR_BRE | CEC_FLAG_LBPE | CEC_FLAG_SBPE | HAL_CEC_ERROR_RXACKE |
                                 HAL_CEC_ERROR_TXUDR | HAL_CEC_ERROR_TXERR | HAL_CEC_ERROR_TXACKE);
