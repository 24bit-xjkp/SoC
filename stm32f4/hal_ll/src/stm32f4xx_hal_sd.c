...
HAL_StatusTypeDef HAL_SD_Init(SD_HandleTypeDef* hsd)
{
    ...
    if(HAL_SD_InitCard(hsd) != HAL_OK) { return HAL_ERROR; }
...
HAL_StatusTypeDef HAL_SD_DeInit(SD_HandleTypeDef* hsd)
{
    ...
    SD_PowerOFF(hsd);
        ...
        #if defined(USE_HAL_SD_REGISTER_CALLBACKS) && (USE_HAL_SD_REGISTER_CALLBACKS == 1U)
    if(hsd->MspDeInitCallback == NULL) { hsd->MspDeInitCallback = HAL_SD_MspDeInit; }
...
void HAL_SD_IRQHandler(SD_HandleTypeDef* hsd)
{
    ...
    if((__HAL_SD_GET_FLAG(hsd, SDIO_FLAG_RXFIFOHF) != RESET) && ((context & SD_CONTEXT_IT) != 0U)) { SD_Read_IT(hsd); }
    ...
    else if(__HAL_SD_GET_FLAG(hsd, SDIO_FLAG_DATAEND) != RESET)
    {
        ...
        __HAL_SD_DISABLE_IT(hsd,
                            SDIO_IT_DATAEND | SDIO_IT_DCRCFAIL | SDIO_IT_DTIMEOUT | SDIO_IT_TXUNDERR | SDIO_IT_RXOVERR |
                                SDIO_IT_TXFIFOHE | SDIO_IT_RXFIFOHF | SDIO_IT_STBITERR);
        #else  /* SDIO_STA_STBITERR not defined */
        __HAL_SD_DISABLE_IT(hsd,
                            SDIO_IT_DATAEND | SDIO_IT_DCRCFAIL | SDIO_IT_DTIMEOUT | SDIO_IT_TXUNDERR | SDIO_IT_RXOVERR |
    ...
    else if((__HAL_SD_GET_FLAG(hsd, SDIO_FLAG_TXFIFOHE) != RESET) && ((context & SD_CONTEXT_IT) != 0U)) { SD_Write_IT(hsd); }
    ...
    {
        /* Set Error code */
        if(__HAL_SD_GET_FLAG(hsd, SDIO_FLAG_DCRCFAIL) != RESET) { hsd->ErrorCode |= HAL_SD_ERROR_DATA_CRC_FAIL; }
        if(__HAL_SD_GET_FLAG(hsd, SDIO_FLAG_DTIMEOUT) != RESET) { hsd->ErrorCode |= HAL_SD_ERROR_DATA_TIMEOUT; }
        if(__HAL_SD_GET_FLAG(hsd, SDIO_FLAG_RXOVERR) != RESET) { hsd->ErrorCode |= HAL_SD_ERROR_RX_OVERRUN; }
        if(__HAL_SD_GET_FLAG(hsd, SDIO_FLAG_TXUNDERR) != RESET) { hsd->ErrorCode |= HAL_SD_ERROR_TX_UNDERRUN; }
        #if defined(SDIO_STA_STBITERR)
...
HAL_StatusTypeDef HAL_SD_ConfigWideBusOperation(SD_HandleTypeDef* hsd, uint32_t WideMode)
{
    ...
    if(hsd->SdCard.CardType != CARD_SECURED)
    {
        if(WideMode == SDIO_BUS_WIDE_8B) { hsd->ErrorCode |= HAL_SD_ERROR_UNSUPPORTED_FEATURE; }
...
HAL_SD_CardStateTypeDef HAL_SD_GetCardState(SD_HandleTypeDef* hsd)
{
    ...
    if(errorstate != HAL_SD_ERROR_NONE) { hsd->ErrorCode |= errorstate; }
...
HAL_StatusTypeDef HAL_SD_Abort(SD_HandleTypeDef* hsd)
{
    ...
    if((context & SD_CONTEXT_DMA) != 0U)
    {
        ...
        if(((context & SD_CONTEXT_WRITE_SINGLE_BLOCK) != 0U) || ((context & SD_CONTEXT_WRITE_MULTIPLE_BLOCK) != 0U))
        {
            if(HAL_DMA_Abort(hsd->hdmatx) != HAL_OK) { hsd->ErrorCode |= HAL_SD_ERROR_DMA; }
        }
        /* Abort the SD DMA Rx channel */
        else if(((context & SD_CONTEXT_READ_SINGLE_BLOCK) != 0U) || ((context & SD_CONTEXT_READ_MULTIPLE_BLOCK) != 0U))
        {
            if(HAL_DMA_Abort(hsd->hdmarx) != HAL_OK) { hsd->ErrorCode |= HAL_SD_ERROR_DMA; }
        }
        else { /* Nothing to do */ }
    }
    ...
    if(hsd->ErrorCode != HAL_SD_ERROR_NONE) { return HAL_ERROR; }
    return HAL_OK;
}
...
HAL_StatusTypeDef HAL_SD_Abort_IT(SD_HandleTypeDef* hsd)
{
    ...
    if((context & SD_CONTEXT_DMA) != 0U)
    {
        ...
        {
            hsd->hdmatx->XferAbortCallback = SD_DMATxAbort;
            if(HAL_DMA_Abort_IT(hsd->hdmatx) != HAL_OK) { hsd->hdmatx = NULL; }
        }
        /* Abort the SD DMA Rx channel */
        else if(((context & SD_CONTEXT_READ_SINGLE_BLOCK) != 0U) || ((context & SD_CONTEXT_READ_MULTIPLE_BLOCK) != 0U))
        {
            hsd->hdmarx->XferAbortCallback = SD_DMARxAbort;
            if(HAL_DMA_Abort_IT(hsd->hdmarx) != HAL_OK) { hsd->hdmarx = NULL; }
        }
        else { /* Nothing to do */ }
    }
...
static uint32_t SD_InitCard(SD_HandleTypeDef* hsd)
{
    ...
    /* Get CSD parameters */
    if(HAL_SD_GetCardCSD(hsd, &CSD) != HAL_OK) { return HAL_SD_ERROR_UNSUPPORTED_FEATURE; }
...
static uint32_t SD_PowerON(SD_HandleTypeDef* hsd)
{
    ...
    if(hsd->SdCard.CardVersion == CARD_V2_X)
    {
        ...
        if(errorstate != HAL_SD_ERROR_NONE) { return HAL_SD_ERROR_UNSUPPORTED_FEATURE; }
    }
    /* SD CARD */
    ...
    if(count >= SDMMC_MAX_VOLT_TRIAL) { return HAL_SD_ERROR_INVALID_VOLTRANGE; }

    if((response & SDMMC_HIGH_CAPACITY) == SDMMC_HIGH_CAPACITY) /* (response &= SD_HIGH_CAPACITY) */
    {
        hsd->SdCard.CardType = CARD_SDHC_SDXC;
    }
    else { hsd->SdCard.CardType = CARD_SDSC; }
...
static uint32_t SD_SendSDStatus(SD_HandleTypeDef* hsd, uint32_t* pSDstatus)
{
    ...
    else if(__HAL_SD_GET_FLAG(hsd, SDIO_FLAG_DCRCFAIL)) { return HAL_SD_ERROR_DATA_CRC_FAIL; }
    else if(__HAL_SD_GET_FLAG(hsd, SDIO_FLAG_RXOVERR)) { return HAL_SD_ERROR_RX_OVERRUN; }
    else { /* Nothing to do */ }
...
static uint32_t SD_SendStatus(SD_HandleTypeDef* hsd, uint32_t* pCardStatus)
{
    ...
    if(pCardStatus == NULL) { return HAL_SD_ERROR_PARAM; }
...
static uint32_t SD_FindSCR(SD_HandleTypeDef* hsd, uint32_t* pSCR)
{
    ...
    while(!__HAL_SD_GET_FLAG(hsd, SDIO_FLAG_RXOVERR | SDIO_FLAG_DCRCFAIL | SDIO_FLAG_DTIMEOUT))
    {
        ...
        else if(!__HAL_SD_GET_FLAG(hsd, SDIO_FLAG_RXACT)) { break; }
