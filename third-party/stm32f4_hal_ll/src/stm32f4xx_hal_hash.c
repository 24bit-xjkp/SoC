...
static HAL_StatusTypeDef
    HASH_WaitOnFlagUntilTimeout(HASH_HandleTypeDef* hhash, uint32_t Flag, FlagStatus Status, uint32_t Timeout);
...
HAL_StatusTypeDef HAL_HASH_Init(HASH_HandleTypeDef* hhash)
{
    ...
    if(hhash == NULL) { return HAL_ERROR; }
    ...
    if(hhash->State == HAL_HASH_STATE_RESET)
    {
        ...
        if(hhash->MspInitCallback == NULL) { hhash->MspInitCallback = HAL_HASH_MspInit; }
...
HAL_StatusTypeDef HAL_HASH_DeInit(HASH_HandleTypeDef* hhash)
{
    ...
    hhash->DigestCalculationDisable = RESET;
        ...
        #if (USE_HAL_HASH_REGISTER_CALLBACKS == 1)
    if(hhash->MspDeInitCallback == NULL) { hhash->MspDeInitCallback = HAL_HASH_MspDeInit; }
 ...
 */
HAL_StatusTypeDef
    HAL_HASH_RegisterCallback(HASH_HandleTypeDef* hhash, HAL_HASH_CallbackIDTypeDef CallbackID, pHASH_CallbackTypeDef pCallback)
{
    ...
    if(HAL_HASH_STATE_READY == hhash->State)
    {
        switch(CallbackID)
        {
            case HAL_HASH_INPUTCPLT_CB_ID: hhash->InCpltCallback = pCallback; break;

            case HAL_HASH_DGSTCPLT_CB_ID: hhash->DgstCpltCallback = pCallback; break;

            case HAL_HASH_ERROR_CB_ID: hhash->ErrorCallback = pCallback; break;

            case HAL_HASH_MSPINIT_CB_ID: hhash->MspInitCallback = pCallback; break;

            case HAL_HASH_MSPDEINIT_CB_ID: hhash->MspDeInitCallback = pCallback; break;
 ...
 */
HAL_StatusTypeDef
    HAL_HASH_MD5_Start_IT(HASH_HandleTypeDef* hhash, const uint8_t* const pInBuffer, uint32_t Size, uint8_t* pOutBuffer)
{
 ...
 */
HAL_StatusTypeDef
    HAL_HASH_MD5_Accmlt_End_IT(HASH_HandleTypeDef* hhash, const uint8_t* const pInBuffer, uint32_t Size, uint8_t* pOutBuffer)
{
    return HASH_Start_IT(hhash, pInBuffer, Size, pOutBuffer, HASH_ALGOSELECTION_MD5);
}
 ...
 */
HAL_StatusTypeDef
    HAL_HASH_SHA1_Start_IT(HASH_HandleTypeDef* hhash, const uint8_t* const pInBuffer, uint32_t Size, uint8_t* pOutBuffer)
{
 ...
 */
HAL_StatusTypeDef
    HAL_HASH_SHA1_Accmlt_End_IT(HASH_HandleTypeDef* hhash, const uint8_t* const pInBuffer, uint32_t Size, uint8_t* pOutBuffer)
{
    return HASH_Start_IT(hhash, pInBuffer, Size, pOutBuffer, HASH_ALGOSELECTION_SHA1);
}
 ...
 */
HAL_StatusTypeDef
    HAL_HMAC_MD5_Start_IT(HASH_HandleTypeDef* hhash, const uint8_t* const pInBuffer, uint32_t Size, uint8_t* pOutBuffer)
{
 ...
 */
HAL_StatusTypeDef
    HAL_HMAC_SHA1_Start_IT(HASH_HandleTypeDef* hhash, const uint8_t* const pInBuffer, uint32_t Size, uint8_t* pOutBuffer)
{
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_HASH_GetStatus(const HASH_HandleTypeDef* hhash) { return hhash->Status; }
...
void HAL_HASH_ContextSaving(HASH_HandleTypeDef* hhash, uint8_t* pMemBuffer)
{
    ...
    *(uint32_t*)(mem_ptr) = READ_BIT(HASH->STR, HASH_STR_NBLW);
    mem_ptr += 4U;
        ...
        #if defined(HASH_CR_MDMAT)
    *(uint32_t*)(mem_ptr) =
        READ_BIT(HASH->CR, HASH_CR_DMAE | HASH_CR_DATATYPE | HASH_CR_MODE | HASH_CR_ALGO | HASH_CR_LKEY | HASH_CR_MDMAT);
        #else
...
HAL_StatusTypeDef HAL_HASH_DMAFeed_ProcessSuspend(HASH_HandleTypeDef* hhash)
{
    ...
    if(hhash->State == HAL_HASH_STATE_READY) { return HAL_ERROR; }
    else
    {
        ...
        if(HASH_WaitOnFlagUntilTimeout(hhash, HASH_FLAG_BUSY, SET, HASH_TIMEOUTVALUE) != HAL_OK) { return HAL_TIMEOUT; }

        if(__HAL_HASH_GET_FLAG(HASH_FLAG_DCIS) != RESET) { return HAL_ERROR; }

        /* Wait for BUSY flag to be set */
        if(HASH_WaitOnFlagUntilTimeout(hhash, HASH_FLAG_BUSY, RESET, HASH_TIMEOUTVALUE) != HAL_OK) { return HAL_TIMEOUT; }
        /* Disable DMA channel */
        /* Note that the Abort function will
          ...
          - Set the State
        */
        if(HAL_DMA_Abort(hhash->hdmain) != HAL_OK) { return HAL_ERROR; }
        ...
        else
        {
            ...
            if(((tmp_words_already_pushed + tmp_initial_DMATransferSize_inWords - tmp_remaining_DMATransferSize_inWords) % 16U) !=
               HASH_NBW_PUSHED())
            {
...
static HAL_StatusTypeDef HASH_WriteData(HASH_HandleTypeDef* hhash, const uint8_t* const pInBuffer, uint32_t Size)
{
    ...
    if((Size % 4U) != 0U)
    {
        if(hhash->Init.DataType == HASH_DATATYPE_16B)
        {
            ...
            if((Size % 4U) <= 2U) { HASH->DIN = (uint32_t)*(uint16_t*)inputaddr; }
            if((Size % 4U) == 3U) { HASH->DIN = *(uint32_t*)inputaddr; }
        }
        else if((hhash->Init.DataType == HASH_DATATYPE_8B) ||
                (hhash->Init.DataType == HASH_DATATYPE_1B)) /* byte swap or bit swap or */
        {
            /* Write remaining input data */
            if((Size % 4U) == 1U) { HASH->DIN = (uint32_t)*(uint8_t*)inputaddr; }
            if((Size % 4U) == 2U) { HASH->DIN = (uint32_t)*(uint16_t*)inputaddr; }
            if((Size % 4U) == 3U)
            {
        ...
        else { HASH->DIN = *(uint32_t*)inputaddr; }
...
static HAL_StatusTypeDef HMAC_Processing(HASH_HandleTypeDef* hhash, uint32_t Timeout)
{
    ...
    if(hhash->Phase == HAL_HASH_PHASE_HMAC_STEP_1)
    {
        ...
        if(hhash->Status != HAL_OK) { return hhash->Status; }
        ...
        if(HASH_WaitOnFlagUntilTimeout(hhash, HASH_FLAG_BUSY, SET, Timeout) != HAL_OK) { return HAL_TIMEOUT; }
    ...
    if(hhash->Phase == HAL_HASH_PHASE_HMAC_STEP_3)
    {
        ...
        if(HASH_WaitOnFlagUntilTimeout(hhash, HASH_FLAG_DCIS, RESET, Timeout) != HAL_OK) { return HAL_TIMEOUT; }
...
HAL_StatusTypeDef HASH_Accumulate(HASH_HandleTypeDef* hhash, const uint8_t* const pInBuffer, uint32_t Size, uint32_t Algorithm)
{
    ...
    if((Size % 4U) != 0U) { return HAL_ERROR; }
...
HAL_StatusTypeDef HASH_Accumulate_IT(HASH_HandleTypeDef* hhash, const uint8_t* const pInBuffer, uint32_t Size, uint32_t Algorithm)
{
    ...
    if((State_tmp == HAL_HASH_STATE_READY) || (State_tmp == HAL_HASH_STATE_SUSPENDED))
    {
        ...
        else
        {
            ...
            else { hhash->HashITCounter = 3; /* 'cruise-speed' reached during a previous buffer processing */ }
                                ...
                                uint32_t Algorithm)
{
    ...
    if((State_tmp == HAL_HASH_STATE_READY) || (State_tmp == HAL_HASH_STATE_SUSPENDED))
    {
        ...
        else { initialization_skipped = 1; /* info user later on in case of multi-buffer */ }
...
HAL_StatusTypeDef HASH_Finish(HASH_HandleTypeDef* hhash, uint8_t* pOutBuffer, uint32_t Timeout)
{
    ...
    {
        /* Check parameter */
        if(pOutBuffer == NULL) { return HAL_ERROR; }
                                ...
                                uint32_t Algorithm)
{
    ...
    if((State_tmp == HAL_HASH_STATE_READY) || (State_tmp == HAL_HASH_STATE_SUSPENDED))
    {
        ...
        else if(hhash->Phase == HAL_HASH_PHASE_HMAC_STEP_2) { /* Restart IT-based HASH processing after Step 2 suspension */ }
        else
        {
