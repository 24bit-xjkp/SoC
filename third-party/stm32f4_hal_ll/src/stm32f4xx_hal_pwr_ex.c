...
HAL_StatusTypeDef HAL_PWREx_EnableBkUpReg(void)
{
    ...
    *(__IO uint32_t*)CSR_BRE_BB = (uint32_t)ENABLE;
    ...
    {
        if((HAL_GetTick() - tickstart) > PWR_BKPREG_TIMEOUT_VALUE) { return HAL_TIMEOUT; }
    }
    return HAL_OK;
}
...
HAL_StatusTypeDef HAL_PWREx_ControlVoltageScaling(uint32_t VoltageScaling)
{
    ...
    {
        if((HAL_GetTick() - tickstart) > PWR_VOSRDY_TIMEOUT_VALUE) { return HAL_TIMEOUT; }
    }
    ...
    return HAL_OK;
        ...
        defined(STM32F401xC) || defined(STM32F401xE) || defined(STM32F410Tx) || defined(STM32F410Cx) || defined(STM32F410Rx) ||  \
...
HAL_StatusTypeDef HAL_PWREx_EnableOverDrive(void)
{
    ...
    {
        if((HAL_GetTick() - tickstart) > PWR_OVERDRIVE_TIMEOUT_VALUE) { return HAL_TIMEOUT; }
    }
