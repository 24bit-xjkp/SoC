...
HAL_StatusTypeDef HAL_LTDC_Init(LTDC_HandleTypeDef* hltdc)
{
    ...
    if(hltdc == NULL) { return HAL_ERROR; }
    ...
    if(hltdc->State == HAL_LTDC_STATE_RESET)
    {
        ...
        if(hltdc->MspInitCallback == NULL) { hltdc->MspInitCallback = HAL_LTDC_MspInit; }
    ...
    hltdc->Instance->GCR |=
        (uint32_t)(hltdc->Init.HSPolarity | hltdc->Init.VSPolarity | hltdc->Init.DEPolarity | hltdc->Init.PCPolarity);
...
HAL_StatusTypeDef HAL_LTDC_DeInit(LTDC_HandleTypeDef* hltdc)
{
    ...
    while(READ_BIT(hltdc->Instance->CDSR, LTDC_CDSR_VSYNCS) == 0U)
    {
        ...
        if((HAL_GetTick() - tickstart) > LTDC_TIMEOUT_VALUE) { break; }
    }
    ...
    __HAL_LTDC_DISABLE(hltdc);
        ...
        #if (USE_HAL_LTDC_REGISTER_CALLBACKS == 1)
    if(hltdc->MspDeInitCallback == NULL) { hltdc->MspDeInitCallback = HAL_LTDC_MspDeInit; }
    ...
    HAL_LTDC_RegisterCallback(LTDC_HandleTypeDef* hltdc, HAL_LTDC_CallbackIDTypeDef CallbackID, pLTDC_CallbackTypeDef pCallback)
{
    ...
    if(hltdc->State == HAL_LTDC_STATE_READY)
    {
        switch(CallbackID)
        {
            case HAL_LTDC_LINE_EVENT_CB_ID: hltdc->LineEventCallback = pCallback; break;

            case HAL_LTDC_RELOAD_EVENT_CB_ID: hltdc->ReloadEventCallback = pCallback; break;
            ...
            case HAL_LTDC_MSPDEINIT_CB_ID: hltdc->MspDeInitCallback = pCallback; break;
...
HAL_StatusTypeDef HAL_LTDC_ConfigCLUT(LTDC_HandleTypeDef* hltdc, uint32_t* pCLUT, uint32_t CLUTSize, uint32_t LayerIdx)
{
    ...
    for(counter = 0U; (counter < CLUTSize); counter++)
    {
        ...
        {
            tmp = (((counter + (16U * counter)) << 24U) | ((uint32_t)(*pcolorlut) & 0xFFU) | ((uint32_t)(*pcolorlut) & 0xFF00U) |
                   ((uint32_t)(*pcolorlut) & 0xFF0000U));
        }
        else
        {
            tmp = ((counter << 24U) | ((uint32_t)(*pcolorlut) & 0xFFU) | ((uint32_t)(*pcolorlut) & 0xFF00U) |
                   ((uint32_t)(*pcolorlut) & 0xFF0000U));
        }
...
static void LTDC_SetConfig(LTDC_HandleTypeDef* hltdc, LTDC_LayerCfgTypeDef* pLayerCfg, uint32_t LayerIdx)
{
    ...
    if(pLayerCfg->PixelFormat == LTDC_PIXEL_FORMAT_ARGB8888) { tmp = 4U; }
    else if(pLayerCfg->PixelFormat == LTDC_PIXEL_FORMAT_RGB888) { tmp = 3U; }
    else if((pLayerCfg->PixelFormat == LTDC_PIXEL_FORMAT_ARGB4444) || (pLayerCfg->PixelFormat == LTDC_PIXEL_FORMAT_RGB565) ||
            (pLayerCfg->PixelFormat == LTDC_PIXEL_FORMAT_ARGB1555) || (pLayerCfg->PixelFormat == LTDC_PIXEL_FORMAT_AL88))
    {
        tmp = 2U;
    }
    else { tmp = 1U; }
