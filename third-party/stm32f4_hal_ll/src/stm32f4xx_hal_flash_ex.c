...
HAL_StatusTypeDef HAL_FLASHEx_OBProgram(FLASH_OBProgramInitTypeDef* pOBInit)
{
    ...
    if((pOBInit->OptionType & OPTIONBYTE_RDP) == OPTIONBYTE_RDP) { status = FLASH_OB_RDP_LevelConfig(pOBInit->RDPLevel); }
    ...
    if((pOBInit->OptionType & OPTIONBYTE_BOR) == OPTIONBYTE_BOR) { status = FLASH_OB_BOR_LevelConfig(pOBInit->BORLevel); }
...
void FLASH_Erase_Sector(uint32_t Sector, uint8_t VoltageRange)
{
    ...
    if(VoltageRange == FLASH_VOLTAGE_RANGE_1) { tmp_psize = FLASH_PSIZE_BYTE; }
    else if(VoltageRange == FLASH_VOLTAGE_RANGE_2) { tmp_psize = FLASH_PSIZE_HALF_WORD; }
    else if(VoltageRange == FLASH_VOLTAGE_RANGE_3) { tmp_psize = FLASH_PSIZE_WORD; }
    else { tmp_psize = FLASH_PSIZE_DOUBLE_WORD; }

    /* Need to add offset of 4 when sector higher than FLASH_SECTOR_11 */
    if(Sector > FLASH_SECTOR_11) { Sector += 4U; }
...
static HAL_StatusTypeDef FLASH_OB_EnableWRP(uint32_t WRPSector, uint32_t Banks)
{
    ...
    if(status == HAL_OK)
    {
        ...
        if((WRPSector == OB_WRP_SECTOR_All) && (Banks == FLASH_BANK_BOTH))
        {
            ...
            if(status == HAL_OK) { *(__IO uint16_t*)OPTCR1_BYTE2_ADDRESS &= (~(WRPSector >> 12)); }
        }
    }
    return status;
}
...
static HAL_StatusTypeDef FLASH_OB_DisableWRP(uint32_t WRPSector, uint32_t Banks)
{
    ...
    if(status == HAL_OK)
    {
        ...
        if((WRPSector == OB_WRP_SECTOR_All) && (Banks == FLASH_BANK_BOTH))
        {
            ...
            if(status == HAL_OK) { *(__IO uint16_t*)OPTCR1_BYTE2_ADDRESS |= (uint16_t)(WRPSector >> 12); }
        }
    }
...
static HAL_StatusTypeDef FLASH_OB_EnableWRP(uint32_t WRPSector, uint32_t Banks)
{
    ...
    if(status == HAL_OK) { *(__IO uint16_t*)OPTCR_BYTE2_ADDRESS &= (~WRPSector); }
...
static HAL_StatusTypeDef FLASH_OB_DisableWRP(uint32_t WRPSector, uint32_t Banks)
{
    ...
    if(status == HAL_OK) { *(__IO uint16_t*)OPTCR_BYTE2_ADDRESS |= (uint16_t)WRPSector; }
...
static HAL_StatusTypeDef FLASH_OB_EnablePCROP(uint32_t Sector)
{
    ...
    if(status == HAL_OK) { *(__IO uint16_t*)OPTCR_BYTE2_ADDRESS |= (uint16_t)Sector; }
...
static HAL_StatusTypeDef FLASH_OB_DisablePCROP(uint32_t Sector)
{
    ...
    if(status == HAL_OK) { *(__IO uint16_t*)OPTCR_BYTE2_ADDRESS &= (~Sector); }
...
static HAL_StatusTypeDef FLASH_OB_RDP_LevelConfig(uint8_t Level)
{
    ...
    if(status == HAL_OK) { *(__IO uint8_t*)OPTCR_BYTE1_ADDRESS = Level; }
...
static uint8_t FLASH_OB_GetRDP(void)
{
    ...
    if(*(__IO uint8_t*)(OPTCR_BYTE1_ADDRESS) == (uint8_t)OB_RDP_LEVEL_2) { readstatus = OB_RDP_LEVEL_2; }
    else if(*(__IO uint8_t*)(OPTCR_BYTE1_ADDRESS) == (uint8_t)OB_RDP_LEVEL_0) { readstatus = OB_RDP_LEVEL_0; }
    else { readstatus = OB_RDP_LEVEL_1; }
