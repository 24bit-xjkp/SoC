...
ErrorStatus LL_RTC_TIME_Init(RTC_TypeDef* RTCx, uint32_t RTC_Format, LL_RTC_TimeTypeDef* RTC_TimeStruct)
{
    ...
    if(LL_RTC_EnterInitMode(RTCx) != ERROR)
    {
        ...
        if(LL_RTC_IsShadowRegBypassEnabled(RTCx) == 0U) { status = LL_RTC_WaitForSynchro(RTCx); }
        else { status = SUCCESS; }
    }
    /* Enable the write protection for RTC registers */
    LL_RTC_EnableWriteProtection(RTCx);
...
ErrorStatus LL_RTC_ALMA_Init(RTC_TypeDef* RTCx, uint32_t RTC_Format, LL_RTC_AlarmTypeDef* RTC_AlarmStruct)
{
    ...
    if(RTC_Format == LL_RTC_FORMAT_BIN)
    {
        ...
        else { assert_param(IS_LL_RTC_WEEKDAY(RTC_AlarmStruct->AlarmDateWeekDay)); }
    }
    else
    {
        if(LL_RTC_GetHourFormat(RTCx) != LL_RTC_HOURFORMAT_24HOUR)
        ...
        else { assert_param(IS_LL_RTC_WEEKDAY(__LL_RTC_CONVERT_BCD2BIN(RTC_AlarmStruct->AlarmDateWeekDay))); }
    }
    ...
    if(RTC_AlarmStruct->AlarmDateWeekDaySel == LL_RTC_ALMA_DATEWEEKDAYSEL_DATE)
    {
        ...
        if(RTC_Format != LL_RTC_FORMAT_BIN) { LL_RTC_ALMA_SetDay(RTCx, RTC_AlarmStruct->AlarmDateWeekDay); }
        else { LL_RTC_ALMA_SetDay(RTCx, __LL_RTC_CONVERT_BIN2BCD(RTC_AlarmStruct->AlarmDateWeekDay)); }
    }
    else
    {
...
ErrorStatus LL_RTC_ALMB_Init(RTC_TypeDef* RTCx, uint32_t RTC_Format, LL_RTC_AlarmTypeDef* RTC_AlarmStruct)
{
    ...
    if(RTC_AlarmStruct->AlarmDateWeekDaySel == LL_RTC_ALMB_DATEWEEKDAYSEL_DATE)
    {
        ...
        if(RTC_Format != LL_RTC_FORMAT_BIN) { LL_RTC_ALMB_SetDay(RTCx, RTC_AlarmStruct->AlarmDateWeekDay); }
        else { LL_RTC_ALMB_SetDay(RTCx, __LL_RTC_CONVERT_BIN2BCD(RTC_AlarmStruct->AlarmDateWeekDay)); }
    }
    else
    {
        /* Set the week day for ALARM */
...
ErrorStatus LL_RTC_EnterInitMode(RTC_TypeDef* RTCx)
{
    ...
    if(LL_RTC_IsActiveFlag_INIT(RTCx) == 0U)
    {
        ...
        {
            if(LL_SYSTICK_IsActiveCounterFlag() == 1U) { timeout--; }
            tmp = LL_RTC_IsActiveFlag_INIT(RTCx);
